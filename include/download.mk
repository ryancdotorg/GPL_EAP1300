...
define DownloadMethod/svn
	...
	$(SCRIPT_DIR)/download.pl "$(DL_DIR)" "$(FILE)" "$(MD5SUM)" "ftp://ftpuser:senao123@rdserver.senao.com/SmartOpenWrt/sources"
endef

define DownloadMethod/snsvn
    $(call wrap_mirror, \
        echo "Checking out files from the svn repository..."; \
        mkdir -p $(TMP_DIR)/dl && \
        cd $(TMP_DIR)/dl && \
        ...
        [ \! -d $(SUBDIR) ] && \
        ( svn help export | grep -q trust-server-cert && \
        svn export --non-interactive --trust-server-cert $(URL) $(SUBDIR) || \
        svn export --non-interactive $(URL) $(SUBDIR) ) && \
        echo "Packing checkout..." && \
        $(call dl_pack,$(TMP_DIR)/dl/$(FILE),$(SUBDIR)) && \
        mv $(TMP_DIR)/dl/$(FILE) $(DL_DIR)/ && \
        rm -rf $(SUBDIR); \

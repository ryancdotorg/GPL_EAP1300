...
sn-prepare-tmpinfo:
	@-grep '^CONFIG_EXTERNAL_KERNEL_TREE' $(TOPDIR)/SENAO/configs/product_config.make > tmp/.sn_ksrc || true
	@-grep '^CONFIG_LINUX' $(TOPDIR)/SENAO/configs/product_config.make >> tmp/.sn_ksrc || true
	@-grep '^CONFIG_TARGET_' $(TOPDIR)/SENAO/configs/product_config.make >> tmp/.sn_ksrc || true

prepare-tmpinfo: sn-prepare-tmpinfo FORCE
	mkdir -p tmp/info
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f include/scan.mk SCAN_TARGET="packageinfo" SCAN_DIR="package" SCAN_NAME="package" SCAN_DEPS="$(TOPDIR)/include/package*.mk $(TOPDIR)/overlay/*/*.mk" SCAN_DEPTH=6 SCAN_EXTRA=""
...
#./SENAO/scripts/luci_version.sh $(TOPDIR) menu
ifneq ($(wildcard SENAO/scripts/luci_version.sh),)
ifneq ($(wildcard SENAO/package/senao-luci),)
define luci_version
	./SENAO/scripts/luci_version.sh $(TOPDIR) $(1)
endef
endif
endif

menuconfig: scripts/config/mconf prepare-tmpinfo FORCE
	$(call luci_version,menu)
...
%::
	...
	@( \
		...
		if ./scripts/kconfig.pl '>' .config tmp/.config | grep -v "^#" | grep -q CONFIG; then \

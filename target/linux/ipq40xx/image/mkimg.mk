...
NOR_MIBIB=nor-system-partition-ipq40xx.bin
NAND_MIBIB=nand-system-partition-ipq40xx.bin
NORPLUSNAND_MIBIB=norplusnand-system-partition-ipq40xx.bin
...
define GetFile
	#git lfs pull
	$(CP) files/$(QCA_SP_REPO)/* -P $(build_dir)
endef
endif
...
nor_parts_pad="$(shell echo `grep -oP '(?<=pad_kb length="4">)[^<]+' "$(SN_CONFIGS_DIR)/ipq-image-config/nor/nor-partition.xml"`)"
...
norplusnand_parts_pad="$(shell echo `grep -oP '(?<=pad_kb length="4">)[^<]+' "$(SN_CONFIGS_DIR)/ipq-image-config/norplusnand/nor-plus-nand-parition.xml"`)"
nand_parts="$(shell echo `grep -oP '(?<=size_kb length="4">)[^<]+' "$(SN_CONFIGS_DIR)/ipq-image-config/nand/nand-partition.xml"`)"
nand_parts_pad="$(shell echo `grep -oP '(?<=pad_kb length="4">)[^<]+' "$(SN_CONFIGS_DIR)/ipq-image-config/nand/nand-partition.xml"`)"
...
define GenMIBIB
	...
	$(PYTHON) $(build_dir)/genimg.py \
		--partition_tool=$(BIN_DIR)/partition_tool \
...
define CopyRenameImg	
	...
	-cp $(BIN_DIR)/$(IMG_PREFIX)-ipq40xx-$(FW_ROOTFS_SUFFIX) $(BIN_DIR)/$(IMG_PREFIX)-$(PRODUCT_NAME_L)-$(FW_ROOTFS_SUFFIX)	
endef

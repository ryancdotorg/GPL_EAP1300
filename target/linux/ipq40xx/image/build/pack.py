...
class Pack(object):
    ...
    DEFAULT_NORPLUSNAND_ROOTFS_IMG_SIZE = (64 * 1024 * 1024)
    norplusnand_rootfs_img_size = DEFAULT_NORPLUSNAND_ROOTFS_IMG_SIZE
    dualimage_support = None
    ...
    def __gen_flash_script(self, info, script, flinfo):
        ...
        """
        count = 0
#        print "norplusnand_rootfs_img_size=%x" % Pack.norplusnand_rootfs_img_size
#        print "dualimage_support=%s" % Pack.dualimage_support
        if Pack.dualimage_support == "y":
            Pack.norplusnand_rootfs_img_size = Pack.norplusnand_rootfs_img_size/2
#        print "norplusnand_rootfs_img_size=%x" % Pack.norplusnand_rootfs_img_size
    ...
    def __process_board_flash(self, ftype, board_section, machid, images):
        ...
        """
#        print "ftype=%s" % ftype
#        print "board_section=%s" % board_section
        pagesize_param = "%s_pagesize" % ftype
    ...
    def main_bconf(self, flash_type, images_dname, out_fname, brdconfig, nand_flash_chipsize, dualimage):
        ...
        """
        """
        print "flash_type: " + flash_type
        print "images_dname: " + images_dname
        print "out_fname: " + out_fname
        print "brdconfig: " + brdconfig
        print "nand_flash_chipsize : %x" % nand_flash_chipsize
        print "dualimage: %s" % dualimage
        """
        self.flash_type = flash_type
        self.images_dname = images_dname
        self.img_fname = out_fname
        Pack.norplusnand_rootfs_img_size = nand_flash_chipsize
#        print "norplusnand_rootfs_img_size=%x" % Pack.norplusnand_rootfs_img_size
        Pack.dualimage_support = dualimage
        print "dualimage_support=%s" % Pack.dualimage_support
...
class ArgParser(object):
    ...
    # The default flash size is 256MB
    ...
    DEFAULT_BCONF_FNAME = "boardconfig"
    ...
    def __init__(self):
        ...
        self.ipq_nand = False
        self.bconf = False
        self.bconf_fname = ArgParser.DEFAULT_BCONF_FNAME
    ...
    def __init_bconf_fname(self, brdconfig):
        """Set the board configuration filename from command line argument

        bconf_fname -- string, the board configuration filename
        """

        if brdconfig == None:
            self.bconf_fname = ArgParser.DEFAULT_BCONF_FNAME
        else:
            self.bconf_fname = brdconfig

        try:
            bconf_fname = os.path.join(brdconfig)
            bconf_fp = open(bconf_fname)
            self.bconf = ConfigParser()
            self.bconf.readfp(bconf_fp)
            bconf_fp.close()
    ...
    def __init_get_pagesize_from_bconf(self, bconf_fname):
        """Get the pagesize, from the boardconf file.        

        Raise UsageError, if pagesize is invalid
        """
        if bconf_fname == None:
            bconf_fname = ArgParser.DEFAULT_BCONF_FNAME

        self.bconf.read(bconf_fname)
        configlist = self.bconf.sections()

        for section in ["ipq40xx"]:
            nand_pagesize = self.bconf.get(section, 'nand_pagesize')
            if nand_pagesize:
                pagesize = "%s" %nand_pagesize
#                print "nand_pagesize=%s" % nand_pagesize
                return nand_pagesize

    def __init_get_pages_per_block_from_bconf(self, bconf_fname):
        """Get the pages_per_block, from the boardconf file.      
        ...
        for section in ["ipq40xx"]:
            nand_pages_per_block = self.bconf.get(section, 'nand_pages_per_block')
            if nand_pages_per_block:
                pages_per_block = "%s" %nand_pages_per_block
#                print "nand_pages_per_block=%s" % nand_pages_per_block
                return nand_pages_per_block

    def __init_get_blocks_per_chip_from_bconf(self, bconf_fname):
        """Get the blocks_per_chip, from boardconf file.        
        ...
        for section in ["ipq40xx"]:
            nand_total_blocks = self.bconf.get(section, 'nand_total_blocks')
            if nand_total_blocks:
                blocks_per_chip = "%s" %nand_total_blocks
#                print "nand_total_blocks=%s" % nand_total_blocks
                return nand_total_blocks

    def get_nand_flash_chipsize(self):
        if self.__chipsize:
#            print "get chipsize=%x" % self.__chipsize
            return self.__chipsize

    def get_dualimage(self):
        if self.dualimage :
#            print "get dualimage=%s" % self.dualimage
            return self.dualimage
    ...
    def parse(self, argv):
        ...
        try:
            opts, args = getopt(argv[1:], "Bib:hp:t:o:c:d:m:f:F:")
        ...
        """
        print "flash_type: " + flash_type
        print "pagesize: " + "{}".format(pagesize)
        print "pages_per_block: " + "{}".format(pages_per_block)
        print "blocks_per_chip: " + "{}".format(blocks_per_chip)
        print "output_filename: " + out_fname
        print "partition_filename: " + part_fname
        print "flash_config: " + fconf_fname
        print "bconf: " +  "{}".format(bconf)
        print "boardconfig: " + bconf_fname        
        print "dualimage: %s" %dualimage
        """

        self.__init_bconf_fname(bconf_fname)
#        print "self.bconf_fname=%s" % self.bconf_fname
#        print "bconf_fname=%s" % bconf_fname
        pagesize = self.__init_get_pagesize_from_bconf(bconf_fname)
#        print "pagesize=%s" % pagesize
        pages_per_block = self.__init_get_pages_per_block_from_bconf(bconf_fname)
#        print "pages_per_block=%s" % pages_per_block
        blocks_per_chip = self.__init_get_blocks_per_chip_from_bconf(bconf_fname)
#        print "blocks_per_chip=%s" % blocks_per_chip
        self.__init_flash_type(flash_type)
#        print "flash_type: " + flash_type
        self.__init_pagesize(pagesize)
#        print "self.pagesize=%x" % self.__pagesize
        self.__init_blocksize(pages_per_block)
#        print "self.blocksize=%x" % self.__blocksize
        self.__init_chipsize(blocks_per_chip)
#        print "self.chipsize=%x" % self.__chipsize
        ...
        self.__init_out_fname(out_fname, self.images_dname,
                              ...
                              self.__chipsize / self.__blocksize,
                              bconf)
#        print "self.out_fname=%s" % self.out_fname
        self.__init_part_fname(part_fname)
#        print "self.part_fname=%s" % self.part_fname
        self.__init_fconf_fname(fconf_fname)
#        print "self.fconf_fname=%s" % self.fconf_fname
        self.ipq_nand = ipq_nand
        self.bconf = bconf
        
        self.dualimage = dualimage
#        print "self.dualimage=%s" % self.dualimage
...
def main():
    ...
    try:
        ...
        chipsize = parser.get_nand_flash_chipsize()
        dualimage = parser.get_dualimage()
    except UsageError, e:
        parser.usage(e.args[0])
        sys.exit(1)

    pack = Pack()
#    print "chipsize=%x" % chipsize
#    print "dualimage=%s" % dualimage
    if parser.bconf:

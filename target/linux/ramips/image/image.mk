...
define Build/append-kernel
	@echo -e "            $(_G)Build/append-kernel$(_N)"
	dd if=$(word 1,$^) $(if $(1),bs=$(1) conv=sync) >> $@
endef

define Build/append-rootfs
	@echo -e "            $(_G)Build/append-rootfs$(_N)"
...
define Build/pad-rootfs
	@echo -e "            $(_G)Build/pad-rootfs$(_N)"
	$(call prepare_generic_squashfs,$@ $(1))
endef

define Build/pad-offset
	@echo -e "            $(_G)Build/pad-offset$(_N)"
	let \
		size="$$(stat -c%s $@)" \
		pad="$(word 1, $(1))" \
		offset="$(word 2, $(1))" \

...
class Pack(object):
	...
	file_exists = 1
        ...
        if flinfo.type == "emmc" and (image_type == "all" or image_type == "hlos"): #senao mod: for norplusemmc/emmc fw-s.img
        ...
        for index in range(parts_length):
            ...
            elif index == (parts_length - 1) and flinfo.type == "emmc" and (image_type == "all" or image_type == "hlos"): #senao mod: for norplusemmc/emmc fw-s.img
                if self.flash_type == "norplusemmc":
                    part_info = root.find(".//data[@type='NORPLUSEMMC_PARAMETER']")
                else:
                    part_info = root.find(".//data[@type='EMMC_PARAMETER']")
		...
		try:
			#if image_type == "all" or section.attrib['image_type'] == image_type:
			if image_type == "all" or section[8].attrib['image_type'] == image_type: # Senao: fix issue that cdt is not included in senao loader-s fit image 
	...
	file_exists = 1
        ...
        if (self.flash_type == "norplusemmc" and flinfo.type == "emmc" and image_type != "sn_loader_s") or (self.flash_type != "norplusemmc"): #senao mod: for norplusemmc/emmc loader-s.img
            if flinfo.type == "emmc":
        ...
        #if flinfo.type == "emmc" and image_type == "all":
        if flinfo.type == "emmc" and (image_type == "all" or image_type == "hlos"): #senao mod: for norplusemmc fw-s.img
            first = True
			...
			if image_type == "all" or section[8].attrib['image_type'] == image_type:
			    ...
			    try:
			 	...
			 	pass
			        ...
			        pass
                        else:                   ## Senao: add for build Senao FIT loader image
                            continue            ##
                    except IndexError, e:
                        if index == (parts_length - 1):
                            return
    ...
    def __mkimage(self, images):
        ...
        try:
            cmd = [SRC_DIR + "/bin/mkimage", "-f", self.its_fname, self.img_fname]
	...
	global MODE
		...
		if MODE == "64":
                ...
                for line in f1:
                    f2.write(line.replace('image=', "image=" + self.images_dname + "/"))
                f1.close()
                f2.close()

                if ftype in ["nand-4k", "nand-audio-4k", "norplusnand-4k"]:
                    UBI_IMG_NAME = "openwrt-ipq-ipq60xx" + MODE_APPEND + "-ubi-root-m4096-p256KiB.img"
                    cmd = '%s -m 4096 -p 256KiB -o root.ubi %s' % ((SRC_DIR + "/bin/ubinize") ,UBINIZE_CFG_NAME)
                    ret = subprocess.call(cmd, shell=True)
                    if ret != 0:
                         error("ubinization got failed")
                    cmd = 'dd if=root.ubi of=%s bs=4k conv=sync' % (self.images_dname + "/" + UBI_IMG_NAME)
                    ret = subprocess.call(cmd, shell=True)
                    if ret != 0:
                    ...
                    cmd = '%s -m 2048 -p 128KiB -o root.ubi %s' % ((SRC_DIR + "/bin/ubinize") ,UBINIZE_CFG_NAME)
                    ret = subprocess.call(cmd, shell=True)
                    if ret != 0:
                         error("ubinization got failed")
                    cmd = 'dd if=root.ubi of=%s bs=2k conv=sync' % (self.images_dname + "/" + UBI_IMG_NAME)
                    ret = subprocess.call(cmd, shell=True)
                    if ret != 0:
                         error("ubi image copy operation failed")
...
# Set flash type to default type (nand) if not given by user
	    ...
	    for flash_type in self.flash_type.split(","):
                ...
                if flash_type not in [ "nand", "nand-4k", "nor", "tiny-nor", "emmc", "norplusnand", "norplusnand-4k", "norplusemmc" ]: # senao add nand-4k norplusnand-4k
...
# senaeo mod: pack one flash-type at a time

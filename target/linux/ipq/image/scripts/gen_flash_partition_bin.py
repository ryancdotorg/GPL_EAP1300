...
def process_nand(config_path):
	...
	# 2018.4.20 Senao George
	nand_partition = nand_partition.replace('$$', rootdir)

	nand_parts = Nand_Params(nand_pagesize, nand_pages_per_block, nand_total_blocks)

	mbn_gen = '$$/scripts/nand_mbn_generator.py'
	# 2018.4.20 Senao George
	mbn_gen = mbn_gen.replace('$$', rootdir)

	# 2108.4.20 Senao George
	if ARCH_NAME == "ipq806x":
		partition_tool = rootdir + '/bin' + '/nor_tool'
	else:
		partition_tool = rootdir + '/bin' + '/partition_tool'
...
def process_nor(config_path):
	...
	nor_partition = nor_partition.replace('$$', rootdir)
	...
	print '\tCreating system partition'
	#prc = subprocess.Popen([partition_tool,'-s',str(nor_parts.pagesize),'-p',str(nor_parts.pages_per_block),'-b',str(nor_parts.total_blocks),'-c',str(1),'-u',userpart_path,'-o',syspart,], cwd=outputdir)
	prc = subprocess.Popen([partition_tool,'-s',str(nor_parts.pagesize),'-p',str(nor_parts.pages_per_block),'-b',str(nor_parts.total_blocks),'-c',str(1),'-u',userpart_path,'-o',syspart,])
	prc.wait()
	if prc.returncode != 0:
		print 'ERROR: unable to create system partition'
		return prc.returncode
...
def process_norplusnand(config_path):
	...
	nand_pages_per_block = int(nand_param.find('pages_per_block').text)
	nand_total_blocks = int(nand_param.find('total_block').text)
	nor_param = root.find(".//data[@type='NORPLUSNAND_PARAMETER']")
	nor_pagesize = int(nor_param.find('page_size').text)
	nor_pages_per_block = int(nor_param.find('pages_per_block').text)
	nor_total_blocks = int(nor_param.find('total_block').text)
	norplusnand_partition = "$$/" + ARCH_NAME + "/flash_partition/norplusnand-partition.xml"
	norplusnand_partition = norplusnand_partition.replace('$$', rootdir)
...
def process_emmc(config_path):
	...
	emmc_partition = emmc_partition.replace('$$', rootdir)
	...
	ptool = ptool.replace('$$', rootdir)
	msp = '$$/scripts/msp.py'
	msp = msp.replace('$$', rootdir)
...
def process_norplusemmc(config_path):
	...
	emmc_partition = "$$/" + ARCH_NAME + "/flash_partition/emmc-partition.xml"
	norplusemmc_partition = norplusemmc_partition.replace('$$', rootdir)
	emmc_partition = emmc_partition.replace('$$', rootdir)
...
def main():
	...
	if len(sys.argv) > 1:
		try:
			opts, args = getopt(sys.argv[1:], "c:f:o:r:")

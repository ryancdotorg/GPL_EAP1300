...
def process_bootconfig(config_file):
	...
	# 2018.4.19 Senao George
	bootconfig = "$$/" + ARCH_NAME + "/bootconfig/bootconfig.xml"
	bootconfig = bootconfig.replace('$$', rootdir)
...
# Create user partition
	...
	bootconfig_gen = rootdir + 'bin' + '/bootconfig_tool'
...
# Senao George, change cwd (current working directory) to prevent error
	prc = subprocess.Popen([bootconfig_gen,'-s',str(nand_pagesize), \
	'-p', str(nand_pages_per_block), '-i', rawpart_path, '-o', \
	nand_bootconfig], cwd=outputdir)
	prc.wait()
	if prc.returncode != 0:
...
def main():
	...
	if len(sys.argv) > 1:
		try:
			opts, args = getopt(sys.argv[1:], "c:o:r:")

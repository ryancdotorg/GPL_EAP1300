	...
	file_exists = 1
		...
		if img != None and 'soc_version' in img.attrib:
		    ...
		    try:
				...
				soc_version = img.get('soc_version')
				filename = img.text
                                print "====Iven debug==== filename==%s" % filename
				    ...
				    if os.path.exists(os.path.join(self.images_dname, filename_qcn9000)):
                                    ...
                                    print "====Iven debug2==== filename==%s" % filename_qcn9000
				if 'optional' in img.attrib:
				    if not os.path.exists(os.path.join(self.images_dname, filename)):
					file_exists = 0
	...
	global IF_QCN9000
        ...
        if flinfo.type != "emmc":
            ...
            if QCN9000:
                ...
                if root.find(".//data[@type='SENAO']/sn_qcn9000") != None and int(root.find(".//data[@type='SENAO']/sn_qcn9000").text): #senao add 
                    script.append('if true; then\n', fatal=False)            #senao add
                else:

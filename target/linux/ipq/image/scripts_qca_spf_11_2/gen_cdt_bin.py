...
def process_cdtinfo(cdt_info):
	...
	configdir = rootdir + ARCH_NAME + '/machid_xml'
	configdir = configdir.replace('$$', cdir)

	cdt_gen = rootdir+'/scripts/cdt_generator.py'
...
def main():
	...
	if len(sys.argv) > 1:
		try:
			opts, args = getopt(sys.argv[1:], "c:o:r:m:")
	...
	srcDir = rootdir + ARCH_NAME + '/machid_xml'
	srcDir = srcDir.replace('$$', cdir)
	if not os.path.exists(srcDir):
		os.makedirs(srcDir)
	cdt_path = rootdir + ARCH_NAME + '/cdt/'
		...
		for entry in entries:
			...
			if memory_profile != "default":
				if memory_profile == '256' or memory_profile == '512':
					...
					config_memory_size_in_bits = int(config_memory_in_bits[0]) * int(config_memory_in_bits[1])

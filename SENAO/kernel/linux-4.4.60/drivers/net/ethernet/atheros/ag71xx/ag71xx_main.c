...
#ifdef CONFIG_SENAO_SUPPORT_LAN_LED_BY_LINK_STATE
#define SENAO_SUPPORT_LAN_LED_BY_LINK_STATE 1
#endif

#if SENAO_SUPPORT_LAN_LED_BY_LINK_STATE
...
#if SENAO_SUPPORT_LAN_LED_BY_LINK_STATE
#define AP152_GPIO_LED_LAN1    1
#define AP152_GPIO_LED_LAN2    2
struct work_struct work_lan_link_detect;
unsigned int lan_link_state=0;
...
static int lan_link_state_open(struct inode *inode, struct file *file)
{
	single_open(file, athprocinfo_show_ul, &lan_link_state);
	return 0;
}

static struct file_operations lan_link_state_fops = {
	.open = lan_link_state_open,
	.read = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
};

static void check_lan_link_state(struct work_struct *work)
{
	struct device_node *mdio_node = NULL;
	struct platform_device *mdio_plat = NULL;
	struct mii_bus *miibus = NULL;
	struct ag71xx_mdio *mdio_data = NULL;
	...
	volatile int link;
	for (i = 0; i <= 4; i++) {
		link = mdiobus_read(miibus, i, MII_BMSR);
	...
	/*Control LAN LED by link-state, [1 is LED off; 0 is LED on]*/
	if(lan_link_state == 2)
	{
		gpio_set_value_cansleep(AP152_GPIO_LED_LAN1,0);
		gpio_set_value_cansleep(AP152_GPIO_LED_LAN2,1);
	}
	else if (lan_link_state == 4)
	{
		gpio_set_value_cansleep(AP152_GPIO_LED_LAN1,1);
		gpio_set_value_cansleep(AP152_GPIO_LED_LAN2,0);
	}
	else if (lan_link_state == 6)
	{
		gpio_set_value_cansleep(AP152_GPIO_LED_LAN1,0);
		gpio_set_value_cansleep(AP152_GPIO_LED_LAN2,0);
	}
	...
	schedule_work(&work_lan_link_detect);
	msleep(1000);

}

static void ag71xx_add_LAN_LINK_STATE(void)
{
	proc_create("LAN_LINK_STATE", 0644, NULL, &lan_link_state_fops);
	INIT_WORK(&work_lan_link_detect, (void(*)(struct work_struct *)) check_lan_link_state);
	schedule_work(&work_lan_link_detect);
}

static void ag71xx_remove_LAN_LINK_STATE(void)
{
	remove_proc_entry("LAN_LINK_STATE", NULL);
}

#endif //SENAO_SUPPORT_LAN_LED_BY_LINK_STATE

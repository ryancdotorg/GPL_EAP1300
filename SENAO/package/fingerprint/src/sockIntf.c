...
static int gen_random(char *output, int size)
{
    ...
    if (fp < 0)
    {
        fingersync_printf(FINGER_DEBUG, "/dev/urandom not exist");
        return -1;
    }
    else
    {
...
int read_interface(char *interface, int *ifindex, uint32_t *addr)
{
    ...
    if((fd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) >= 0)
    {
        ...
        if(addr)
        {
            if(ioctl(fd, SIOCGIFADDR, &ifr) == 0)
            {
                ...
                *addr = our_ip->sin_addr.s_addr;
                fingersync_printf(FINGER_DEBUG, "%s (our ip) = %s\n", ifr.ifr_name, inet_ntoa(our_ip->sin_addr));
            }
            else
            {
                fingersync_printf(FINGER_ERROR, "SIOCGIFADDR failed, is the interface up and configured?\n");
                close(fd);
                return -1;
            }
        }
        ...
        {
            fingersync_printf(FINGER_DEBUG, "adapter index %d\n", ifr.ifr_ifindex);
            *ifindex = ifr.ifr_ifindex;
        }
        else
        {
            fingersync_printf(FINGER_ERROR, "SIOCGIFINDEX failed!\n");
            close(fd);
            return -1;
        }
    }
    ...
    {
        fingersync_printf(FINGER_ERROR, "socket failed!\n");
        close(fd);
        return -1;
    }
    close(fd);
...
int listen_socket(uint32_t ip, int port, char *inf)
{
    ...
    fingersync_printf(FINGER_DEBUG, "Opening listen socket on 0x%08x:%d %s\n", ip, port, inf);
    if((fd = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
    {
        fingersync_printf(FINGER_ERROR, "socket call failed\n");
        return -1;
    }
...
int udp_packet_enc(uint8_t *payload, int payloadSize, uint32_t dest_ip, int dest_port, unsigned char *type)
{
    ...
    /* finger_sync_pkt_t *body = NULL; */
    int header_length = 0;


    header = (finger_sync_header_pkt_t *) packet;
    header->type = type;
    header->magic[0] = FINGER_MAJOR_MAGIC;
    header->magic[1] = FINGER_MINOR_MAGIC;
    header->magic[2] = FINGER_NEW_MAJOR_MAGIC;
    header->magic[3] = FINGER_NEW_MINOR_MAGIC;

    header->version[0] = FINGER_MAJOR_VERSION;
    header->version[1] = FINGER_MINOR_VERSION;

    /* body = (finger_sync_pkt_t *) payload; */

    if(gen_random((char *)salt, 8) < 0)
    {
        fingersync_printf(FINGER_DEBUG, "cannot gen random");
    }
...
#else
    len = payloadSize;
    memcpy(&packet[header_length], payload, len);
    bytes = udp_packet(packet, len + header_length, dest_ip, dest_port);
#endif
...
int udp_packet(uint8_t *payload, int payloadSize, uint32_t dest_ip, int dest_port)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_BROADCAST, &broadcast, sizeof broadcast) == -1) {
        fingersync_printf(FINGER_ERROR, "setsockopt (SO_BROADCAST)");
        close(fd);
        return 0;
    }
    ...
    if(result <= 0)
    {
        fingersync_printf(FINGER_ERROR, "write on socket failed\n");
    }
    close(fd);
        ...
        uint32_t dest_ip, int dest_port, uint8_t *dest_arp, int ifindex)
{
    ...
    {
        fingersync_printf(FINGER_ERROR, "bind call failed\n");
        close(fd);
        return -1;
    }

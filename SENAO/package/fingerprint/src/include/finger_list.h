#ifndef _FINGER_LIST_H
#define _FINGER_LIST_H
...
typedef struct _ip_node_info_t
{
    int vlan_id;
...
typedef struct _finger_sync_header_pkt_t
{
    unsigned char type;
...
} __attribute__ ((packed)) finger_sync_header_pkt_t;


typedef struct _finger_sync_pkt_t
{
    int length;
    unsigned char mac[6];
    uint64_t send_time;
...
} __attribute__ ((packed)) finger_sync_pkt_t;

typedef struct _ip_sync_pkt_t
{
    unsigned char mac[6];
    uint64_t send_time;
    uint64_t update_time;
    ip_node_info_t ipnodeinfo;
} __attribute__ ((packed)) ip_sync_pkt_t;

struct finger_cfg_list
{
    finger_sync_pkt_t data;

...
/*                           Parameter                                     */
/*-------------------------------------------------------------------------*/
int fingersync_debug_level = FINGER_ERROR;
int fingersync_debug_show_keys = 0;
int fingersync_debug_timestamp = 0;
/*-------------------------------------------------------------------------*/
/*                           Function                                      */
/*-------------------------------------------------------------------------*/
...
void fingersync_printf(int level, char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    if (level >= fingersync_debug_level) {
        fingersync_debug_print_timestamp();
        vprintf(fmt, ap);
    }
    va_end(ap);
}

static void _fingersync_hexdump(int level, const char *title, const u8 *buf,
        size_t len, int show)
{
    size_t i;
    if (level < fingersync_debug_level)
...
{
    _fingersync_hexdump(level, title, buf, len, 1);
}


void fingersync_hexdump_key(int level, const char *title, const u8 *buf, size_t len)
{
    _fingersync_hexdump(level, title, buf, len, fingersync_debug_show_keys);
}


static void _fingersync_hexdump_ascii(int level, const char *title, const u8 *buf,
        size_t len, int show)
{
    int i, llen;
    const u8 *pos = buf;
...
void fingersync_hexdump_ascii(int level, const char *title, const u8 *buf, size_t len)
{
    _fingersync_hexdump_ascii(level, title, buf, len, 1);
}
...
void fingersync_hexdump_ascii_key(int level, const char *title, const u8 *buf,
        size_t len)
{
    _fingersync_hexdump_ascii(level, title, buf, len, fingersync_debug_show_keys);
}

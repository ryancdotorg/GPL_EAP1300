...
int check_http_status(char *query_host, char *query_path)
{
    ...
    if (query_host == NULL || query_path == NULL){
        status = HTTP_DATA_ERR; 
        return status;
    }

	if ((info = getHostInfo(query_host, "http")) == NULL){
    ...
    while (1){
        ...
        if (nselect > 0){
            ...
            if (nread == 0){ // socket close
                if (recvbuf && strstr(recvbuf, "HTTP/1.1 301 Moved Permanently") != NULL)
                    status = STATUS_OK;
                else

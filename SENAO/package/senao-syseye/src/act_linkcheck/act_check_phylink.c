...
void *check_phylink(void *args)
{
    ...
    handle = seipc_create("act_check_phylink", "/tmp/syseye.unix");
...
int act_check_phylink_main(gdata_t *g, int path_key, void *args)
{
    ...
    snprintf(cfgpath, sizeof(cfgpath), "Actions[key=%d]/Connectivity[protocol=\"%s\"]/status", path_key, "phylink");
...
#if defined(USE_FORK)
    safe_fork_fn(check_phylink, act_args);
    if (act_args){
	if (act_args->cmd){
	    free(act_args->cmd);
	    act_args->cmd = NULL;
    ...
    if (pthread_create(&thread_id, &attr, check_phylink, act_args) != 0){
...
void *act_check_phylink(void *action_data)
{
    ...
    // TODO: handle delay
    act_check_phylink_main(gdata, t->path_key, t->args);
    return NULL;
}

...
int http_cgi_headers (server *srv, connection *con, http_cgi_opts *opts, http_cgi_header_append_cb cb, void *vdata) {
    ...
    data_string *im_app = (data_string *)array_get_element(con->request.headers, "X-IM-APP");

    char jwt_buf[256] = {0}, im_buf[64] = {0};
    int auth_len = 0, im_length = 0;

    if (NULL == im_app || buffer_is_empty(im_app->value))
    {
        /* go through */
    }
    else
    {
        im_length = buffer_string_length(im_app->value);

        if (im_length > 0)
        {
			snprintf(im_buf, sizeof(im_buf), "%s", im_app->value->ptr);
			//fprintf(stderr, "========im_buf:%s====\n", im_buf);
			rc |= cb(vdata, CONST_STR_LEN("X_IM_APP"), CONST_STR_LEN("true"));
        }
    }
    ...
    {
        auth_len = buffer_string_length(ds->value);

        if (auth_len > 7 && 0 == strncasecmp(ds->value->ptr, "Bearer ", sizeof("Bearer ")-1))
        {

            snprintf(jwt_buf, sizeof(jwt_buf), "%s", ds->value->ptr+sizeof("Bearer ")-1);
            jwt_buf[auth_len-(sizeof("Bearer ")-1)] ='\0';
        }
    }

    if (strlen(jwt_buf) > 0)
    {
        rc |= cb(vdata, CONST_STR_LEN("JWT_TOKEN"), CONST_STR_LEN(jwt_buf));
    }
    else
    {
        rc |= cb(vdata, CONST_STR_LEN("JWT_TOKEN"), CONST_STR_LEN(""));
    }

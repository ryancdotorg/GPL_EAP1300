...
int ping(char *mesh_iface, int argc, char **argv)
{
	...
	struct batadv_icmp_packet_sn_data icmp_packet_out, icmp_packet_in;
#else
...
#if SENAO_BATCTL_ICMP_COMMUNICATION
	int senao_ping_channel = -1;
#endif
#if 1 // senao patch: fixed ping result, packets count isn't equal to packets out
	int packet_count = 0;
#endif

#if SENAO_BATCTL_ICMP_COMMUNICATION
	while ((optchar = getopt(argc, argv, "hc:i:t:RTS:")) != -1) 
#else
...
#if SENAO_BATCTL_ICMP_COMMUNICATION
		case 'S':
			senao_ping_channel = strtol(optarg, NULL , 10);
			found_args += ((*((char*)(optarg - 1)) == optchar ) ? 1 : 2);
			break;
#endif
		default:
...
#else
	if (senao_ping_channel>=0 ) {
		packet_len = sizeof(struct batadv_icmp_packet_sn_data);
		icmp_packet_out.sn_cmd = senao_ping_channel;
		memset(&icmp_packet_out.sn_data, 0, SENAO_BATMAN_MAX_DATA_LENGTH);
	}
#endif
	else {
...
#else
			if (read_len == sizeof(struct batadv_icmp_packet_sn_data)) {
				printf(", sn_cmd=%d, sn_data=%s\n", icmp_packet_in.sn_cmd, icmp_packet_in.sn_data);
			}
#endif
			printf("\n");
...
sleep:
	...
	{
#if 1 // senao patch: fixed ping result, packets count isn't equal to packets out
		if (packets_out > packet_count) packets_out = packet_count;
#endif

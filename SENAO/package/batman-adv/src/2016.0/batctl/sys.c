...
#define SENAO_MAX_DATA_CHANNEL 	16
#define SENAO_MAX_DATA_LENGTH	SENAO_BATMAN_MAX_DATA_LENGTH
static void set_ping_reply_data_usage(void)
{
        fprintf(stderr, "Usage: batctl SN [channel] [data] \n");
        fprintf(stderr, "channel 0~%d:\n", (SENAO_MAX_DATA_CHANNEL-1));
        fprintf(stderr, " \t -c ping reply data\n");
}



int set_ping_reply_data(char *mesh_iface, int argc, char **argv)
{
        char *path_buff, dataStr[256];
        int channel=0;
	int res = EXIT_FAILURE;
        ...
        snprintf(path_buff, PATH_BUFF_LEN, SYS_BATIF_PATH_FMT, mesh_iface);
        if (argc == 1) {
                res = read_file(path_buff, SYS_PING_REPLY, USE_READ_BUFF, 0, 0, 0);
                goto out;
        }else if(argc == 3){
		channel = strtol(argv[1], (char **) NULL, 10);
		if(channel < 0 || channel >= SENAO_MAX_DATA_CHANNEL){
			set_ping_reply_data_usage;
			free(path_buff);
			return EXIT_FAILURE;
		}
                strcpy(dataStr, argv[2]);
		sprintf(dataStr, "%d	%s", channel, argv[2]);
		res = write_file(path_buff, SYS_PING_REPLY, dataStr, NULL);
        }else{
                set_ping_reply_data_usage();
		free(path_buff);
                return EXIT_FAILURE;

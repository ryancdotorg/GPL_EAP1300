...
#if SENAO_BATCTL_ICMP_COMMUNICATION
static ssize_t batadv_show_ping_reply(struct kobject *kobj, struct attribute *attr,
                                   char *buff)
{
	struct net_device *net_dev = batadv_kobj_to_netdev(kobj);
	...
	//batadv_info(net_dev,"show buff=%s\n",buff);
	strcpy(buff, "ping reply data:\n");
        for (channel_counter=0; channel_counter <= (SENAO_BATMAN_MAX_DATA_CHANNEL-1); channel_counter++ ){
	        bytes_written = sprintf(buff, "%s%d  %s\n",
                                buff, channel_counter, bat_priv->ping_reply[channel_counter].ping_reply_str);
	}
	batadv_info(net_dev,"%s\n",buff);

	return bytes_written;
}

static ssize_t batadv_store_ping_reply(struct kobject *kobj,
                                    struct attribute *attr, char *buff,
                                    size_t count)
{
        struct net_device *net_dev = batadv_kobj_to_netdev(kobj);
...
//	batadv_info(net_dev,"store buff=%s\n",buff);
	sscanf(buff, "%d	%s", &channel, dataStr);
	strcpy(bat_priv->ping_reply[channel].ping_reply_str, dataStr);
...
#if SENAO_BATCTL_ICMP_COMMUNICATION
static BATADV_ATTR(ping_reply_data, S_IRUGO | S_IWUSR, batadv_show_ping_reply,
                   batadv_store_ping_reply);
#endif

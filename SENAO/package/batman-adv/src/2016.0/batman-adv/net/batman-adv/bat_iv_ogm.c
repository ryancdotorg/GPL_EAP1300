				     ...
				     int max_if_num)
{
	...
	sn_spin_lock_bh(&orig_node->bat_iv.ogm_cnt_lock);
...
unlock:
	sn_spin_unlock_bh(&orig_node->bat_iv.ogm_cnt_lock);
					...
					int own_packet)
{
	...
	sn_spin_lock_bh(&bat_priv->forw_bat_list_lock);
	hlist_add_head(&forw_packet_aggr->list, &bat_priv->forw_bat_list);
	sn_spin_unlock_bh(&bat_priv->forw_bat_list_lock);
			  ...
			  enum batadv_dup_status dup_status)
{
	...
	hlist_for_each_entry_rcu(tmp_neigh_node,
		...
		sn_spin_lock_bh(&tmp_neigh_node->ifinfo_lock);
		batadv_ring_buffer_set(neigh_ifinfo->bat_iv.tq_recv,
				       &neigh_ifinfo->bat_iv.tq_index, 0);
		tq_avg = batadv_ring_buffer_avg(neigh_ifinfo->bat_iv.tq_recv);
		neigh_ifinfo->bat_iv.tq_avg = tq_avg;
		sn_spin_unlock_bh(&tmp_neigh_node->ifinfo_lock);
	...
	sn_spin_lock_bh(&neigh_node->ifinfo_lock);
	batadv_ring_buffer_set(neigh_ifinfo->bat_iv.tq_recv,
			       &neigh_ifinfo->bat_iv.tq_index,
			       batadv_ogm_packet->tq);
	tq_avg = batadv_ring_buffer_avg(neigh_ifinfo->bat_iv.tq_recv);
	neigh_ifinfo->bat_iv.tq_avg = tq_avg;
	sn_spin_unlock_bh(&neigh_node->ifinfo_lock);
	...
	if (router_ifinfo &&
		...
		sn_spin_lock_bh(&orig_node_tmp->bat_iv.ogm_cnt_lock);
		if_num = router->if_incoming->if_num;
		sum_orig = orig_node_tmp->bat_iv.bcast_own_sum[if_num];
		sn_spin_unlock_bh(&orig_node_tmp->bat_iv.ogm_cnt_lock);
				  ...
				  struct batadv_hard_iface *if_incoming)
{
	...
	if (is_my_orig) {
		...
		if (has_directlink_flag &&
			...
			sn_spin_lock_bh(&orig_neigh_node->bat_iv.ogm_cnt_lock);
			...
			*weight = bitmap_weight(word,
						BATADV_TQ_LOCAL_WINDOW_SIZE);
			sn_spin_unlock_bh(&orig_neigh_node->bat_iv.ogm_cnt_lock);
		}

...
{
	sn_spin_lock_bh(&hardif_neigh->if_incoming->neigh_list_lock);
	hlist_del_init_rcu(&hardif_neigh->list);
	sn_spin_unlock_bh(&hardif_neigh->if_incoming->neigh_list_lock);
			...
			struct batadv_hard_iface *if_outgoing)
{
	...
	sn_spin_lock_bh(&neigh->ifinfo_lock);
...
out:
	sn_spin_unlock_bh(&neigh->ifinfo_lock);
			   ...
			   const u8 *neigh_addr)
{
	...
	sn_spin_lock_bh(&hard_iface->neigh_list_lock);
...
out:
	sn_spin_unlock_bh(&hard_iface->neigh_list_lock);
	return hardif_neigh;
}

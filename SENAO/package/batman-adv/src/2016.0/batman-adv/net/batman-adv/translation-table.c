				      ...
				      unsigned short vid, int v)
{
	...
	if (atomic_add_return(v, &vlan->tt.num_entries) == 0) {
		sn_spin_lock_bh(&orig_node->vlan_list_lock);
		hlist_del_init_rcu(&vlan->list);
		sn_spin_unlock_bh(&orig_node->vlan_list_lock);
				  ...
				  u8 event_flags)
{
	...
	sn_spin_lock_bh(&bat_priv->tt.changes_list_lock);
...
unlock:
	sn_spin_unlock_bh(&bat_priv->tt.changes_list_lock);
			 ...
			 unsigned short vid, int ifindex, u32 mark)
{
	...
	vlan = batadv_softif_vlan_get(bat_priv, vid);
	if (!vlan) {
        /*2017.2.21 Senao George, https://patchwork.kernel.org/patch/9278853/ */
		net_ratelimited_function(batadv_info, soft_iface,
...
static void batadv_tt_tvlv_container_update(struct batadv_priv *bat_priv)
{
	...
	sn_spin_lock_bh(&bat_priv->tt.last_changeset_lock);
	...
	sn_spin_unlock_bh(&bat_priv->tt.last_changeset_lock);
				...
				struct batadv_orig_node *orig_node, int ttvn)
{
	...
	sn_spin_lock_bh(&tt_global->list_lock);
	hlist_add_head_rcu(&orig_entry->list,
			   &tt_global->orig_list);
	sn_spin_unlock_bh(&tt_global->list_lock);
...
batadv_tt_global_del_orig_list(struct batadv_tt_global_entry *tt_global_entry)
{
	...
	sn_spin_lock_bh(&tt_global_entry->list_lock);
	head = &tt_global_entry->orig_list;
	hlist_for_each_entry_safe(orig_entry, safe, head, list)
		_batadv_tt_global_del_orig_entry(tt_global_entry, orig_entry);
	sn_spin_unlock_bh(&tt_global_entry->list_lock);
}
...
static void batadv_tt_req_list_free(struct batadv_priv *bat_priv)
{
	...
	sn_spin_lock_bh(&bat_priv->tt.req_list_lock);
	...
	sn_spin_unlock_bh(&bat_priv->tt.req_list_lock);
}
				       ...
				       u16 tt_buff_len)
{
	 ...
	 */
	sn_spin_lock_bh(&orig_node->tt_buff_lock);
	if (tt_buff_len > 0) {
	...
	sn_spin_unlock_bh(&orig_node->tt_buff_lock);
}
					  ...
					  u8 *req_src, u8 *req_dst)
{
	 ...
	 */
	if (!full_table) {
		sn_spin_lock_bh(&req_dst_orig_node->tt_buff_lock);
		...
		sn_spin_unlock_bh(&req_dst_orig_node->tt_buff_lock);
	} else {
				      ...
				      u8 *resp_src, u16 num_entries)
{
	...
	sn_spin_lock_bh(&orig_node->tt_lock);
	...
	sn_spin_unlock_bh(&orig_node->tt_lock);
...
static void batadv_tt_roam_list_free(struct batadv_priv *bat_priv)
{
	...
	sn_spin_lock_bh(&bat_priv->tt.roam_list_lock);
	...
	sn_spin_unlock_bh(&bat_priv->tt.roam_list_lock);
}
...
void batadv_tt_local_resize_to_mtu(struct net_device *soft_iface)
{
	...
	sn_spin_lock_bh(&bat_priv->tt.commit_lock);
	...
	sn_spin_unlock_bh(&bat_priv->tt.commit_lock);
}

					     ...
					     u8 mcast_flags)
{
	...
	if (mcast_flags & BATADV_MCAST_WANT_ALL_UNSNOOPABLES &&
		...
		sn_spin_lock_bh(&bat_priv->mcast.want_lists_lock);
		/* flag checks above + mcast_handler_lock prevents this */
		WARN_ON(!hlist_unhashed(node));

		hlist_add_head_rcu(node, head);
		sn_spin_unlock_bh(&bat_priv->mcast.want_lists_lock);
					     ...
					     u16 tvlv_value_len)
{
	...
	sn_spin_lock_bh(&orig->mcast_handler_lock);
	...
	sn_spin_unlock_bh(&orig->mcast_handler_lock);
}

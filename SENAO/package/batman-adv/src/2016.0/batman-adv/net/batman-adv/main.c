				      ...
				      u8 type, u8 version)
{
	...
	sn_spin_lock_bh(&bat_priv->tvlv.container_list_lock);
	tvlv = batadv_tvlv_container_get(bat_priv, type, version);
	batadv_tvlv_container_remove(bat_priv, tvlv);
	sn_spin_unlock_bh(&bat_priv->tvlv.container_list_lock);
}
				  ...
				  u8 type, u8 version, u8 flags)
{
	...
	sn_spin_lock_bh(&bat_priv->tvlv.handler_list_lock);
	hlist_add_head_rcu(&tvlv_handler->list, &bat_priv->tvlv.handler_list);
	sn_spin_unlock_bh(&bat_priv->tvlv.handler_list_lock);
}

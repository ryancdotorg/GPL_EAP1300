			     ...
			     const char *fmt, ...)
{
	...
	sn_spin_lock_bh(&debug_log->lock);
	va_start(args, fmt);
	...
	sn_spin_unlock_bh(&debug_log->lock);
...
static int batadv_bla_sn_loopdetect_open(struct inode *inode, struct file *file)
{
	struct net_device *net_dev = (struct net_device *)inode->i_private;

	return single_open(file, batadv_bla_sn_loopdetect_seq_print_text, net_dev);
}
#endif
...
#if 1 // senao patch: add loop detect notice
static BATADV_DEBUGINFO(bla_sn_loopdetect, S_IRUGO, batadv_bla_sn_loopdetect_open);
#endif
#endif
#ifdef CONFIG_BATMAN_ADV_DAT
...
#endif
#if 1 // senao patch: add loop detect notice
	&batadv_debuginfo_bla_sn_loopdetect,
#endif
	NULL,
};

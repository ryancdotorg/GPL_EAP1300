...
batadv_bla_del_backbone_claims(struct batadv_bla_backbone_gw *backbone_gw)
{
	...
	sn_spin_lock_bh(&backbone_gw->crc_lock);
	backbone_gw->crc = BATADV_BLA_CRC_INIT;
	sn_spin_unlock_bh(&backbone_gw->crc_lock);
}
...
#if 1 // senao patch: add loop detect notice
	senao_loop_detector = 1;
	printk(KERN_ALERT "Possible loop detected which can't be handled by BLA - please check your network setup!\n");
#else
				 ...
				 struct batadv_bla_backbone_gw *backbone_gw)
{
	...
	if (!claim) {
		...
		sn_spin_lock_bh(&claim->backbone_gw->crc_lock);
		claim->backbone_gw->crc ^= crc16(0, claim->addr, ETH_ALEN);
		sn_spin_unlock_bh(&claim->backbone_gw->crc_lock);
		batadv_backbone_gw_free_ref(claim->backbone_gw);
	}
	/* set (new) backbone gw */
				   ...
				   struct sk_buff *skb)
{
	...
	sn_spin_lock_bh(&bat_priv->bla.bcast_duplist_lock);
...
out:
	sn_spin_unlock_bh(&bat_priv->bla.bcast_duplist_lock);
		  ...
		  unsigned short vid, bool is_bcast)
{
	...
	if (!claim) {
		 ...
		 */
#if 1 // patch: looping unicast packets when using BLA
...
#if 1 // senao patch: add loop detect notice
/**
 * batadv_bla_sn_loopdetect_seq_print_text - print the loop detect information
 * @seq: seq file to print on
 * @offset: not used
 *
 * Return: always 0

...
/* SENAO Define */
#ifdef SENAO_SPIN_LOCK_BH_DELAY
#define sn_spin_lock_bh(x)			spin_lock_irq(x)
#define sn_spin_unlock_bh(x)		spin_unlock_irq(x)
#else
#define sn_spin_lock_bh(x)			spin_lock_bh(x)
#define sn_spin_unlock_bh(x)		spin_unlock_bh(x)
#endif
 ...
 */
#if 1 // senao patch: modify purge timeout
#define BATADV_PURGE_TIMEOUT 40000 /* 40 seconds */
#else
...
#if 1 // senao patch: modify dat timeout
#define BATADV_DAT_ENTRY_TIMEOUT (1 * 60000) /* 1 mins in milliseconds */
#else

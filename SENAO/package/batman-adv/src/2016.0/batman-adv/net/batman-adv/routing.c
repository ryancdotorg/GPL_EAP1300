				 ...
				 struct batadv_neigh_node *neigh_node)
{
	...
	sn_spin_lock_bh(&orig_node->neigh_list_lock);
	rcu_assign_pointer(orig_ifinfo->router, neigh_node);
	sn_spin_unlock_bh(&orig_node->neigh_list_lock);
			    ...
			    struct batadv_hard_iface *recv_if)
{
	...
	struct batadv_icmp_packet_sn_data *icmp_packet_sn_data;
#else
	...
	if ((icmph->msg_type == BATADV_ECHO_REPLY ||
...
#if SENAO_BATCTL_ICMP_COMMUNICATION
	    (skb->len >= sizeof(struct batadv_icmp_packet_sn_data))) 
#else
		...
		icmph = (struct batadv_icmp_header *)skb->data;
#if SENAO_BATCTL_ICMP_COMMUNICATION
		icmp_packet_sn_data = (struct batadv_icmp_packet_sn_data *)icmph;
#else
...
#if SENAO_BATCTL_ICMP_COMMUNICATION
		int channel=0;
		channel=icmp_packet_sn_data->sn_cmd;
		if(icmph->msg_type == BATADV_ECHO_REQUEST)
			strcpy(icmp_packet_sn_data->sn_data, bat_priv->ping_reply[channel].ping_reply_str);
#endif
...
#if 1 // patch: drop unicast packets from other backbone gw
	...
	if (batadv_is_my_mac(bat_priv, unicast_packet->dest)) {
		...
		if (orig_node_gw) {
			is_gw = batadv_bla_is_backbone_gw(skb, orig_node_gw,
							  hdr_size);
			batadv_orig_node_free_ref(orig_node_gw);
			if (is_gw) {
			     ...
			     struct batadv_hard_iface *recv_if)
{
	...
	sn_spin_lock_bh(&orig_node->bcast_seqno_lock);
	...
	sn_spin_unlock_bh(&orig_node->bcast_seqno_lock);

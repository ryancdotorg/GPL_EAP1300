					   ...
					   struct sk_buff *skb)
{
	...
	if (dat_entry) {
		 ...
		 * destination.
		 */
		if (!batadv_bla_check_claim(bat_priv, dat_entry->mac_addr, vid)) {
			batadv_dbg(BATADV_DBG_DAT, bat_priv,
					   ...
					   struct sk_buff *skb, int hdr_size)
{
	...
	struct sk_buff *skb_new;
	struct batadv_dat_entry *dat_entry = NULL;
#if 1 // patch: prevent duplication of ARP replies in BLA
	struct batadv_unicast_4addr_packet *unicast_4addr_packet;
	...
	if (!dat_entry) {
		...
		* it as this leads to multiplication of arp requests in bla setups
		* as long as there is no dat_entry fo this answer. In this
		* case better drop the DHT_GET. Normal bla code doesn't take care of
		* these packets as they are tunneled via unicast.
		*/
		unicast_4addr_packet = (struct batadv_unicast_4addr_packet*)skb->data;
		orig_node = batadv_orig_hash_find(bat_priv, unicast_4addr_packet->src);
		if (orig_node) {
			if ((unicast_4addr_packet->u.packet_type == BATADV_UNICAST_4ADDR) &&
				...
				(batadv_bla_is_backbone_gw(skb, orig_node, hdr_size))) {
					batadv_dbg(BATADV_DBG_DAT, bat_priv, "Doubled ARP request removed: "
							   ...
							   hw_src, &ip_src, batadv_arp_hw_dst(skb, hdr_size), &ip_dst,
							   unicast_4addr_packet->src);
					ret = true;
			}

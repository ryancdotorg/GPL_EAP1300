...
#ifdef IFF_802_1Q_VLAN_PASS // senao patch: add vlan-passthrough
	if (skb->dev->priv_flags & IFF_802_1Q_VLAN_PASS)
...
void batadv_softif_vlan_free_ref(struct batadv_softif_vlan *vlan)
{
	...
	if (atomic_dec_and_test(&vlan->refcount)) {
		sn_spin_lock_bh(&vlan->bat_priv->softif_vlan_list_lock);
		hlist_del_rcu(&vlan->list);
		sn_spin_unlock_bh(&vlan->bat_priv->softif_vlan_list_lock);
...
int batadv_softif_create_vlan(struct batadv_priv *bat_priv, unsigned short vid)
{
	...
	sn_spin_lock_bh(&bat_priv->softif_vlan_list_lock);
	hlist_add_head_rcu(&vlan->list, &bat_priv->softif_vlan_list);
	sn_spin_unlock_bh(&bat_priv->softif_vlan_list_lock);

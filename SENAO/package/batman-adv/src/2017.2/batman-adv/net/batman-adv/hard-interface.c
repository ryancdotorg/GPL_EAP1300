...
hardif_put:
	...
	// senao WAR fix. 
	// unregister_netdevice: waiting for ath35 to become free. Usage count = 1
	// if hard_iface->refcount != 0, we dev_put again
	if(event == NETDEV_UNREGISTER && *(int*)&(hard_iface->refcount) >= 1)
	{
		batadv_hardif_put(hard_iface);
	}
out:

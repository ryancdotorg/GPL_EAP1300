...
CWBool CWConfigFileDestroyLib()
{
    ...
    {
        CW_CREATE_ARRAY_ERR(gDefaultACAddresses, gConfigValues[0].count, char*, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););

        for(i = 0; i < gConfigValues[0].count; i++)
        {
            CW_CREATE_STRING_FROM_STRING_ERR(gDefaultACAddresses[i], (gConfigValues[0].value.str_array_value)[i], return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
        }

        gDefaultACAddressCount = gConfigValues[0].count;
    }
#ifdef CW_DEBUGGING
    if(gEnabledDebugLog)
    {

...
/*Added by larger*/
char *sock_ntop_r_without_port(const struct sockaddr *sa, char *str)
{
    switch(sa->sa_family)
    {
        case AF_INET:
...
unsigned short sock_get_port_cw(struct sockaddr *sa)
{
    switch(sa->sa_family)
    {
        case AF_INET:
...
struct ifi_info *get_ifi_info(int family, int doaliases)
{
    ...
    for(ptr = buf; ptr < buf + ifc.ifc_len;)
    {
        ...
        /*begin,20150707,Andy Hu,modified for 64 bit platform*/
        ...
        ifi = (struct ifi_info *)CWMemZeroAlloc(sizeof(struct ifi_info));
        *ifipnext = ifi;			/* prev points to this new one */
        ...
        switch(ifr->ifr_addr.sa_family)
        {
            case AF_INET:
                ...
                ifi->ifi_addr = (struct sockaddr *)CWMemZeroAlloc(sizeof(struct sockaddr_in));
                memcpy(ifi->ifi_addr, sinptr, sizeof(struct sockaddr_in));

                /* SENAO Jonse 2013/5/22: subnet mask */
                ioctl(sockfd, SIOCGIFNETMASK, &ifrcopy);
                sinptr = (struct sockaddr_in *) &ifrcopy.ifr_addr;
                ifi->ifi_netmask = (struct sockaddr *)CWMemZeroAlloc(sizeof(struct sockaddr_in));
                ...
                if(flags & IFF_BROADCAST)
                {
                    ...
                    ifi->ifi_brdaddr = (struct sockaddr *)CWMemZeroAlloc(sizeof(struct sockaddr_in));
            ...
            case AF_INET6:
                ...
                ifi->ifi_addr = (struct sockaddr *)CWMemZeroAlloc(sizeof(struct sockaddr_in6));
...
void free_ifi_info(struct ifi_info *ifihead)
{
    ...
    for(ifi = ifihead; ifi != NULL; ifi = ifinext)
    {
        ...
        {
            CWMemFree(ifi->ifi_addr);
        }
        if(ifi->ifi_netmask != NULL)
        {
            CWMemFree(ifi->ifi_netmask);
        }
        if(ifi->ifi_brdaddr != NULL)
        {
            CWMemFree(ifi->ifi_brdaddr);
        }
        ifinext = ifi->ifi_next;	/* can't fetch ifi_next after free() */
        CWMemFree(ifi);					/* the ifi_info{} itself */
    }
}
/* end free_ifi_info */

int get_ifi_addr(const char *ifi_name, struct sockaddr *sa, struct sockaddr *sam, int family)
{
    int sockfd;
    struct ifreq ifr;
    ...
    strncpy(ifr.ifr_name, ifi_name, IFNAMSIZ - 1);
    ...
    memcpy(sa, &ifr.ifr_addr, family == AF_INET6 ?
    ...
    memcpy(sam, &ifr.ifr_addr, family == AF_INET6 ?
    	sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in));

...
CWBool CWCreateThread(CWThread *newThread, CW_THREAD_FUNCTION threadFunc, void *arg)
{
    ...
    {
        return CWErrorRaise(CW_ERROR_GENERAL, "Can't init thread attribute");
    }

    if(pthread_attr_setstacksize(&attr, CW_THREAD_STACK_SIZE) != 0)
    {
        return CWErrorRaise(CW_ERROR_GENERAL, "Can't set thread stacksize");
    }
...
CWBool CWCreateDetachedThread(CWThread *newThread, CW_THREAD_FUNCTION threadFunc, void *arg)
{
    pthread_attr_t attr;
    ...
    {
        return CWErrorRaise(CW_ERROR_GENERAL, "Can't set thread detachstate");
    }
...
CWBool CWWaitThreadCondition(CWThreadCondition *theCondition, CWThreadMutex *theMutex)
{
    ...
    switch((ret = pthread_cond_wait(theCondition, theMutex)))
    {
        case 0: // success
            break;
        default:
            CWLog("Error waiting on thread condition ret=%d", ret);
...
CWBool CWThreadMutexLock(CWThreadMutex *theMutex)
{
    ...
    if((ret = pthread_mutex_lock(theMutex)) != 0)
    {
        CWLog("pthread_mutex_lock failed ret = %d", ret);
...
void CWThreadMutexUnlock(CWThreadMutex *theMutex)
{
    ...
    if((ret = pthread_mutex_unlock(theMutex)) != 0)
    {
        CWLog("pthread_mutex_unlock failed ret = %d", ret);
    }
    /*
...
CWBool CWTimerRequest(int sec, CWThread *threadPtr, CWTimerID *idPtr, int signalToRaise)
{
    ...
    {
        return CWErrorRaise(CW_ERROR_GENERAL, "timer_add failed");
    }

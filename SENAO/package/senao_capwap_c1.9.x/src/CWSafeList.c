...
CWBool CWWaitElementFromSafeListTimeout(CWSafeList safeList, int milliSec)
{
    CWPrivateSafeList *pList = (CWPrivateSafeList *)safeList;
    ...
    timeout.tv_sec = now.tv_sec + (milliSec / 1000); /* seconds */
    timeout.tv_nsec = (now.tv_usec * 1000) + ((milliSec % 1000) * 1000000); /* nanoseconds */

    return CWWaitThreadConditionTimeout(pList->pThreadCond, pList->pThreadMutex, &timeout);
}
...
unsigned int CWGetCountElementFromSafeList(CWSafeList safeList)
{
    CWPrivateSafeList *pList = (CWPrivateSafeList *)safeList;

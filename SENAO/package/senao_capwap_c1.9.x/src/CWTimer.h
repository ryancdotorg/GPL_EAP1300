...
CWBool CWTimerCreate(int sec, CWTimerID *idPtr, void (*hdl)(CWTimerArg, CWTimerID), CWTimerArg arg)
{
    int2 ev;
...
CWTimerArg CWTimerGetArg(CWTimerID *idPtr)
{
    CWTimerArg pResult;
...
CWBool CWTimerSetArg(CWTimerID *idPtr, CWTimerArg arg)
{
    if(idPtr == NULL)
    {
        CWErrorRaise(CW_ERROR_WRONG_ARG, NULL);
...
CWBool CWTimerDestroy(CWTimerID *idPtr)
{
    if(idPtr == NULL)
    {
        return CWErrorRaise(CW_ERROR_WRONG_ARG, NULL);
...
void _clkini()
{
    ...
    */
    if((new_stack.ss_sp = CWMemAlloc(SIGSTKSZ)) == NULL)
    {

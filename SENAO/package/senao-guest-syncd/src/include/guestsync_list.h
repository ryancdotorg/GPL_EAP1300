...
typedef struct _node_info_t
{
    ...
    unsigned int gonetime;
    char sessionid[32];
    uint64_t maxinputoctets; // reserved
    uint64_t maxoutputoctets; // reserved
    uint64_t maxtotaloctets; // reserved
    uint64_t bandwidthmaxup;   // reserved
    uint64_t bandwidthmaxdown; // reserved
    unsigned int interiminterval; //acct-interm
...
typedef struct _guest_sync_header_pkt_t
{
    unsigned char magic[4];
    unsigned char version[2];
    unsigned char salt[8]; // for encryption
    unsigned char reserved[8];
} __attribute__ ((packed)) guest_sync_header_pkt_t;


typedef struct _guest_sync_pkt_t
{
    unsigned char version[2];
    unsigned char type;
    int length;
    ...
    unsigned char mac[6];
    char dhcpif[16];
    unsigned char random[16]; // for identify, and salt use
    unsigned int expiretime; // + 30 sec
    char ssid[32];
    char group[32];
    node_info_t nodeinfo;
} __attribute__ ((packed)) guest_sync_pkt_t;

struct guest_cfg_list
{
    guest_sync_pkt_t data;

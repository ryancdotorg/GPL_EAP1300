        ...
        void *eloop_data, void *user_data)
{
    ...
    tmp = (struct eloop_sock *)
        realloc(eloop.readers, (eloop.reader_count + 1) * sizeof(struct eloop_sock));
    if(tmp == NULL)
        return -1;
        ...
        void *user_data)
{
    ...
    tmp = (struct eloop_signal *)
        realloc(eloop.signals, (eloop.signal_count + 1) *sizeof(struct eloop_signal));
    if(tmp == NULL)
        return -1;
...
void eloop_run(void)
{
    ...
    while(!eloop.terminate && (eloop.timeout || eloop.reader_count > 0))
    {
        ...
        res = select(eloop.max_sock + 1, rfds, NULL, NULL, eloop.timeout ? &tv : NULL);

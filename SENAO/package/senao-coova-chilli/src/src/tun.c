...
int tun_discover(struct tun_t *this) {
  ...
  for (i=0; i<len; ++i) {
    ...
    {
        if (!strcmp(ifr->ifr_name, _options.wanif)) {
            log_dbg("skipping dhcpif %s", _options.wanif);
            continue;
        }
    }
    else
...
#error  "Unknown platform!"
  ...
  ifr.ifr_flags = ifr.ifr_flags
      ...
      | IFF_BROADCAST
#endif
     //SENAO: kernel 4.4 will cause use huge memory when each tun up
#if 0//def IFF_PROMISC
      | IFF_PROMISC
#endif
#ifdef IFF_ONE_QUEUE
      | IFF_ONE_QUEUE
  ...
  {
      int mtu;
      mtu = net_get_mtu(_options.wanif);
      log_dbg("[BridgeMode] %s: auto sync mtu %d from %s", netif->devname, mtu, _options.wanif);
      if(mtu > 0)
      {
          net_set_mtu(netif, mtu);
      }
  }
  else
  {
...
int tun_runscript(struct tun_t *tun, char* script, int wait) {
  ...
  set_env("CONFIG_NAME", VAL_STRING, _options.configname ? _options.configname : "", 0);

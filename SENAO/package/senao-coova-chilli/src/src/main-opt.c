...
int main(int argc, char **argv) {
  ...
  _options.defgonetimeout = args_info.defgonetimeout_arg;
  _options.nasport = args_info.nasport_arg;
  _options.radiusnasporttype = args_info.radiusnasporttype_arg;
  _options.radiusauthport = args_info.radiusauthport_arg;
  _options.radiusauthport2 = args_info.radiusauthport2_arg;
  _options.radiusacctport = args_info.radiusacctport_arg;
  _options.radiusacctport2 = args_info.radiusacctport2_arg;
  ...
  _options.wanif = STRDUP(args_info.wanif_arg);
  _options.configname = STRDUP(args_info.configname_arg);
  ...
  if (!args_info.radiussecret2_arg) {
    log_err(0, "radiussecret2 must be specified!");
    if (!args_info.forgiving_flag)
      goto end_processing;
  }

  if (!args_info.radiusacctsecret_arg) {
    log_err(0, "radiusacctsecret must be specified!");
    if (!args_info.forgiving_flag)
      goto end_processing;
  }

  if (!args_info.radiusacctsecret2_arg) {
    log_err(0, "radiusacctsecret2 must be specified!");
    if (!args_info.forgiving_flag)
      goto end_processing;
  }
  ...
  _options.oauth = args_info.oauth_flag;
  _options.natipbymac = args_info.natipbymac_flag;
  _options.sslkeypasshc = args_info.sslkeypasshc_flag;

  if (args_info.ezmasterip_arg &&
          !inet_aton(args_info.ezmasterip_arg, &_options.ezmasterip)) {
      log_err(0, "Invalid ezmasterip IP address: %s!", args_info.ezmasterip_arg);
      if (!args_info.forgiving_flag)
          goto end_processing;
  }
 ...
 for (numargs = 0; numargs < MAX_UAM_BLOCK_DOMAINS; ++numargs) {
    if (_options.uamblockdomains[numargs])
      free(_options.uamblockdomains[numargs]);
    _options.uamblockdomains[numargs] = 0;
  }

  if (args_info.uamblockdomain_given) {
    for (numargs = 0, i=0;
	 numargs < args_info.uamblockdomain_given && i < MAX_UAM_BLOCK_DOMAINS;
	 ++numargs) {
      char *tb = args_info.uamblockdomain_arg[numargs];
      char *tok, *str, *ptr;
      for (str = tb ; i < MAX_UAM_BLOCK_DOMAINS; str = NULL) {
	tok = strtok_r(str, ",", &ptr);
	if (!tok) break;
	log_dbg("uamblockdomain %s", tok);
	_options.uamblockdomains[i++] = STRDUP(tok);
      }
    }
  }
  ...
  /* If no option is specified terminate                          */
  /* Do hostname lookup to translate hostname to IP address       */
  if (args_info.radiusacctserver1_arg) {
    if (!(host = gethostbyname(args_info.radiusacctserver1_arg))) {
      log_err(0, "Invalid radiusacctserver1 address: %s! [%s]",
	      args_info.radiusacctserver1_arg, strerror(errno));
      if (!args_info.forgiving_flag)
	goto end_processing;
    }
    else {
      memcpy(&_options.radiusacctserver1.s_addr, host->h_addr, host->h_length);
    }
  }
  else {
    log_err(0,"No radiusacctserver1 address given!");
    if (!args_info.forgiving_flag)
      goto end_processing;
  }
  ...
  /* If no option is specified terminate                          */
  /* Do hostname lookup to translate hostname to IP address       */
  if (args_info.radiusacctserver2_arg) {
    if (!(host = gethostbyname(args_info.radiusacctserver2_arg))) {
      log_err(0, "Invalid radiusacctserver2 address: %s! [%s]",
	      args_info.radiusacctserver2_arg, strerror(errno));
      if (!args_info.forgiving_flag)
	goto end_processing;
    }
    else {
      memcpy(&_options.radiusacctserver2.s_addr, host->h_addr, host->h_length);
    }
  }
  else {
    _options.radiusacctserver2.s_addr = 0;
  }
...
#if 1
  if (args_info.macallowedfile_given)
  {
      FILE *fp = NULL;
      char *line = NULL;
      size_t len = 0;
      ...
      fp = fopen(args_info.macallowedfile_arg, "r");
      if (fp==NULL)
      {
          log_dbg("[MacallowedFile] %s open error", args_info.macallowedfile_arg);
      }
      else
      {
          /* read mac list */
          while ((read = getline(&line, &len, fp)) != -1) {
              ...
              else
              {
                  ...
                  log_dbg("[MacallowedFile]" MAC_FMT2, MAC_ARG(_options.macok[_options.macoklen]));
                  _options.macoklen++;
              }
          }
  ...
  if (!args_info.proxysecret_arg) {
    ...
    // TODO: not support radiussecret2_arg
  }
  else {
  ...
  _options.macallowedfile = STRDUP(args_info.macallowedfile_arg);
  ...
  _options.networkid = STRDUP(args_info.networkid_arg);
  _options.ssidid = STRDUP(args_info.ssidid_arg);
  _options.ssidprofileid = STRDUP(args_info.ssidprofileid_arg);
  _options.logintype = STRDUP(args_info.logintype_arg);
  _options.modelname = STRDUP(args_info.modelname_arg);
  _options.fwversion = STRDUP(args_info.fwversion_arg);
  _options.apmac = STRDUP(args_info.apmac_arg);
  _options.radiuscalledstaid = STRDUP(args_info.radiuscalledstaid_arg);
  ...
  _options.radiussecret2 = STRDUP(args_info.radiussecret2_arg);
  _options.radiusacctsecret = STRDUP(args_info.radiusacctsecret_arg);
  _options.radiusacctsecret2 = STRDUP(args_info.radiusacctsecret2_arg);
#ifdef ENABLE_LARGELIMITS

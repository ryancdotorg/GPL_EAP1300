  ...
  if (len < 0) {
    switch (errno) {
      ...
      case EMSGSIZE:
        if (dlen > netif->mtu)
        {
            log_dbg("%s: dlen=[%d] > netif-mtu=[%d]", netif->devname, dlen, netif->mtu);
            if (!_options.bridgemode)
            {
                log_dbg("%s: set mtu %d", netif->devname, _options.mtu);
                net_set_mtu(netif, dlen);
            }
            else
            {
                log_dbg("[BridgeMode] %s: skip net set mtu %d", netif->devname, _options.mtu);
            }
        }
        break;
#endif
...
int net_get_mtu(char *ifname) {
  ...
  if (ioctl(fd, SIOCGIFMTU, &ifr) < 0) {
    log_err(errno, "%d could not get MTU on dev=%s",
           errno, ifname);
    close(fd);
    return -1;
  }
...
int net_set_mtu(net_interface *netif, size_t mtu) {
  ...
  if (ioctl(fd, SIOCSIFMTU, &ifr) < 0) {
    log_err(errno, "%d could not set MTU of %zu on dev=%s",
...
int net_route(struct in_addr *dst, struct in_addr *gateway,
	      struct in_addr *mask, const char* ifname, int delete) {
  ...
  memset (&r, 0, sizeof (r));
  r.rt_flags = RTF_UP; /* RTF_HOST not set */
  ...
  if(ifname) r.rt_dev = (void*) ifname;
...
int net_open_eth(net_interface *netif) {
  ...
  if (!_options.bridgemode)
  {
      log_dbg("%s: net_open_eth set mtu %d", netif->devname, _options.mtu);
      net_set_mtu(netif, _options.mtu);
  }
  else
  {
      log_dbg("[BridgeMode] %s: skip tuntap set mtu %d", netif->devname, _options.mtu);
  }

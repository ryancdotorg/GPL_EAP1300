...
struct options_t {
  ...
  uint16_t radiusauthport2;       /* Authentication UDP port */
  uint16_t radiusacctport;       /* Accounting UDP port */
  uint16_t radiusacctport2;       /* Accounting UDP port */
...
#define MAX_MOREIF (MAX_RAWIF-1)
  ...
  struct in_addr ezmasterip;        /* ezMaster ip */
  // senao config
  char *logintype;
  char *networkid;
  char *ssidid;
  ...
  char *fwversion;
  char *apmac;
  char *radiuscalledstaid;
  char *wanif;                      /* Interface: br-lan */
  uint8_t oauth:1;
  char* uamblockdomains[MAX_UAM_BLOCK_DOMAINS];
  uint8_t natipbymac:1;
  int nasport;
  uint8_t sslkeypasshc:1;
  char *configname;
...
#define EXTADMVSA_ATTR_CNT 4
  ...
  /* Random MAC Authentication list, for l2_block use */
  uint8_t dynmacok[DYN_MACOK_MAX][PKT_ETH_ALEN]; /* Allowed Dynamic MACs */
  int dynmacoklen;                   /* Number of Dynamic MAC addresses */

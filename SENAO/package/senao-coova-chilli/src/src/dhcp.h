...
struct dhcp_t {
  ...
  int arpfd;            /* ARP monitor socket for bridge mode */
...
//yolin add
int dhcp_authconn(struct dhcp_t *this, struct dhcp_conn_t **conn, uint8_t *hwaddr);
//
int dhcp_update_ip(struct dhcp_t *this, struct dhcp_conn_t **conn, struct in_addr *new_addr);

...
#define QUERY_TIMEOUT_CHECK_FILE "/var/run/chilli/query_timeout"
...
static cmd_info commands[] = {
  ...
  { CMDSOCK_LIST_MACALLOW, "listmacallow",  NULL },
  { CMDSOCK_RELOAD,        "reload",        NULL },
  { CMDSOCK_DHCP_LIST,     "dhcp-list",     NULL },
  { CMDSOCK_DHCP_RELEASE,  "dhcp-release",  NULL },
  { CMDSOCK_DHCP_NEW,      "dhcp-new",      NULL },
  { CMDSOCK_DHCP_CONNECT,  "dhcp-connect",  NULL },
  { CMDSOCK_DHCP_UPDATE,   "dhcp-update",   NULL },
  { CMDSOCK_DHCP_DISCONNECT,"dhcp-disconnect",   NULL },
...
#endif
  { CMDSOCK_ADD_MACALLOW,  "addmacallow", NULL },
  { CMDSOCK_REM_MACALLOW,  "remmacallow", NULL },
...
static struct cmd_arguments args[] = {
  ...
  { "sessiontime",
    CMDSOCK_FIELD_INTEGER,
    sizeof(request.d.sess.state.start_time),
    &request.d.sess.state.start_time,
    "Session time (in seconds)", 0, 0 },
  { "idletime",
    CMDSOCK_FIELD_INTEGER,
    sizeof(request.d.sess.state.last_sent_time),
    &request.d.sess.state.last_sent_time,
    "Idle time (in seconds)", 0, 0 },
  { "gonetime",
    CMDSOCK_FIELD_INTEGER,
    sizeof(request.d.sess.state.gone_time),
    &request.d.sess.state.gone_time,
    "Gone time (in seconds)", 0, 0 },
  { "gonetimeout",
    CMDSOCK_FIELD_INTEGER,
    sizeof(request.d.sess.params.gonetimeout),
    &request.d.sess.params.gonetimeout,
    "Gone time out(in seconds)", 0, 0 },
#endif
  { "sessiontimeout",
    CMDSOCK_FIELD_INTEGER,
...
static void timeout_alarm(int signum) {
  ...
  // Yolin: hope never happened... , if chilli_query timeout over 3 times, reload portal
  system("date >> "QUERY_TIMEOUT_CHECK_FILE);
  system("[ `cat "QUERY_TIMEOUT_CHECK_FILE" |wc -l` -gt 3 ] && luci-reload auto portal &");
  exit(1);
}
...
int main(int argc, char **argv) {
    ...
    if (!strcmp(cmd, commands[s].command)) {
      ...
      switch(request.type) {
        ...
        case CMDSOCK_DHCP_RELEASE:
          if (request.type == CMDSOCK_DHCP_RELEASE)
          {
              request.d.sess.params.flags |= NO_SCRIPT;
          }
        ...
        case CMDSOCK_DHCP_DISCONNECT:
          {
            ...
            if(argv[argidx+1]) // ip addreas
            {
              if (!inet_aton(argv[argidx+1], &request.ip)) {
                fprintf(stderr, "Invalid IP Address: %s\n", argv[argidx+1]);
                return usage(argv[0]);
              }
            }
  ...
  if(access(QUERY_TIMEOUT_CHECK_FILE, F_OK) != -1)
  {
    unlink(QUERY_TIMEOUT_CHECK_FILE);
  }

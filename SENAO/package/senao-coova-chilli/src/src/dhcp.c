...
#if 0
static int get_apmac(int fd, u_int8_t *mac_addr, int len) {
    if (len < PKT_ETH_ALEN)
    {
        log_dbg("get_apmac len error!");
        return 1;
    }

    if(_options.apmac)
    {
        int mac_str[PKT_ETH_ALEN] = {0};
        sscanf(_options.apmac, "%x-%x-%x-%x-%x-%x",
                &mac_str[0], &mac_str[1], &mac_str[2],
                &mac_str[3], &mac_str[4], &mac_str[5]);
        mac_addr[0] = mac_str[0]&0xff;
        mac_addr[1] = mac_str[1]&0xff;
        mac_addr[2] = mac_str[2]&0xff;
        mac_addr[3] = mac_str[3]&0xff;
        mac_addr[4] = mac_str[4]&0xff;
        mac_addr[5] = mac_str[5]&0xff;
    }
    else if(fd)
    {
        struct ifreq ifr;
        memset(&ifr, 0, sizeof(struct ifreq));
        safe_strncpy(ifr.ifr_name, _options.wanif, sizeof(ifr.ifr_name));
        if (ioctl(fd, SIOCGIFHWADDR, (caddr_t)&ifr) < 0) {
            log_err(errno, "ioctl(SIOCGIFHWADDR) failed");
            return 1;
        }
        memcpy(mac_addr, ifr.ifr_hwaddr.sa_data, PKT_ETH_ALEN);
    }
    else
    {
        log_dbg("get_apmac error!");
        return 1;
    }
    return 0;
}
...
static void dhcp_release_ip(struct dhcp_conn_t *conn) {
    struct app_conn_t *appconn = dhcp_get_appconn_addr(conn, (struct in_addr *)&conn->hisip);

    if (appconn) {
        log_dbg("[BridgeMode] connect mac: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
        ...
        if(ipm) {
            ...
            if (_options.kname) {
                syslog(LOG_DEBUG, "[nfcoova] kmod_coova_release %s(%d) " MAC_FMT,
                        __FUNCTION__, __LINE__, MAC_ARG(conn->hismac));
                kmod_coova_release(conn);
            }
#endif
            log_dbg("[BridgeMode] free ip %s", inet_ntoa(ipm->addr));
            if (ippool_freeip(ippool, ipm)) {
                log_dbg("[BridgeMode] free ip failure");
            }

            appconn->hisip.s_addr=0;
            appconn->uplink=0;
        }
    }
}
...
int dhcp_update_ip(struct dhcp_t *this,
		 struct dhcp_conn_t **conn,
		 struct in_addr *new_addr)
{
    /* if (_options.debug) */
    /*     syslog(LOG_DEBUG, " %s(%d) ip %s", */
    /*             __FUNCTION__, __LINE__, inet_ntoa(*new_addr)); */

    struct app_conn_t * old_appconn = 0;
    old_appconn = dhcp_get_appconn_ip(0, new_addr);

    //checksubnet
    if((new_addr->s_addr & _options.mask.s_addr) != _options.net.s_addr)
    {
        if (_options.debug)
            syslog(LOG_DEBUG, " %s(%d) different subnet request ignore new_ip %s",
                    __FUNCTION__, __LINE__,  inet_ntoa(*new_addr));
        return 0;
    }

    // new ip not used by other client
    if(!old_appconn && new_addr->s_addr != 0)
    {
        struct app_conn_t * old_appconn = (struct app_conn_t*)(*conn)->peer;

        old_appconn = dhcp_get_appconn_ip(0, &(*conn)->hisip);
        if(old_appconn && old_appconn->hisip.s_addr != new_addr->s_addr)
        {
            if (_options.debug)
                syslog(LOG_DEBUG, " %s(%d) release old ip %s",
                        __FUNCTION__, __LINE__, inet_ntoa((*conn)->hisip));
            dhcp_release_ip(*conn);
        }

		if (_options.debug)
			syslog(LOG_DEBUG, " %s(%d) new ip %s",
					__FUNCTION__, __LINE__, inet_ntoa(*new_addr));

        if(dhcp->cb_request(*conn, new_addr, 0, 0) == 0)
        {
#ifdef HAVE_NETFILTER_COOVA
            if (_options.kname) {
                struct app_conn_t * appconn1 = (struct app_conn_t*)(*conn)->peer;
                syslog(LOG_DEBUG, "[nfcoova] kmod_coova_update %s(%d) hisip %s hismac "MAC_FMT,
                        __FUNCTION__, __LINE__,
                        inet_ntoa(appconn1->hisip),
                        MAC_ARG(appconn1->hismac));
                kmod_coova_update(appconn1);
            }
#endif
        }
    }
    else if(new_addr->s_addr != (*conn)->hisip.s_addr)
    {
        if (_options.debug)
        {
            syslog(LOG_DEBUG, " %s(%d) new ip %s new mac "MAC_FMT,
                    __FUNCTION__, __LINE__,
                    inet_ntoa((*conn)->hisip),
                    MAC_ARG((*conn)->hismac));
        }

        if(new_addr->s_addr != 0) {
            if(old_appconn)
            {
                if (_options.debug)
                {
                    syslog(LOG_DEBUG, " %s(%d) release old ip %s old mac "MAC_FMT,
                            __FUNCTION__, __LINE__,
                            inet_ntoa(old_appconn->hisip),
                            MAC_ARG(old_appconn->hismac));
                }

                // this mac is ip conflic
                dhcp_release_mac(this, old_appconn->hismac, RADIUS_TERMINATE_CAUSE_LOST_CARRIER);
                // if have ip, release my ip
                dhcp_release_ip(*conn);
            }
 ...
 * Returns -1 if unsuccessful.
 **/
int dhcp_authconn(struct dhcp_t *this,
		 struct dhcp_conn_t **conn,
		 uint8_t *hwaddr)
{
    struct app_conn_t *appconn = (struct app_conn_t *)(*conn)->peer;
    ...
    if(_options.defsessiontimeout) {
        appconn->s_params.sessiontimeout=_options.defsessiontimeout;
    }
    else
    {
    ...
    {
        appconn->s_params.idletimeout=_options.defidletimeout;
    }
    else
    {
    ...
    if(_options.defgonetimeout)
    {
        appconn->s_params.gonetimeout=_options.defgonetimeout;
    }
    else
    {
    ...
    appconn->s_state.start_time = mainclock_now();
    appconn->s_state.last_sent_time = mainclock_now() - 25;
    appconn->s_state.last_time = mainclock_now();
    appconn->s_state.gone_time = mainclock_now();
    /* appconn->dnprot = DNPROT_DHCP_NONE; */
    log_dbg("dhcp_newconn: %d %d", (*conn)->authstate, appconn->s_state.authenticated);
...
int dhcp_getreq(struct dhcp_ctx *ctx, uint8_t *pack, size_t len) {
  ...
  switch(message_type->v[0]) {
    ...
    case DHCPRELEASE:
      if (_options.debug)
        syslog(LOG_DEBUG, "%s(%d): is DHCP-Release", __FUNCTION__, __LINE__);
...
int dhcp_receive_ip(struct dhcp_ctx *ctx, uint8_t *pack, size_t len) {
  ...
  if (!dhcp_hashget(this, &conn, pack_ethh->src)) {
    ...
    if(_options.bridgemode)
    {
        ...
        // just update ip, becasue I don't know packet from lan or from wifi
        ...
        {
            dhcp_update_ip(this, &conn, &reqaddr);
        }
    }
  } else {
  ...
  switch (authstate) {
    ...
    case DHCP_AUTH_PASS:
      ...
      if (_options.kname) {
        ...
        if(_options.bridgemode && pack_iph->daddr == _options.uamlisten.s_addr)
        {
...
else
{
  ...
  else
  {
      if(reqaddr.s_addr != 0)
      {
          ...
          {
              if (_options.debug)
                  syslog(LOG_DEBUG, "%s(%d): ARP Address found: %s, update ip", __FUNCTION__, __LINE__, inet_ntoa(reqaddr));
          ...
          if ( ! conn->is_reserved) {
              if ((_options.macoklen || (_options.dynmacoklen)) &&
                      (conn->authstate != DHCP_AUTH_PASS) &&
                      !maccmp(conn->hismac)) {
                  syslog(LOG_DEBUG, "%s(%d): conn->authstate: %d, change to DHCP_AUTH_PASS(%d)",
                          __FUNCTION__, __LINE__, conn->authstate, DHCP_AUTH_PASS);
                  ...
                  if (_options.kname) {
                      ...
                      syslog(LOG_DEBUG, "[nfcoova] kmod_coova_update %s(%d) hisip %s hismac "MAC_FMT,
                              ...
                              inet_ntoa(appconn1->hisip),
                              MAC_ARG(appconn1->hismac));
                      appconn1->s_state.authenticated = 1;
                      kmod_coova_update(appconn1);
                  }
#endif
              }

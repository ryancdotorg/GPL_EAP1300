...
static unsigned int ip_list_tot = 1024;
...
static void coova_entry_sync(struct coova_entry *e, int state,
        u_int64_t bytes_in, u_int64_t bytes_out,
        u_int64_t pkts_in, u_int64_t pkts_out)
{
	if(state < 0)
	{
		// don't change state
...
static void coova_entry_reset(struct coova_entry *e, int state)
{
	if(state < 0)
	{
		// don't change state
...
coova_mt(const struct sk_buff *skb, struct xt_action_param *par) 
{
	...
	unsigned char *dest_hwaddr = 0;
	bool ret = 0;
	const uint8_t bmac[ETH_ALEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	const uint8_t nmac[ETH_ALEN] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	...
	if (info->side != XT_COOVA_DEST) {
		if (skb_mac_header(skb) >= skb->head &&
			...
			dest_hwaddr = eth_hdr(skb)->h_dest;
		} else {
			return ret;
		}
	}
	...
	if (e == NULL) {
		// Yoln: auto learn if use mdmac
		if(memcmp(info->mdmac, nmac, ETH_ALEN))
		{
	...
	else if (hwaddr && e->hwaddr)
	{
		// for XT_COOVA_SOURCE, if not in the list, not match.
		if (memcmp(hwaddr, e->hwaddr, ETH_ALEN))
		{
			ret = 0;
			goto out;
		}
	...
	{
		//just continue, for XT_COOVA_DEST counting use.
	}
	...
	if (e->state) {
		...
		if (!info->invert)
		{
			if (info->side != XT_COOVA_DEST)
			{
				if(memcmp(info->mdmac, nmac, ETH_ALEN))
				{
					if(hwaddr && dest_hwaddr)
					{
						// not broadcast, change dest to br-lan mac
						if(memcmp(dest_hwaddr, bmac, ETH_ALEN) && memcmp(hwaddr, info->mdmac, ETH_ALEN))
						{
							memcpy(dest_hwaddr, info->mdmac, ETH_ALEN);
						}
					}
				}
			}
	...
	else
	{
		if (info->invert)
		{
			...
			else //XT_COOVA_DEST and not auth
			{
		    ...
		    size_t size, loff_t *loff)
{
...
#endif
	struct coova_entry *e;
	/* char buf[sizeof("+00-aa-bb-cc-dd-10@b335:1d35:1e55:dead:c0de:1715:5afe:c0de")]; */
	char buf[896];
	const char *c = buf;
	const char *p = NULL;
	union nf_inet_addr addr = {};
	...
	bool release=false;
	bool succ;
	unsigned int maci[ETH_ALEN];
	uint8_t mac[ETH_ALEN];
	int i, retsize;
	bool updatemac=false;
	bool reset=false;
	bool sync=false;
	u_int64_t bytes_in=0, bytes_out=0, pkts_in=0, pkts_out=0;
	int ret=0;
	...
	if(sync==true)
	{
		ret = sscanf(c, "%llu,%llu,%llu,%llu^%*s", &bytes_in, &bytes_out, &pkts_in, &pkts_out);
		if(ret != 4)
		{
			return -EINVAL;
		}
	...
	if(strnchr(c, size, '@') != NULL) {
		if(6==sscanf(c, "%2x-%2x-%2x-%2x-%2x-%2x@", &maci[0],&maci[1],&maci[2],&maci[3],&maci[4],&maci[5])) {
			for(i=0;i<6;i++)
				mac[i]=maci[i]&0xFF;
			c+=18;
			size-=18;
			...
			printk(KERN_INFO KBUILD_MODNAME ": illegal mac address written "
					"to procfs\n");
			return -EINVAL;
		}
	}
	...
	if (release) {
		...
		if (e != NULL) {
			...
			{
				coova_entry_sync(e, -1, bytes_in, bytes_out, pkts_in, pkts_out);
			}

			if(updatemac)
				memcpy(e->hwaddr, mac, ETH_ALEN);

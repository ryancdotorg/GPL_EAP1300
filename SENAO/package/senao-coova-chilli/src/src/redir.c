...
#define CHILLI_SUPPORT_OAUTH_LOGIN 1
...
static void bstring_buildurl(bstring str, struct redir_conn_t *conn,
  ...
  if (_options.networkid) {
    bcatcstr(str, amp);
    bcatcstr(str, "networkId=");
    bassigncstr(bt, _options.networkid);
    redir_urlencode(bt, bt2);
    bconcat(str, bt2);
  }
  ...
  if (_options.ssidid) {
    bcatcstr(str, amp);
    bcatcstr(str, "ssidId=");
    bassigncstr(bt, _options.ssidid);
    redir_urlencode(bt, bt2);
    bconcat(str, bt2);
  }
  ...
  if (_options.ssidprofileid) {
    bcatcstr(str, amp);
    bcatcstr(str, "ssidProfileId=");
    bassigncstr(bt, _options.ssidprofileid);
    redir_urlencode(bt, bt2);
    bconcat(str, bt2);
  }


  if (_options.modelname) {
    bcatcstr(str, amp);
    bcatcstr(str, "model=");
    bassigncstr(bt, _options.modelname);
    redir_urlencode(bt, bt2);
    bconcat(str, bt2);
  }

  if (_options.fwversion) {
    bcatcstr(str, amp);
    bcatcstr(str, "version=");
    bassigncstr(bt, _options.fwversion);
    redir_urlencode(bt, bt2);
    bconcat(str, bt2);
  }
  ...
  if(strlen(conn->s_state.redir.redirurl)>0)
  {
      if(strcmp(resp, "success") == 0 || strcmp(resp, "already") == 0)
      {
          bcatcstr(str, amp);
          bcatcstr(str, "redirurl=");
          bassigncstr(bt, conn->s_state.redir.redirurl);
          redir_urlencode(bt, bt2);
          bconcat(str, bt2);
      }
  }
  else if (_options.redirurl && redirurl) {
    bcatcstr(str, amp);
    bcatcstr(str, "redirurl=");
...
int redir_reply(struct redir_t *redir, struct redir_socket_t *sock,
  ...
  if((_options.logintype) && (atoi(_options.logintype)==RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_CLOUD_FACEBOOK_WIFI))
  {
      is_fbwifi = 1;
  }
  else
  ...
  if (conn->format == REDIR_FMT_JSON) {
    ...
    if (resp) {
      ...
      buffer_url = bfromcstralloc(1024, "");
      if(is_fbwifi)
      {
        bcatcstr(buffer_url, redir->homepage);
      } else if (url) {

        bconcat(buffer_url, url);

      } else if (!_options.redirurl && redirurl && *redirurl) {
	bcatcstr(buffer_url, redirurl);
      } else {
        bt = bfromcstralloc(1024,"");
        redir_buildurl(conn, bt, redir, resp, timeleft, hexchal,
                       uid, userurl, reply, redirurl, hismac, hisip);
        bconcat(buffer_url, bt);
        bdestroy(bt);
      }

      bconcat(buffer, buffer_url);
      bcatcstr(buffer, "\r\nContent-Type: text/html; charset=UTF-8\r\n");

      if (is_fbwifi)
      {
      bbody = bfromcstralloc(2048, "");
      bcatcstr(bbody, "<html><head><title>Redirecting to Captive Portal</title></head><body>"
                           "If you are not redirected within 5 seconds, please"
                           "<a href=\"");
      bconcat(bbody, buffer_url);
      bcatcstr(bbody, "\">click here</a>.</body></html>\r\n");
      }
      else
      {
...
static int redir_getreq(struct redir_t *redir, struct redir_socket_t *sock,
  ...
  while (!done && (redir->starttime + REDIR_HTTP_MAX_TIME) > mainclock_now()) {
	...
	{
		p1++;
		while (*p1 == '/') p1++; /* Advance through additional splash space */
	}
	...
	*p2 = 0;
	...
	else if ((_options.logintype) && (atoi(_options.logintype)==RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_CLOUD_FACEBOOK_WIFI) &&
                 (!strncmp(path, "fbwifi/v2.0/", 12)) && (strlen(path) > 12))
	    conn->type = REDIR_FBWIFI;
    ...
    else if (!strncasecmp(buffer,"Origin:",7)) {
	  p = buffer + 7;
	  strlcpy(httpreq->origin, p, sizeof(httpreq->origin));
	  while (*p && isspace((int) *p)) p++;
#if(_debug_ > 1)
          if (_options.debug)
            syslog(LOG_DEBUG, "%s(%d): Origin: %s", __FUNCTION__, __LINE__, httpreq->origin);
#endif
	}
      }
...
#if 1// support redirurl
      if (!redir_getparam(redir, httpreq->qs, "redirurl", bt)) {
          bstring bt2 = bfromcstr("");
          redir_urldecode(bt, bt2);
          bstrtocstr(bt2, conn->s_state.redir.redirurl, sizeof(conn->s_state.redir.redirurl));
          log_dbg("-->> Setting redirurl=[%s]", conn->s_state.redir.redirurl);
          bdestroy(bt2);
      }
      else
      {
          bstring bt2 = bfromcstr("");
          bstrtocstr(bt2, conn->s_state.redir.redirurl, sizeof(conn->s_state.redir.redirurl));
          log_dbg("-->> Setting redirurl null");
          bdestroy(bt2);
      }
#endif

#if 1 // for oauth_AAA use (login_type 105)

      if (!redir_getparam(redir, httpreq->qs, "provider", bt)) {
          bstring bt2 = bfromcstr("");
          redir_urldecode(bt, bt2);
          bstrtocstr(bt2, conn->s_state.redir.provider, sizeof(conn->s_state.redir.provider));
          log_dbg("-->> Setting provider=[%s]", conn->s_state.redir.provider);
          bdestroy(bt2);
      }
      else
      {
          bstring bt2 = bfromcstr("");
          bstrtocstr(bt2, conn->s_state.redir.provider, sizeof(conn->s_state.redir.provider));
          log_dbg("-->> Setting provider null=[%s]",conn->s_state.redir.provider);
          bdestroy(bt2);
      }

      if (!redir_getparam(redir, httpreq->qs, "token", bt)) {
          FILE *fp;
          char filename[64] = {0};

          bstring bt2 = bfromcstr("");
          redir_urldecode(bt, bt2);
          bstrtocstr(bt2, token, REDIR_TOKENSIZE);
          log_dbg("[Social-Login] redir Setting token=[%s]", token);
          bdestroy(bt2);
          sprintf(filename, "/var/run/chilli_token_%s_%.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
                  _options.configname,
                  conn->hismac[0], conn->hismac[1], conn->hismac[2],
                  conn->hismac[3], conn->hismac[4], conn->hismac[5]
                 );
          /* log_dbg("-->>[Social-Login] redir token filename=[%s]", filename); */
          fp = fopen(filename, "w");
          if(fp)
          {
              fprintf(fp, "%s", token);
...
#if CHILLI_SUPPORT_OAUTH_LOGIN
    if(_options.oauth) {
      if (!redir_getparam(redir, httpreq->qs, "oauth", bt)) {
          char oauth[64]={0};
          // oauth = md5(username+secret)
          MD5_CTX context;
          unsigned char cksum[16];
          char hex[32+1];
          int i;

          bstrtocstr(bt, oauth, 64);
          log_dbg("checking for oauth login...");
          log_dbg("-->> oauth=[%s]", oauth);
          log_dbg("-->> username=[%s]", conn->s_state.redir.username);

          if(_options.defsessiontimeout) {
              conn->s_params.sessiontimeout=_options.defsessiontimeout;
          }
          else
          {
              conn->s_params.sessiontimeout=0;
          }
          log_dbg("-->> sessiontimeout=[%d]", conn->s_params.sessiontimeout);

          if(_options.defidletimeout)
          {
              conn->s_params.idletimeout=_options.defidletimeout;
          }
          else
          {
              conn->s_params.idletimeout=0;
          }
          log_dbg("-->> idletimeout=[%d]", conn->s_params.idletimeout);

          MD5Init(&context);
          MD5Update(&context, (uint8_t *)conn->s_state.redir.username, strlen(conn->s_state.redir.username));

          if (redir->secret && *redir->secret) {
              /* take the md5 of the $username+$uamsecret+cLoUd as a checksum */
              log_dbg("-->> secret=[%s]", redir->secret);
              MD5Update(&context, (uint8_t *)redir->secret, strlen(redir->secret));
              MD5Update(&context, (uint8_t *)"cLoUd", 5);
          }
          else {
              log_dbg("-->> no uam_secret");
          }
          MD5Final(cksum, &context);
          ...
          log_dbg("-->> checkmd5=[%s]", hex);

          if(strcasecmp(oauth, hex)==0)
          {
  ...
  case REDIR_FBWIFI:
    {
      ...
      int wlen = snprintf(wwwfile, sizeof(wwwfile), "%s", conn->wwwfile);
      int flen = snprintf(filepath, sizeof(filepath), "%s/%s", _options.wwwdir, wwwfile);

      // WAR: remove last char '/' , becasue fbwifi cert use fbwifi/v2.0/auth/?token=12345 to check...
      if(wlen > 13 && filepath[flen-1] == '/')
      {
          filepath[flen-1] = '\0';
          wwwfile[wlen-1] = '\0';

          bstring bt3 = bfromcstr(wwwfile);
          bstrtocstr(bt3, conn->wwwfile, sizeof(conn->wwwfile));
          bdestroy(bt3);

          log_dbg("Re Serving file %s", conn->wwwfile);
      }

      if(access(filepath, F_OK) != -1)
      {
          // file exists run haserl
          // go through
      }
      else
      {
          // file doesn't exist
          bstring bt4 = bfromcstr("fbwifi/redir");
          bstrtocstr(bt4, conn->wwwfile, sizeof(conn->wwwfile));
          bdestroy(bt4);
          log_dbg("Re Serving file %s", conn->wwwfile);
      }
    } 
...
static int redir_cb_radius_auth_conf(struct radius_t *radius,
  ...
  struct radius_server_t *radset;
  radset = &radius->radserver[0];
  ...
  if (_options.debug) {
    ...
    if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
      ...
      if (!radius_keydecode(radius, dstbuffer, RADIUS_ATTR_VLEN, &dstlen,
			    ...
			    pack_req->authenticator, radset->secret0,
			    radset->secretlen0) != 0) {
...
#if 1 // for oauth_AAA use (login_type 105)
  if(strlen(conn->s_state.redir.provider) > 0)
  {
      radius_addattr(radius, &radius_pack, RADIUS_ATTR_VENDOR_SPECIFIC,
              RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_OAUTH_PROVIDER,
              0, (uint8_t*) conn->s_state.redir.provider, strlen(conn->s_state.redir.provider));
  }
#endif
    ...
    case REDIR_WWW:
      {
...
#endif
        char isFBWIFI = conn.type == REDIR_FBWIFI;
        pid_t forkpid;
        int fd = -1;
        ...
        if (_options.wwwdir && (*conn.wwwfile
          ...
          if (isFBWIFI) {
              char buf[128] = {0};
              safe_snprintf(buf, sizeof(buf), "%s/%s", _options.wwwdir, filename);
              if(access(buf, F_OK) != -1)
              {
                  // file exists
                  ctype = "application/json"; parse = 1;
              }
          }
          else
          {
...
#if 1//senao mod
            setenv("CHI_REDIRURL", conn.s_state.redir.redirurl, 1);
#endif
            safe_snprintf(buffer, sizeof(buffer), "%lld", conn.s_state.input_octets);
            setenv("CHI_INPUT_BYTES", buffer, 1);
            snprintf(buffer, sizeof(buffer), "%" PRId64, conn.s_state.output_octets);
            setenv("CHI_OUTPUT_BYTES", buffer, 1);
            snprintf(buffer, sizeof(buffer), "%" PRId64, conn.s_params.sessiontimeout);
...
#endif
              case REDIR_FBWIFI:
                setenv("HTTP_ORIGIN", httpreq.origin, 1);


                int is_fbwifi_auth = 0;

                if(httpreq.is_post && strncmp(filename, "fbwifi/v2.0/auth", 16) == 0)
                {
                    is_fbwifi_auth = 1;
                }

                char auth_file[256] = {0};
                char done_file[256] = {0};

                snprintf(auth_file, sizeof(auth_file), "/var/run/fbwifi/%s/tokens/"MAC_FMT"-auth",
                        _options.configname, MAC_ARG(conn.hismac));

                snprintf(done_file, sizeof(done_file), "/var/run/fbwifi/%s/tokens/"MAC_FMT"-done",
                        _options.configname, MAC_ARG(conn.hismac));
                ...
                if(pid < 0)
                {
                    ...
                    execv(*binqqargs, binqqargs);
                    /* syslog(LOG_DEBUG, "Return not expected. Must be an execv error.n"); */
                }
                ...
                if(is_fbwifi_auth)
                {
                    ...
                    if (_options.debug)
                    {
                        syslog(LOG_DEBUG, "[FBWIFI] token path [%s]", auth_file);
                        syslog(LOG_DEBUG, "[FBWIFI] token path [%s]", done_file);
                    }
                    ...
                    while(counter < 8)
                    {
                        counter++;
                        if (_options.debug)
                            syslog(LOG_DEBUG, "[FBWIFI] check token [%d/7]", counter);
                        if(access(auth_file, F_OK) != -1)
                        {
                            // file exist, check token file.

                            fp = fopen(auth_file, "r");
                            if(fp)
                            {
                                if( fgets(token, REDIR_TOKENSIZE, fp) != NULL ) {
                                    //puts(token);
                                }
                                fclose(fp);
                            }
                            ...
                            {
                                if (_options.debug)
                                    syslog(LOG_DEBUG, "[FBWIFI] token:[%s]", token);
                                // file exist, authenticated client.
                                if (strcmp(token, "fail") == 0)
                                {
                                    // do nothing curl fail.
                                }
                                else if (strcmp(token, "null") == 0)
                                {
                                    msg.mtype = REDIR_FBWIFI;
                                    conn.s_params.sessiontimeout=0;
                                    conn.s_params.idletimeout=0;
                                    strncpy(conn.s_state.redir.username, "null", REDIR_USERNAMESIZE);
                                ...
                                else
                                {
                                    ...
                                    conn.s_params.sessiontimeout=0;
                                    conn.s_params.idletimeout=0;
                                    strncpy(conn.s_state.redir.username, token, REDIR_USERNAMESIZE);
                                    redir_msg_send(REDIR_MSG_OPT_REDIR | REDIR_MSG_OPT_PARAMS);

                                    int counter_done = 0;
                                    while(counter_done < 6)
                                    {
                                        counter_done++;
                                        if (_options.debug)
                                            syslog(LOG_DEBUG, "[FBWIFI] check done [%d/6]", counter_done);
                                        if(access(done_file, F_OK) != -1)
                                        {
                                            break;
                                        }
                                        sleep(1);
  ...
  switch (conn.type) {
    ...
    case REDIR_LOGIN: {
      ...
      if (conn.response == REDIR_SUCCESS) { /* Accept-Accept */
        ...
        bstring custom_redirurl;

        if((_options.logintype) && (atoi(_options.logintype)==RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_WATCHGUARD_CLICK_THROUGH))
        {
            if(strlen(conn.s_state.redir.redirurl) > 0)
            {
                custom_redirurl = bfromcstr(conn.s_state.redir.redirurl);
            }
            else
            {
                custom_redirurl = bfromcstr(conn.s_state.redir.userurl);
            }
        }
        else
        {
            custom_redirurl = NULL;
        }

        redir_reply(redir, &socket, &conn, REDIR_SUCCESS, custom_redirurl,

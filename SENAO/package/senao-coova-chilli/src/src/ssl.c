...
#ifdef HAVE_OPENSSL
static int _openssl_passwd_hc(char *buf, int size, int rwflag, void *ud) {
  strlcpy(buf, "027ea461ea7d29fa8c605b0975eb476b7445ced4", size);
  return strlen(buf);
}
...
openssl_env_init(openssl_env *env, char *engine, int server) {
...
#ifdef HAVE_OPENSSL
  {
    ...
    else if(_options.sslkeypasshc) {
      SSL_CTX_set_default_passwd_cb(env->ctx, _openssl_passwd_hc);
    }

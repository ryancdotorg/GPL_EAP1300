...
#endif
#ifdef SUPPORT_SENAO_LOG
#include "snlog.h"
#include "snlog_portal.h"
#endif
...
static int check_net_route(char *ifname, uint32_t dstip)
{
    FILE *fp;
    char *line = NULL;
    size_t len = 0;
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        /* log_dbg("%s\t%08x\t%08x\t%04x\t%d\t%d\t%d\t%08x\t%d\t%u\t%u\n", */
        /*         iface, (dest), (gw), flags, refcnt, use, */
        /*         metric, (mask), mtu, window, irtt); */
        if(mask==0xffffffff && strcmp(ifname, iface) == 0)
        {
            /* struct in_addr rtaddr; */
            /* rtaddr.s_addr = dest; */

            /* log_dbg("[BridgeMode] routing tab [%s] ", inet_ntoa(rtaddr)); */
            /* log_dbg("[BridgeMode] request ip  [%s]", inet_ntoa(dstaddr)); */

            if(dest==dstip)
            {
                struct in_addr dstaddr;
                dstaddr.s_addr = dstip;
                log_dbg("[BridgeMode] %s client %s in routing table", ifname, inet_ntoa(dstaddr));
                matched = 1;
                break;
            }
        }
...
static int acct_req(acct_type type,
		    ...
		    uint8_t status_type);
static int __acct_req(acct_type type,
		    struct app_conn_t *conn,
		    uint8_t status_type, int reset);
...
#endif
    //FIXME WAR: don't relaunch chilli_proxy, to avoid loop restart...
#if 0//def ENABLE_CHILLIPROXY
...
static void _set_sessionid(struct app_conn_t *appconn, char full, char *custom) {
  ...
  if(full)
  {
      ...
      else
      {
          ...
          // yolin mod to avoid create the same sessionid from different ap
          snprintf(appconn->s_state.sessionid,
                  sizeof(appconn->s_state.sessionid),
                  "%.2X%.2X%.2X%.2X%.2X%.2X-"
                  "%.2X%.2X%.2X-%d-"
                  "%.8lx",
                  MAC_ARG(his), called[3], called[4], called[5],
                  (_options.ssidid ? atoi(_options.ssidid) : 0),
                  appconn->rt);
      }
  }
...
static void set_sessionid(struct app_conn_t *appconn, char full) {
    _set_sessionid(appconn, full, NULL);
}
...
int runscript(struct app_conn_t *appconn, char* script,
	      char *loc, char *oloc, int forcesend) {
  int status;
  ...
  if(forcesend)
    set_env("GUESTSYNCD_FORCESEND", VAL_STRING, "1", 0);

  set_env("RUNSCRIPT", VAL_STRING, "chilli", 0);
#ifdef ENABLE_LAYER3
  if (_options.layer3)
  ...
  set_env("IDLE_TIME", VAL_ULONG, &idletime, 0);
  gonetime = mainclock_diffu(appconn->s_state.gone_time);
  set_env("GONE_TIME", VAL_ULONG, &gonetime, 0);
  set_env("AUTHENTICATED", VAL_STRING, appconn->s_state.authenticated ? "1" : "0", 1);
  set_env("SYNC_FLAG", VAL_STRING, appconn->s_state.sync_flag ? "1" : "0", 1);
...
#if 1
      // support NAT mode use mac to determine ip
    if (!_options.bridgemode && _options.natipbymac)
    {
        struct in_addr natip;
        struct in_addr nattmpip;

        // mac[2].mac[3].mac[4].mac[5]
        nattmpip.s_addr = hismac[5] * 0x1000000 +
            hismac[4] * 0x10000 +
            hismac[3] * 0x100 +
            hismac[2];

        natip.s_addr = _options.uamlisten.s_addr & _options.mask.s_addr;
        natip.s_addr |= nattmpip.s_addr & ~(_options.mask.s_addr);

        hisip = &natip;

       if (_options.debug)
           syslog(LOG_DEBUG, "%s(%d): NAT mode use hash ip %s", __FUNCTION__, __LINE__,  inet_ntoa(*hisip));
    }
#endif
...
static int dnprot_terminate(struct app_conn_t *appconn) {
  ...
  if (_options.kname) {
    syslog(LOG_DEBUG, "[nfcoova] kmod_coova_update %s(%d) hisip %s hismac "MAC_FMT,
...
#ifdef SUPPORT_SENAO_LOG
char* terminate_cause_to_string(int terminate_cause)
{
...
// if chages user_here status, return 1
int update_user_here(struct app_conn_t *appconn, int reason) {
    int user_here = 0;
    char ssidprofileid[32] = {0};
    char client_ip[56] = {0};
    int in_routing_table = 0;
    int skip_add_route = 0;
    ...
    if(atoi(_options.logintype) == RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_CLOUD_FACEBOOK_WIFI)
    {
        // if fbwifi, must delay route replace timeimg, because after client auth, it will query AP again very quickly. if add routing here, packet will lost.
        skip_add_route = 1;
    }

    if(_options.ssidprofileid)
        snprintf(ssidprofileid, sizeof(ssidprofileid), _options.ssidprofileid);

    strlcpy(client_ip, inet_ntoa(appconn->hisip), sizeof(client_ip));

    switch(reason)
    {
        case USER_HERE_LOGOUT:
#ifdef SUPPORT_SENAO_LOG
            {
                CLOUD_EVENT(LOG_INFO, SNLOG_USER_LOGOUT_EVENT,
                        " ssid_profile_id='%s',"
                        " client_mac='%.2x:%.2x:%.2x:%.2x:%.2x:%.2x',"
                        " client_ip='%s',"
                        " username='%.256s',"
                        " session_timeout='%d',"
                        " idle_timeout='%d',"
                        ...
                        mainclock_diffu(appconn->s_state.last_sent_time),
                        terminate_cause_to_string(appconn->s_state.terminate_cause));
            }
#endif
            user_here = 0;
            appconn->s_state.sync_flag = 0;
            appconn->s_params.flags |= NO_ACCOUNTING;
            break;
        case USER_HERE_ROAMING:
            user_here = 0;
            // roaming to other ap
            if (appconn->s_state.sync_flag == 1 && appconn->s_state.authenticated == 1)
            {
                appconn->s_state.terminate_cause = RADIUS_TERMINATE_CAUSE_LOST_CARRIER;
            ...
            break;
        default:
        case USER_HERE_LEAVE:
            user_here = 0;
            break;
        case USER_HERE_LOGIN:
#ifdef SUPPORT_SENAO_LOG
            {
                CLOUD_EVENT(LOG_INFO, SNLOG_USER_LOGIN_EVENT,
                        " ssid_profile_id='%s',"
                        " client_mac='%.2x:%.2x:%.2x:%.2x:%.2x:%.2x',"
                        " client_ip='%s',"
                        " username='%.256s',"
            ...
            user_here = 1;
            appconn->s_params.flags &= ~NO_ACCOUNTING;
            appconn->s_state.sync_flag = 1;
            break;
        case USER_HERE_UPDATE:
            user_here = 1;
            appconn->s_params.flags &= ~NO_ACCOUNTING;
            // roaming to this ap
            if (appconn->s_state.sync_flag == 0 && appconn->s_state.authenticated == 1)
            {
                    _set_sessionid(appconn, 1, NULL);
                    __acct_req(ACCT_USER, appconn, RADIUS_STATUS_TYPE_START, 0);
            }
            appconn->s_state.sync_flag = 1;
            break;
    }

    appconn->s_state.user_here = append_user_here(appconn->s_state.user_here, user_here);
    switch(appconn->s_state.user_here)
    {
        case 0:
        default:
            // user not here, do nothing
            return 0;
        case 1:
            // detect user connect to here
            if (_options.debug)
                syslog(LOG_DEBUG, "[ClientInfo] %s(%d) User here hisip %s hismac "MAC_FMT,
                        __FUNCTION__, __LINE__,
                        inet_ntoa(appconn->hisip),
                        MAC_ARG(appconn->hismac));
            if(reason != USER_HERE_LOGIN && appconn->s_state.authenticated == 1)
            {
                if(appconn->s_params.sessiontimeout)
                {
                    if( (mainclock_diffu(appconn->s_state.start_time) > 10) &&
                            (appconn->s_params.sessiontimeout - mainclock_diffu(appconn->s_state.start_time) > 20))
                    {
                        // because macup.sh +2 sec session time to other device, restore 1 secs here
                        appconn->s_state.start_time = appconn->s_state.start_time + 1;
                    }
                }

                if(_options.acct_update) {
                    if (appconn->s_params.interim_interval)
                    {
                        // wait 8 sec to send interim update
                        appconn->s_state.interim_time = mainclock - appconn->s_params.interim_interval + 8;
                    }
                }

#ifdef SUPPORT_SENAO_LOG
                CLOUD_EVENT(LOG_INFO, SNLOG_USER_ROAMING_EVENT,
                        " ssid_profile_id='%s',"
                        " client_mac='%.2x:%.2x:%.2x:%.2x:%.2x:%.2x',"
                        " client_ip='%s',"
                        " username='%.256s',"
                ...
                // FIXME: war to fix nat download tp 100->200
                if(!_options.bridgemode && appconn->s_state.authenticated == 1)
                {
                    in_routing_table = check_net_route(_options.dhcpif, appconn->hisip.s_addr);

                    if(in_routing_table == 0)
                    {
                        log_dbg("[NATMode] replace route for %s dev %s", client_ip, _options.dhcpif);
                        ...
                        {
                            sprintf(cmd, "ip route replace %s dev %s", client_ip, _options.dhcpif);
                            system(cmd);
                        }

                        if (_options.kname) {
                            char table_name[16] = {0};
                            if(strcmp("br-lan", _options.wanif))
                            {
                                sprintf(table_name, "%s%s", _options.configname, "vlan");
                            }
                            else
                            {
                                sprintf(table_name, "%s", "portal-main");
                            }
                            ...
                            {
                                sprintf(cmd, "ip route replace %s dev %s table %s", client_ip, _options.dhcpif, table_name);
                                system(cmd);
                            }
                        }
                    }
            ...
            return 1;
        case 2:
            // detect user leave from here
            if (_options.debug)
                syslog(LOG_DEBUG, "[ClientInfo] %s(%d) User leaving hisip %s hismac "MAC_FMT,
                        __FUNCTION__, __LINE__,
                        inet_ntoa(appconn->hisip),
                        MAC_ARG(appconn->hismac));
            ...
            if(_options.acct_update) {
                if (appconn->s_params.interim_interval)
                {
                    // send interim update immediately
                    appconn->s_state.interim_time = mainclock - appconn->s_params.interim_interval;
                }
            }

            // FIXME: war to fix nat download tp 100->200
            if(!_options.bridgemode && appconn->s_state.authenticated == 0)
            {
                in_routing_table = check_net_route(_options.dhcpif, appconn->hisip.s_addr);

                if(in_routing_table == 1)
                {
                    log_dbg("[NATMode] del route for %s dev %s", client_ip, _options.dhcpif);
                    char cmd[128] = {0};
                    sprintf(cmd, "ip route del %s dev %s", client_ip, _options.dhcpif);
                    system(cmd);
                    if (_options.kname) {
                        char table_name[16] = {0};
                        if(strcmp("br-lan", _options.wanif))
                        ...
                        sprintf(cmd, "ip route del %s dev %s table %s", client_ip, _options.dhcpif, table_name);
                        system(cmd);
                    }
                }
            }
...
void session_interval(struct app_conn_t *conn) {
...
#if 1
  if(conn->s_state.sync_flag == 1)
  {
      if (current_user_here(conn->s_state.user_here) == 0)
      {
          sync_interval = 35;
      }
      else
      ...
      if(mainclock_diffu(conn->s_state.last_sync_time) > sync_interval)
      {
          do_sync = 1;
      }
  }
  ...
  if (current_user_here(conn->s_state.user_here) == 0)
  {
      conn->s_state.connect_start_time = mainclock;
  }
  else
  {
      conn->s_state.gone_time = mainclock;
  }
#endif
...
#if 1
  // not into terminate_appconn in previout code
  if(conn->s_state.authenticated == 1)
  {
      // last_sent_time will update by dhcp-new, use sync_flag to monitor packet
      if(do_sync == 1)
      {
          conn->s_state.last_sync_time = mainclock_now();
          if (_options.macup) {
              // syslog(LOG_DEBUG, "%s(%d): Calling MAC up script: %s", __FUNCTION__, __LINE__, _options.macup);
              runscript(conn, _options.macup, 0, 0, 1);
          }
      }
      else if (current_user_here(conn->s_state.user_here) == 0 && conn->s_params.gonetimeout > 0)
      {
          if(mainclock_diffu(conn->s_state.gone_time) > conn->s_params.gonetimeout || mainclock_diffu(conn->s_state.last_sent_time) > conn->s_params.gonetimeout)
          {
              terminate_appconn(conn, RADIUS_TERMINATE_CAUSE_LOST_CARRIER);
          }
      }
  }
#endif
...
#ifdef HAVE_NETFILTER_COOVA
  if (_options.kname) {
    // syslog(LOG_DEBUG, "[nfcoova] kmod_coova_sync %s(%d)", __FUNCTION__, __LINE__);
    kmod_coova_sync();
  }
#endif
...
int static macallow_update_by_file(char *filename, int dynamic) {
    FILE *fp = NULL;
    char *line = NULL;
    size_t len = 0;
    ssize_t read;
    ...
    if(dynamic == 1)
    {
        ...
        if (fp==NULL)
        {
            log_dbg("[MacallowedFile] %s(%d): %s open error", __FUNCTION__, __LINE__, filename);
            return -1;
        }
        else
        {
            /* read mac list */
            while ((read = getline(&line, &len, fp)) != -1) {
                if (_options.dynmacoklen>=DYN_MACOK_MAX)
                {
                    log_dbg("[MacallowedFile] %s(%d): %s list size > %d", __FUNCTION__, __LINE__, filename, DYN_MACOK_MAX);
                    break;
                }
                else
                {
                    ...
                    for (i = 0; i < 6; i++)
                    {
                        _options.dynmacok[_options.dynmacoklen][i] = (unsigned char) mac[i];
                    }

                    log_dbg("[MacallowedFile] %s(%d):" MAC_FMT2, __FUNCTION__, __LINE__, MAC_ARG(_options.dynmacok[_options.dynmacoklen]));
                    _options.dynmacoklen++;
                }
            }
    ...
    else
    {
        ...
        else
        {
            ...
            while ((read = getline(&line, &len, fp)) != -1) {
                ...
                {
                    log_dbg("[MacallowedFile] %s(%d): %s list size > %d" , __FUNCTION__, __LINE__, filename, MACOK_MAX);
                    break;
                }
                else
                {
                    ...
                    log_dbg("[MacallowedFile] %s(%d):" MAC_FMT2, __FUNCTION__, __LINE__, MAC_ARG(_options.macok[_options.macoklen]));
                    _options.macoklen++;
                }
            }
...
int static macallow_flush(int dynamic)
{
    int i;

    if(dynamic == 1)
    {
        for (i=0; i<_options.dynmacoklen; i++)
        {
            memset(_options.dynmacok[i], 0, PKT_ETH_ALEN);
        }
        _options.dynmacoklen = 0;
    }
    else
    {
        for (i=0; i<_options.macoklen; i++)
        {
            memset(_options.macok[i], 0, PKT_ETH_ALEN);
        }
        _options.macoklen = 0;
    }
    return 0;
...
int static macallow_remove(unsigned char *mac, int dynamic)
{
    int ret = -1;
    int i;
    ...
    if(dynamic == 1)
    {
        for (i=0; i<_options.dynmacoklen; i++)
        {
            if (!memcmp(mac, _options.dynmacok[i], PKT_ETH_ALEN))
            {
                //copy latest to matched, and clear latest
                memcpy(_options.dynmacok[i], _options.dynmacok[_options.dynmacoklen-1], PKT_ETH_ALEN);
                memset(_options.dynmacok[_options.dynmacoklen-1], 0, PKT_ETH_ALEN);
                _options.dynmacoklen--;
                ret = 0;
                break;
            }
    ...
    else
    {
        for (i=0; i<_options.macoklen; i++)
        {
            ...
            {
                //copy latest to matched, and clear latest
                memcpy(_options.macok[i], _options.macok[_options.macoklen-1], PKT_ETH_ALEN);
                memset(_options.macok[_options.macoklen-1], 0, PKT_ETH_ALEN);
                _options.macoklen--;
                ret = 0;
                break;
            }
...
int static macallow_add(unsigned char *mac, int dynamic)
{
    int match = 0, ret = -1;
    int i;
    ...
    if(dynamic == 1)
    {
        ...
        if(match == 0)
        {
            if(_options.dynmacoklen < DYN_MACOK_MAX)
            {
                memcpy(_options.dynmacok[_options.dynmacoklen], mac, PKT_ETH_ALEN);
                _options.dynmacoklen++;
                ret = 0;
            }
            else
    ...
    else
    {
        ...
        if(match == 0)
        {
            if(_options.macoklen < MACOK_MAX)
            {
                memcpy(_options.macok[_options.macoklen], mac, PKT_ETH_ALEN);
                _options.macoklen++;
                ret = 0;
            }
            else
...
// action 0:remove 1:add 2:flush 3:add by file
int static macallow_update(struct cmdsock_request *req, int dynamic, int action) {
    ...
    {
        case 3: //file
            macallow_flush(dynamic);
            ch = strchr(req->d.data, '=');
            if(ch)
            {
                macallow_update_by_file(ch+1, dynamic); //req->d.data = "@file="
            }
            break;
        ...
        case 0: //remove
            macallow_remove(req->mac, dynamic);
            break;

        default:
        case 1: //add
            macallow_add(req->mac, dynamic);
            break;
    }
    return 0;
}
...
static int __acct_req(acct_type type,
		    struct app_conn_t *conn, uint8_t status_type, int reset)
{
  struct radius_packet_t radius_pack;
  ...
  switch(status_type) {
    ...
    case RADIUS_STATUS_TYPE_ACCOUNTING_ON:
      ...
      conn->s_state.acct_start_time = mainclock;
      conn->s_state.connect_start_time = mainclock;
      conn->s_state.gone_time = mainclock;
  ...
  if (RADIUS_STATUS_TYPE_ACCOUNTING_ON  != status_type &&
    ...
    switch(type) {
      ...
      case ACCT_USER:
        ...
        {
            strlcpy(conn->s_state.redir.username, "-", USERNAMESIZE);
        }
        radius_addattr(radius, &radius_pack, RADIUS_ATTR_USER_NAME, 0, 0, 0,
  ...
  if (status_type == RADIUS_STATUS_TYPE_STOP ||
    ...
    switch(type) {
      ...
      case ACCT_USER:
        ...
        timediff = mainclock_diffu(conn->s_state.acct_start_time);
...
{
    return __acct_req(type, conn, status_type, 1);
}
...
int dnprot_accept(struct app_conn_t *appconn) {
  ...
  if (!(appconn->s_params.flags & REQUIRE_UAM_AUTH)) {
    ...
    /* 2020/5/8 Already fixed this WAR in the following commit, but I cannot remove this code, it is horrible...
     *
     *  * 82f485e - <yolin>
     *  | use curl to grap redirect http page and send to user (Fri May 8 09:43:43 2020)
     *  ...
     *  * 776bffe - <yolin>
     *  | use chilli do 302 redirect to userurl (Mon Apr 20 17:51:18 2020)
     */
    /* 2020/5/13 bridge mode iptable nat rule change from SNAT to NETMAP need this code. */
    //Yolin: WAR to fix after login cannot redirect to userurl (it will reset by server)
    if(_options.bridgemode)
    {
        struct in_addr tmp;
        char cmd[64] = {0};
        tmp.s_addr = _options.uamlisten.s_addr & _options.mask.s_addr;
        tmp.s_addr |= appconn->hisip.s_addr & ~(_options.mask.s_addr);
        // delete conntrack
        // ( uamlisten & netmask ) || ( addr & ~netmask )
        sprintf(cmd, "conntrack -D -s %s", inet_ntoa(tmp));
        system(cmd);
    }
    else // nat mode
    {
        ...
        {
            // if fbwifi, must delay route replace timeimg, because after client auth, it will query AP again very quickly. if add routing here, packet will lost.
            // delay to add routing rule to WAR
            skip_add_route = 1;
        }
        ...
        if(in_routing_table == 0)
        {
            ...
            if (_options.kname) {
                ...
                if(strcmp("br-lan", _options.wanif))
                {
                    sprintf(table_name, "%s%s", _options.dhcpif, "vlan");
                }
                else
                {
                    sprintf(table_name, "%s", "portal-main");
    ...
    update_user_here(appconn, USER_HERE_LOGIN);
 ...
 if (_options.debug)
     syslog(LOG_DEBUG, "%s(%d): Calling connection up script: %s\n", __FUNCTION__, __LINE__, _options.conup);
      runscript(appconn, _options.conup, 0, 0, 0);
    }

    if (_options.macup) {
        if (_options.debug)
          syslog(LOG_DEBUG, "%s(%d): Calling MAC up script: %s", __FUNCTION__, __LINE__, _options.macup);
        runscript(appconn, _options.macup, 0, 0, 1);
        appconn->s_state.last_sync_time = mainclock_now();
        appconn->s_state.gone_time = mainclock_now();
    }
  }
...
chilli_learn_location(uint8_t *loc, int loclen,
  ...
  if (appconn->s_state.authenticated == 1 &&
    ...
    appconn->s_state.gone_time = mainclock;
...
session_disconnect(struct app_conn_t *appconn,
  ...
  if (_options.macdown) {
    ...
    runscript(appconn, _options.macdown, 0, 0, 0);
  }
...
void session_param_defaults(struct session_params *params) {
  ...
  if (_options.defgonetimeout && !params->gonetimeout)
    params->gonetimeout = _options.defgonetimeout;
}
...
int cb_radius_auth_conf(struct radius_t *radius,
  ...
  if (!radius_getattr(pack, &sendattr, RADIUS_ATTR_VENDOR_SPECIFIC,
    ...
    if (radius_keydecode(radius, appconn->sendkey, RADIUS_ATTR_VLEN, &appconn->sendlen,
			 ...
			 radius->radserver[0].secret0, radius->radserver[0].secretlen0)) {
...
int cb_dhcp_request(struct dhcp_conn_t *conn, struct in_addr *addr,
  ...
  /* if bridgemode is on we have to filter out which ip's are allowed */
  if (_options.bridgemode && addr && addr->s_addr) {

    if (addr->s_addr == _options.uamlisten.s_addr) {
        syslog(LOG_DEBUG, "%s(%d): [BridgeMode] ignoring uamlisten ip request %s", __FUNCTION__, __LINE__,
	      inet_ntoa(*addr));
      return -1;
    }
    if (addr->s_addr == _options.ezmasterip.s_addr) {
        syslog(LOG_DEBUG, "%s(%d): [BridgeMode] ignoring ezmaster ip request %s", __FUNCTION__, __LINE__,
	      inet_ntoa(*addr));
        return -1;
    }
    if (addr->s_addr == _options.dhcpgwip.s_addr) {
        syslog(LOG_DEBUG, "%s(%d): [BridgeMode] ignoring dhcpgwip ip request %s", __FUNCTION__, __LINE__,
	      inet_ntoa(*addr));
        return -1;
    }
    if (addr->s_addr == _options.dhcprelayip.s_addr) {
        syslog(LOG_DEBUG, "%s(%d): [BridgeMode] ignoring dhcprelayip ip request %s", __FUNCTION__, __LINE__,
	      inet_ntoa(*addr));
        return -1;
    }
    ...
    if ((addr->s_addr & ipv4ll_mask.s_addr) == ipv4ll_ip.s_addr) {
	 ...
	 They must rather get a proper dynamic ip via dhcp */
      syslog(LOG_DEBUG, "%s(%d): [BridgeMode] IPv4LL/APIPA address requested, ignoring %s", __FUNCTION__, __LINE__,
	      inet_ntoa(*addr));
      return -1;
    }
  }
  ...
  if (appconn->uplink) {
    ...
    if ( ! conn->is_reserved) {
      ...
      if ((_options.macoklen || (_options.dynmacoklen)) &&
	  (appconn->dnprot == DNPROT_DHCP_NONE) &&
	  !maccmp(conn->hismac)) {
  ...
  if (_options.macup) {
    if (_options.debug)
      syslog(LOG_DEBUG, "%s(%d): Calling MAC up script: %s", __FUNCTION__, __LINE__, _options.macup);
    runscript(appconn, _options.macup, 0, 0, 0);
  }
  appconn->s_state.last_sync_time = mainclock_now();
...
int cb_dhcp_connect(struct dhcp_conn_t *conn) {
  ...
  if(appconn->s_state.sync_flag == 1)
  {
      appconn->s_state.last_sync_time = mainclock_now();
  }
  update_user_here(appconn, USER_HERE_UPDATE);
  return 0;
}
...
static void macallow_print(int fd, int is_json) {
  char line[512];
  int i;
  char mac[MACSTRLEN+1];
#ifdef ENABLE_JSON
  if(is_json)
  {
      ...
      // static
      safe_snprintf(line, sizeof(line), "\"static\":{");
      if (!safe_write(fd, line, strlen(line))) /* error */;

      safe_snprintf(line, sizeof(line), "\"count\":%d,", _options.macoklen);
      if (!safe_write(fd, line, strlen(line))) /* error */;

      safe_snprintf(line, sizeof(line), "\"limit\":%d,", MACOK_MAX);
      if (!safe_write(fd, line, strlen(line))) /* error */;

      safe_snprintf(line, sizeof(line), "\"mac\":[");
      if (!safe_write(fd, line, strlen(line))) /* error */;
      ...
      for (i=0; i < _options.macoklen; i++)
      {
          ...
          snprintf(mac, sizeof(mac), MAC_FMT, MAC_ARG(_options.macok[i]));
          safe_snprintf(line, sizeof(line), "\"%s\"", mac);
          if (!safe_write(fd, line, strlen(line))) /* error */;
      ...
      safe_snprintf(line, sizeof(line), "\"dynamic\":{");
      if (!safe_write(fd, line, strlen(line))) /* error */;

      safe_snprintf(line, sizeof(line), "\"count\":%d,", _options.dynmacoklen);
      if (!safe_write(fd, line, strlen(line))) /* error */;

      safe_snprintf(line, sizeof(line), "\"limit\":%d,", DYN_MACOK_MAX);
      if (!safe_write(fd, line, strlen(line))) /* error */;
      ...
      for (i=0; i < _options.dynmacoklen; i++)
      {
          ...
          snprintf(mac, sizeof(mac), MAC_FMT, MAC_ARG(_options.dynmacok[i]));
          safe_snprintf(line, sizeof(line), "\"%s\"", mac);
          if (!safe_write(fd, line, strlen(line))) /* error */;
      }
...
#endif
  {
      safe_snprintf(line, sizeof(line), "static macallow (%d/%d):\n", _options.macoklen, MACOK_MAX);
      if (!safe_write(fd, line, strlen(line))) /* error */;

      for (i=0; i < _options.macoklen; i++)
      {
          snprintf(mac, sizeof(mac), MAC_FMT, MAC_ARG(_options.macok[i]));
          safe_snprintf(line, sizeof(line), "%s\n", mac);

        if (!safe_write(fd, line, strlen(line))) /* error */;
      }

      safe_snprintf(line, sizeof(line), "dynamic macallow (%d/%d):\n", _options.dynmacoklen, DYN_MACOK_MAX);
      if (!safe_write(fd, line, strlen(line))) /* error */;
...
clear_appconn(struct app_conn_t *appconn) {
  appconn->s_params.bandwidthmaxup =
      ...
      appconn->s_params.gonetimeout = 0;
  appconn->s_params.idletimeout = 0;
  appconn->s_params.interim_interval = 0;
  appconn->s_params.sessionterminatetime = 0;
}

void kick_client(char *client_mac, int ssidid, int radio, int block_sec)
{
    char athif[16] = {0};
    char cmd[128] = {0};

    if(ssidid == 1)
        safe_snprintf(athif, sizeof(athif), "ath%d", radio);
    else
        safe_snprintf(athif, sizeof(athif), "ath%d%d", radio, ssidid-1);

    safe_snprintf(cmd, sizeof(cmd), "/usr/sbin/ban_kick_client.sh %s %s %d &",
            athif, client_mac, block_sec);
    system(cmd);
}
...
int terminate_appconn(struct app_conn_t *appconn, int terminate_cause) {
  ...
  if (appconn->s_state.authenticated == 1) {
    ...
    if (_options.condown && !(appconn->s_params.flags & NO_SCRIPT)) {
      ...
      runscript(appconn, _options.condown, 0, 0, 0);
    }

    acct_req(ACCT_USER, appconn, RADIUS_STATUS_TYPE_STOP);
    update_user_here(appconn, USER_HERE_LOGOUT);
    ...
    if(_options.ssidid)
    {
        int block_sec;
        int ssidid;
        char client_mac[24] = {0};

        log_dbg("[Portal] kick "MAC_FMT2" of %s",
                MAC_ARG(appconn->hismac), _options.dhcpif);

        ssidid = atoi(_options.ssidid);

        if(current_user_here(appconn->s_state.user_here) == 1)
        {
            block_sec = 1;
        }
        else
        {
            // ensure clients not re-association to here when sessiontimeout
            block_sec = 4;
        }

        snprintf(client_mac, sizeof(client_mac), MAC_FMT2, MAC_ARG(appconn->hismac));

        kick_client(client_mac, ssidid, 4, block_sec);
        kick_client(client_mac, ssidid, 1, block_sec);
        kick_client(client_mac, ssidid, 0, block_sec);
    }
...
int static uam_msg(struct redir_msg_t *msg) {
  ...
  mtype = msg->mtype;
  if(mtype == REDIR_FBWIFI)
  {
      if(!strcmp(msg->mdata.redir.username, "null"))
      {
          // logout
          is_fbwifi = 2;
          mtype = REDIR_LOGOUT;
      ...
      memcpy(&appconn->s_state.redir.username, &msg->mdata.redir.username, sizeof(msg->mdata.redir.username));
  }
  ...
  switch(mtype) {
    case REDIR_LOGIN:
      ...
      if(is_fbwifi == 1 && appconn->s_state.authenticated == 1)
      {
          syslog(LOG_INFO, "[FBWIFI] username=%s IP=%s alreay login, just call script to finish",
                  msg->mdata.redir.username, inet_ntoa(appconn->hisip));

          char cmd[256] = {0};
          snprintf(cmd, sizeof(cmd), "fbwifi-auth.sh %s "MAC_FMT" &",
                  _options.configname, MAC_ARG(dhcpconn->hismac));
          system(cmd);
      ...
      ret = upprot_getip(appconn, 0, 0);

      if(is_fbwifi == 1)
      {
          if (_options.debug)
              syslog(LOG_DEBUG, "[FBWIFI] Successful login with auth api. run fbwifi-auth.sh");
    ...
    case REDIR_LOGOUT:
      ...
      if (appconn->s_state.authenticated == 1) {
        ...
        {
            if (_options.debug)
                syslog(LOG_DEBUG, "[FBWIFI] Successful logout with auth api. sleep 2 secs to wait tcp finish");
...
int chilli_cmd(struct cmdsock_request *req, bstring s, int sock) {
  ...
  switch(req->type) {
    ...
    case CMDSOCK_REM_MACALLOW:
    case CMDSOCK_ADD_MACALLOW:
      {
          char do_add = (req->type == CMDSOCK_ADD_MACALLOW);
          // default action for dynamic macallow array
          int is_dynamic = 1;
          int action = do_add ? 1 : 0;
          int data_len = strlen(req->d.data);

          if (_options.debug)
              syslog(LOG_DEBUG, "%s(%d): req->d.data size[%d] value[%s]", __FUNCTION__, __LINE__,
                      (int)(strlen(req->d.data)), req->d.data);

          if(data_len > 0)
          {
              if(!strncmp(req->d.data, "static", data_len))
              {
                  is_dynamic = 0;
              }
              else if(!strncmp(req->d.data, "@static_file=", 13))
              {
                  is_dynamic = 0;
                  action = 3;
              }
              else if(!strncmp(req->d.data, "static_flush", data_len))
              {
                  is_dynamic = 0;
                  action = 2;
              }
              else if(!strncmp(req->d.data, "flush", data_len))
              {
                  action = 2;
              }
              else if(!strncmp(req->d.data, "@file=", 6))
              {
                  action = 3;
              }
          }
          ...
          // default add dynamic
          macallow_update(req, is_dynamic, action);
          break;
      }
      break;
    ...
    case CMDSOCK_LIST_MACALLOW:
      {
          int listfmt = (req->options & CMDSOCK_OPT_JSON) ?
              LIST_JSON_FMT : LIST_LONG_FMT;

#ifdef ENABLE_JSON
          macallow_print(sock, listfmt == LIST_JSON_FMT?1:0);
#else
          macallow_print(sock, 0);
#endif
      }
      break;
    case CMDSOCK_LIST:
    ...
    case CMDSOCK_DHCP_UPDATE:
      {
        struct dhcp_conn_t *dhcpconn = 0;

        if(!strncmp(req->d.data, "release_nologin", 15))
        {
          ...
          if (_options.debug)
            syslog(LOG_DEBUG, "[dhcp-update] %s(%d): %s",
                    __FUNCTION__, __LINE__,
                    req->d.data);
          if(dhcpconn)
          {
    ...
    case CMDSOCK_DHCP_CONNECT:
      {
          ...
          if (_options.debug)
              syslog(LOG_DEBUG, "[dhcp-connect] %s(%d): ip= %s",
                      __FUNCTION__, __LINE__,
                      inet_ntoa(req->ip));

          if (!dhcpconn) {
              log_dbg("[dhcp-connect] mac=%.2X%.2X%.2X%.2X%.2X%.2X",
                      req->mac[0],req->mac[1],req->mac[2],
                      req->mac[3],req->mac[4],req->mac[5]);

              if (dhcp_newconn(dhcp, &dhcpconn, req->mac)) {
                  log_err(0, "[dhcp-connect] error");
              }
          }
    ...
    case CMDSOCK_DHCP_DISCONNECT:
      {
          ...
          if (_options.debug)
              log_dbg("[dhcp-disconnect] mac=%.2X%.2X%.2X%.2X%.2X%.2X",
                      req->mac[0],req->mac[1],req->mac[2],
                      req->mac[3],req->mac[4],req->mac[5]);

          if (!dhcpconn) {
              log_dbg("[dhcp-disconnect] mac not found.");
          }
          else
          {
              struct app_conn_t * appconn = (struct app_conn_t*)dhcpconn->peer;
              if(appconn)
              {
                  update_user_here(appconn, USER_HERE_LEAVE);
                  if (!appconn->s_state.authenticated) {
                      if (_options.logintype)
                      {
                          if(atoi(_options.logintype) == RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_CLOUD_FACEBOOK_WIFI)
                          {
                              if (_options.debug)
                                  log_dbg("[FBWIFI] release mac="MAC_FMT,
                                          MAC_ARG(req->mac));
                              // release mac immediately if not auth
                              dhcp_release_mac(dhcp, req->mac, RADIUS_TERMINATE_CAUSE_LOST_CARRIER);
                          }
                      }
                  }
              }
    ...
    case CMDSOCK_DHCP_NEW:
      {
          ...
          if (_options.debug)
              syslog(LOG_DEBUG, "[dhcp-new] %s(%d): ip= %s",
                      __FUNCTION__, __LINE__,
                      inet_ntoa(req->ip));

          if (!dhcpconn) {
              log_dbg("[dhcp-new] mac=%.2X%.2X%.2X%.2X%.2X%.2X",
                      req->mac[0],req->mac[1],req->mac[2],
                      req->mac[3],req->mac[4],req->mac[5]);

              if (dhcp_newconn(dhcp, &dhcpconn, req->mac)) {
                  log_err(0, "[dhcp-new] error");
              }
          }

          dhcp_authconn(dhcp, &dhcpconn, req->mac);

          struct app_conn_t * appconn = (struct app_conn_t*)dhcpconn->peer;
          if(appconn)
          {
              log_dbg("[dhcp-new] update ip to %s", inet_ntoa(req->ip));
              dhcp_update_ip(dhcp, &dhcpconn, &req->ip);
              update_user_here(appconn, USER_HERE_ROAMING);
          }
      }
      break;
    ...
    case CMDSOCK_AUTHORIZE:
      if (dhcp) {
        ...
        if (appconn) {
          ...
          char *sessionid = req->d.sess.sessionid;
          ...
          if (sessionid[0])
          {
              if(strcmp(appconn->s_state.sessionid, sessionid) != 0)
              {
                  _set_sessionid(appconn, 1, sessionid);
              }
          }

          if(current_user_here(appconn->s_state.user_here) == 0)
          {
              // default no accounting. until login..
              appconn->s_params.flags |= NO_ACCOUNTING;
          }

          if(req->d.sess.state.start_time > 0)
          {
              if (_options.debug)
                  syslog(LOG_DEBUG, "%s(%d): update sessiontime %u", __FUNCTION__, __LINE__, (uint32_t) req->d.sess.state.start_time);
              appconn->s_state.start_time = mainclock_now() - req->d.sess.state.start_time;           //session time
          }

          if(req->d.sess.state.last_sent_time > 0)
          {
              if (_options.debug)
                  syslog(LOG_DEBUG, "%s(%d): update idletime %u", __FUNCTION__, __LINE__, (uint32_t) req->d.sess.state.last_sent_time);
              appconn->s_state.last_sent_time = mainclock_now() - req->d.sess.state.last_sent_time;   //idle time
          }

          if(req->d.sess.state.gone_time > 0)
          {
              if (_options.debug)
                  syslog(LOG_DEBUG, "%s(%d): update gonetime %u", __FUNCTION__, __LINE__, (uint32_t) req->d.sess.state.gone_time);
              appconn->s_state.gone_time = mainclock_now() - req->d.sess.state.gone_time;   //gone time
          }
...
int chilli_main(int argc, char **argv) {
  ...
  // if fclose(fp), chilli_binconfig will open file fail
  if (!_options.foreground) {
    ...
    if (dhcp_new(&dhcp,
                 ...
                 _options.bridgemode?_options.wanif:_options.dhcpif,
                 _options.dhcpusemac,
                 _options.dhcpmac, 1,
...
#ifdef HAVE_NETFILTER_COOVA
      if (_options.kname) {
        syslog(LOG_DEBUG, "[nfcoova] kmod_coova_clear %s(%d)", __FUNCTION__, __LINE__);
        kmod_coova_clear();
      }
#endif
    }
    ...
    while (keep_going) {
      ...
      if (do_interval) {
        ...
        if (_options.logintype)
        {
            if(atoi(_options.logintype) == RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_CLOUD_FACEBOOK_WIFI)
            {
                tun_runscript(tun, "/usr/sbin/fbwifi-update.sh", 0);
            }
        }

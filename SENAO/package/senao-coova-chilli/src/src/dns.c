...
rem_A_from_garden(uint8_t *p) {
  ...
  pt.mask.s_addr = 0xffffffff;
  pt.host = reqaddr;
  if (pass_through_rem(dhcp->pass_throughs, 
		       &dhcp->num_pass_throughs,
		       &pt
#ifdef HAVE_PATRICIA
		       , dhcp->ptree_dyn
...
#define return_error {                                                  \
    if (_options.debug) log_dbg("failed parsing DNS packet"); return -1; }
  ...
  if (q) {
    ...
    *left = len;
    ...
    if(!isReq && *qmatch == 1 && _options.uamblockdomains[0])
    {
        int idx;
        for (idx=0; idx < MAX_UAM_BLOCK_DOMAINS && _options.uamblockdomains[idx]; idx++) {
            {
                if (_options.debug)
                    log_dbg("checking x %s [%s]",
                            _options.uamblockdomains[idx], question);

                if(!strcmp(_options.uamblockdomains[idx], (char *) question)) {
                    if (_options.debug)
                        log_dbg("skip android check domain %s [%s]", _options.uamblockdomains[idx], question);
                    *qmatch = 2;
                    break;
                }
            }
  ...
  switch (type) {
      ...
      if (*qmatch > 0) {
        ...
        for (offset=0; offset < rdlen; offset += 4) {
            ...
            if (_options.debug)
            {
                struct in_addr reqaddr;
                memcpy(&reqaddr.s_addr, p_pkt+offset, 4);
                log_dbg("DNS matched %d Query %s = %s", *qmatch, name, inet_ntoa(reqaddr));
            }
#endif
            if(*qmatch == 1)
                add_A_to_garden(p_pkt+offset);
            else if (*qmatch == 2)
                rem_A_from_garden(p_pkt+offset);
        }
      }
      break;
    ...
    case 41:
      if (_options.debug)
        log_dbg("EDNS OPT pseudorecord");
      break;
    case 47:
      if (_options.debug)
        log_dbg("NSEC record"); break;
      break;
  }

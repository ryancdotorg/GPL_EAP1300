...
static int http_aaa_finish(proxy_request *req) {
  ...
  struct radius_server_t *radset, *radacctset;
  radset = &radius->radserver[0];
  radacctset = &radius->radserver[1];
    ...
    case RADIUS_CODE_ACCESS_REQUEST:
      {
        ...
        if (!radius_getattr(&req->radius_res, &ma, RADIUS_ATTR_MESSAGE_AUTHENTICATOR, 0,0,0)) {
          radius_hmac_md5(radius, &req->radius_res, radset->secret0, radset->secretlen0, ma->v.t);
        }
...
static void process_radius(struct radius_t *radius, struct radius_packet_t *pack, struct sockaddr_in *peer) {
  ...
  tmp = bfromcstralloc(1024,"");
  tmp2 = bfromcstralloc(1024,"");
...
#if 0 // Yolin: here to support diffreent http aaa accounting server.
  bassignformat(req->url, "%s%c",
		_options.uamaaaaccturl,
		strchr(_options.uamaaaaccturl, '?') > 0 ? '&' : '?');
#endif
  ...
  if (!error) {
    ...
    if (_options.networkid) {
      bcatcstr(req->url, "&networkId=");
      bassignblk(tmp, _options.networkid, strlen(_options.networkid));
      redir_urlencode(tmp, tmp2);
      bconcat(req->url, tmp2);
    }

    if (_options.ssidid) {
      bcatcstr(req->url, "&ssidId=");
      bassignblk(tmp, _options.ssidid, strlen(_options.ssidid));
      redir_urlencode(tmp, tmp2);
      bconcat(req->url, tmp2);
    }

    if (_options.ssidprofileid) {
      bcatcstr(req->url, "&ssidProfileId=");
      bassignblk(tmp, _options.ssidprofileid, strlen(_options.ssidprofileid));
      redir_urlencode(tmp, tmp2);
      bconcat(req->url, tmp2);
    }

    if (_options.modelname) {
      bcatcstr(req->url, "&model=");
      bassignblk(tmp, _options.modelname, strlen(_options.modelname));
      redir_urlencode(tmp, tmp2);
      bconcat(req->url, tmp2);
    }

    if (_options.fwversion) {
      bcatcstr(req->url, "&version=");
      bassignblk(tmp, _options.fwversion, strlen(_options.fwversion));
      redir_urlencode(tmp, tmp2);
      bconcat(req->url, tmp2);
    }

    if (_options.apmac) {
      bcatcstr(req->url, "&apmac=");
      bassignblk(tmp, _options.apmac, strlen(_options.apmac));
      redir_urlencode(tmp, tmp2);
      bconcat(req->url, tmp2);
    }
    ...
    if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
                RADIUS_VENDOR_ENGENIUS,
                RADIUS_ATTR_ENGENIUS_LOGIN_TYPE, 0)) {
        uint32_t login_type = ntohl(attr->v.i);
        FILE *fp;
        char filename[64] = {0};
        char token[REDIR_TOKENSIZE] = {0};

        log_dbg("login_type [%d]", login_type);
        switch (login_type) {
#if 1 // for oauth_AAA use (login_type 105)
            case RADIUS_VALUE_ENGENIUS_LOGIN_TYPE_CLOUD_SOCIAL_LOGIN:
                sprintf(filename, "/var/run/chilli_token_%s_%s", _options.configname, mac);
                /* log_dbg("[Social-Login] main-proxy filename [%s]", filename); */
                fp = fopen(filename, "r");
                if(fp)
                {
                    ...
                    /* log_dbg("[Social-Login] token:[%s]", token); */
                    if(token[0])
                    {
                        bcatcstr(req->url, "&token=");
                        bassignblk(tmp, token, strlen(token));
                        redir_urlencode(tmp, tmp2);
                        bconcat(req->url, tmp2);
                    }
                    fclose(fp);
                ...
                if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
                            RADIUS_VENDOR_ENGENIUS,
                            RADIUS_ATTR_ENGENIUS_OAUTH_PROVIDER, 0)) {
                    bcatcstr(req->url, "&provider=");
                    bassignblk(tmp, attr->v.t, attr->l-2);
                    redir_urlencode(tmp, tmp2);
                    bconcat(req->url, tmp2);
                }
...
#endif
            default:
                log_dbg("Unknown login_type %d", login_type);
                break ;
        }
    }
...
int main(int argc, char **argv) {
  ...
  if(_options.bridgemode)
      net_getmac(_options.wanif, nas_hwaddr);
  else

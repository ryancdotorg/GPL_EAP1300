...
// copy from redir.c
/* Decode a string using the base64 algorithm into an input_str. 
 * Returns 0 if decoding was successfull, returns 1 if the input_str is too big
 * for the EAP msg. Return 2 for other errors */
static int base64decoder(char *input_str, char *output_str, int len)
{
  ...
  int  x_in = 0; 
  int  x_out = 0;
  unsigned int  len_in = strlen(input_str);
  ...
  while (input_str[len_in-1] == '='){
    input_str[len_in-1] = 0;
    len_in--;
  }
  ...
  while (x_in < len_in) {
    ...
    output_str[x_out++] = (in64[0] << 2) | (in64[1] >> 4);
    if (x_out < len_out) 
      output_str[x_out++] = (in64[1] << 4) | (in64[2] >> 2);
    if (x_out < len_out) 
      output_str[x_out++] = (in64[2] << 6) | (in64[3]);
  }
...
void radius_addcalledstation(struct radius_t *this,
  ...
  if(_options.radiuscalledstaid) {
      strncpy(base64_str, _options.radiuscalledstaid, strlen(_options.radiuscalledstaid));
      if(base64decoder(base64_str, decode_str, sizeof(decode_str)) == 0)
      {
          mac = (uint8_t *) decode_str;
      }
  }
...
int radius_printqueue(int fd, struct radius_t *this) {
  ...
  for(i=0; i<2; ++i)
  {
      safe_snprintf(line, sizeof(line), "s0:%16s:%-6d secret: %s\n",
              inet_ntoa(this->radserver[i].hisaddr0), this->radserver[i].port0,
              this->radserver[i].secret0);
      safe_write(fd, line, strlen(line));

      safe_snprintf(line, sizeof(line), "s1:%16s:%-6d secret: %s\n",
              inet_ntoa(this->radserver[i].hisaddr1), this->radserver[i].port1,
              this->radserver[i].secret1);
      safe_write(fd, line, strlen(line));
  }
  ...
  for(n=0; n < mx; n++) {
    if (this->queue[n].state) {
      safe_snprintf(line, sizeof(line),
              "n=%-3d id=%-3d state=%-3d next=%-3d prev=%-3d sec=%-10d.%-8d retrans=%d lastsent=%d\n",
              n,
...
int radius_acctreq_authenticator(struct radius_t *this,
				 struct radius_packet_t *pack, char *secret, size_t secretlen) {
...
int radius_queue_in(struct radius_t *this,
  ...
  if (pack->code == RADIUS_CODE_ACCOUNTING_REQUEST || pack->code == RADIUS_CODE_ACCOUNTING_RESPONSE)
  {
      radset = &this->radserver[1];
  }
  else
  {
      radset = &this->radserver[0];
  }
  ...
  if (!radius_getattr(pack, &ma,
      ...
      if(!this->lastreply)
      {
          radius_hmac_md5(this, pack,
                  radset->secret0, radset->secretlen0,
                  ma->v.t);
      }
      else
      {
          radius_hmac_md5(this, pack,
                  radset->secret1, radset->secretlen1,
                  ma->v.t);
      }
  }
  ...
  if (pack->code == RADIUS_CODE_ACCOUNTING_REQUEST) {
      if(!this->lastreply)
      {
          radius_acctreq_authenticator(this, pack, radset->secret0, radset->secretlen0);
      }
      else
      {
          radius_acctreq_authenticator(this, pack, radset->secret1, radset->secretlen1);
      }
  }
  ...
  if (this->first != -1 &&
    ...
    if (this->queue[this->first].retrans < _options.radiusretry) {
      ...
      if (RADIUS_QUEUE_HASPKT(this->queue[this->first].p)) {
          ...
          if (RADIUS_QUEUE_PKT(this->queue[this->first].p,code) == RADIUS_CODE_ACCOUNTING_REQUEST)
          {
              radset = &this->radserver[1];
          }
          else
          ...
          if (this->queue[this->first].retrans == (_options.radiusretrysec - 1)) {
              ...
              if (this->queue[this->first].lastsent) {
                  addr.sin_addr = radset->hisaddr0;
                  addr.sin_port = htons(radset->port0);
                  this->queue[this->first].lastsent = 0;
              }
              else {
                  addr.sin_addr = radset->hisaddr1;
                  addr.sin_port = htons(radset->port1);
        ...
        if (RADIUS_QUEUE_PKT(this->queue[this->first].p,code)
          ...
          addr.sin_port = htons(radacctset->port);
        } else {
...
radius_addattr(struct radius_t *this, struct radius_packet_t *pack,
  ...
  struct radius_server_t *radset;
  // only auth need RADIUS_ATTR_USER_PASSWORD attribute, just use radset[auth]
  radset = &this->radserver[0];
...
radius_free(struct radius_t *this) {
  ...
  if (this->urandom_fp) {
    ...
    if (fclose(this->urandom_fp)) {
      log_err(errno, "radius: %s: fclose(urandom_fp=%d) failed!",
             strerror(errno), fd);
    }
  }
  fd = this->fd;
  if (close(this->fd)) {
    log_err(errno, "radius: %s: close(fd=%d) failed!", strerror(errno), fd);
  }
#ifdef ENABLE_RADPROXY
  fd = this->proxyfd;
...
void radius_set(struct radius_t *this, unsigned char *hwaddr, int debug) {
  ...
  struct radius_server_t *radset, *radaccset;
  radset = &this->radserver[0];
  radaccset = &this->radserver[1];


  /* Remote radius server parameters */
  if (_options.radsec) {
    inet_aton("127.0.0.1", &radset->hisaddr0);
    radset->hisaddr1.s_addr = radset->hisaddr0.s_addr;
    radset->secretlen0 = 6;
    radset->secretlen1 = 6;
    radaccset->hisaddr0.s_addr = radset->hisaddr0.s_addr;
    radaccset->hisaddr1.s_addr = radset->hisaddr0.s_addr;
    radaccset->secretlen0 = 6;
    radaccset->secretlen1 = 6;
    strlcpy(radset->secret0, "radsec", sizeof(radset->secret0));
    strlcpy(radset->secret1, "radsec", sizeof(radset->secret1));
  } else {
    radset->hisaddr0.s_addr = _options.radiusserver1.s_addr;
    radset->hisaddr1.s_addr = _options.radiusserver2.s_addr;

    radaccset->hisaddr0.s_addr = _options.radiusacctserver1.s_addr;
    radaccset->hisaddr1.s_addr = _options.radiusacctserver2.s_addr;

    if ((radset->secretlen0 = strlen(_options.radiussecret)) > RADIUS_SECRETSIZE) {
      log_err(0, "Radius secret too long. Truncating to %d characters",
              RADIUS_SECRETSIZE);
      radset->secretlen0 = RADIUS_SECRETSIZE;
    }
    if ((radset->secretlen1 = strlen(_options.radiussecret2)) > RADIUS_SECRETSIZE) {
      log_err(0, "Radius secret too long. Truncating to %d characters",
              RADIUS_SECRETSIZE);
      radset->secretlen1 = RADIUS_SECRETSIZE;
    }

    memcpy(radset->secret0, _options.radiussecret, radset->secretlen0);
    memcpy(radset->secret1, _options.radiussecret2, radset->secretlen1);

    if ((radaccset->secretlen0 = strlen(_options.radiusacctsecret)) > RADIUS_SECRETSIZE) {
      log_err(0, "Radius secret too long. Truncating to %d characters",
              RADIUS_SECRETSIZE);
      radaccset->secretlen0 = RADIUS_SECRETSIZE;
    }
    if ((radaccset->secretlen1 = strlen(_options.radiusacctsecret2)) > RADIUS_SECRETSIZE) {
      log_err(0, "Radius secret too long. Truncating to %d characters",
              RADIUS_SECRETSIZE);
      radaccset->secretlen1 = RADIUS_SECRETSIZE;
    }

    memcpy(radaccset->secret0, _options.radiusacctsecret, radaccset->secretlen0);
    memcpy(radaccset->secret1, _options.radiusacctsecret2, radaccset->secretlen1);

  }

  if (_options.radiusauthport) {
    radset->port0 = _options.radiusauthport;
  }
  else {
    radset->port0 = RADIUS_AUTHPORT;
  }

  if (_options.radiusauthport2) {
    radset->port1 = _options.radiusauthport2;
  }
  else {
    radset->port1 = RADIUS_AUTHPORT;
  }

  if (_options.radiusacctport) {
    radaccset->port0 = _options.radiusacctport;
  }
  else {
    radaccset->port0 = RADIUS_ACCTPORT;
  }

  if (_options.radiusacctport2) {
    radaccset->port1 = _options.radiusacctport2;
  }
  else {
    radaccset->port1 = RADIUS_ACCTPORT;
  }
	       ...
	       void *cbp)
{
  ...
  log_dbg("[Radius] radius_req pack->code=[%d]", pack->code);
  if (pack->code == RADIUS_CODE_ACCOUNTING_REQUEST)
  {
      log_dbg("[Radius] push to accounting queue");
      radset = &this->radserver[1];
  }
  else
  {
      log_dbg("[Radius] push to radius queue");
      radset = &this->radserver[0];
  }
...
int radius_coaresp(struct radius_t *this,
  ...
  log_dbg("[Radius] radius_coaresp pack->code=[%d]", pack->code);
  if (pack->code == RADIUS_CODE_ACCOUNTING_REQUEST || pack->code == RADIUS_CODE_ACCOUNTING_RESPONSE)
  {
      radset = &this->radserver[1];
  }
  ...
  if (!radius_getattr(pack, &ma, RADIUS_ATTR_MESSAGE_AUTHENTICATOR, 0,0,0)) {
        if(!this->lastreply)
        {
            radius_hmac_md5(this, pack, radset->secret0, radset->secretlen0, ma->v.t);
        }
        else
        {
            radius_hmac_md5(this, pack, radset->secret1, radset->secretlen1, ma->v.t);
        }
  }
...
#if 1// engenius default radius attribute
      if (_options.logintype)
      {
          //log_dbg("[radius] logintype [%s]\n", _options.logintype);
          radius_addattr(this, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
                  RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_LOGIN_TYPE,
                  atoi(_options.logintype), 0, 0);
      }

      if (_options.networkid)
      {
          //log_dbg("[radius] networkid [%s]\n", _options.networkid);
          radius_addattr(this, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
                  RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_NETWORK_ID,
                  0, (uint8_t*)_options.networkid,  strlen(_options.networkid));
      }

      if (_options.ssidprofileid)
      {
          //log_dbg("[radius] ssidprofileid [%s]\n", _options.ssidprofileid);
          radius_addattr(this, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
                  RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_SSID_PROFILE_ID,
                  0, (uint8_t*)_options.ssidprofileid, strlen(_options.ssidprofileid));
      }

      if (_options.ssidid)
      {
          //log_dbg("[radius] ssidid [%s]\n", _options.ssidid);
          radius_addattr(this, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
                  RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_CONFIG_ID,
                  atoi(_options.ssidid), 0, 0);
      }

      if (_options.modelname)
      {
          //log_dbg("[radius] modelname [%s]\n", _options.modelname);
          radius_addattr(this, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
                  RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_DEVICE_MODEL_NAME,
                  0, (uint8_t*)_options.modelname, strlen(_options.modelname));
      }

      if (_options.fwversion)
      {
          //log_dbg("[radius] fwversion [%s]\n", _options.fwversion);
          radius_addattr(this, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
                  RADIUS_VENDOR_ENGENIUS, RADIUS_ATTR_ENGENIUS_DEVICE_FIRMWARE_VERSION,
                  0, (uint8_t*)_options.fwversion, strlen(_options.fwversion));
      }
#endif
		 ...
		 struct radius_packet_t *pack_req)
{
  ...
  if(!this->lastreply)
  {
      MD5Update(&context, (uint8_t *)radset->secret0, radset->secretlen0);
  }
  else
  {
      MD5Update(&context, (uint8_t *)radset->secret1, radset->secretlen1);
  }
  ...
  switch (pack.code) {
    ...
    case RADIUS_CODE_COA_REQUEST:
      if (!this->coanocheck) {
        ...
        if ((addr.sin_addr.s_addr != radset->hisaddr0.s_addr) &&
            (addr.sin_addr.s_addr != radset->hisaddr1.s_addr)) {
    ...
    default:
      ...
      if (((addr.sin_addr.s_addr != radset->hisaddr0.s_addr) || (addr.sin_port != htons(radset->port0))) &&
          ((addr.sin_addr.s_addr != radset->hisaddr1.s_addr) || (addr.sin_port != htons(radset->port1))) ) {
      ...
      if ((addr.sin_port != htons(radset->port0)) &&
          (addr.sin_port != htons(radset->port1))) {
        log_warn(0, "Received radius packet from wrong port %d!",
	       ntohs(addr.sin_port));
        return -1;

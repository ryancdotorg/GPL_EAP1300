...
#define RADIUS_PROXYVSA 256
  ...
  char provider[REDIR_PROVIDER_LEN];
#endif
#if 1
  char redirurl[REDIR_USERURLSIZE];
#endif
} __attribute__((packed));
...
struct session_state {
  ...
  time_t acct_start_time;
  time_t interim_time;

#if 1
  int user_here;         /* is User connect to this ap ? */
  int sync_flag;         /* Last sync flag */
  time_t last_sync_time; /* Last guest_syncd sync time */
  time_t gone_time; /* Last guest_syncd do chilli_query update gonetimeout */
#endif

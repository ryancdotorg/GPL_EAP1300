...
SSL *SSL_new(sslKeys_t *keys)
{
	...
	/* mook 2009-0407: memory region should be initialized */
...
int SSL_dataBuffered(SSL *ssl)
{
	int bytes = 0;

	if (ssl) {
		/* 2010-0105 Mook: packets are buffered in input buffer */
		if (ssl->inbuf.buf)
			bytes = ssl->inbuf.end - ssl->inbuf.start;

		/* Packets are buffered in input socket */
		if (bytes <= 0)
			bytes = ssl->insock.end - ssl->insock.start;
	}
	return bytes;
}
...
//printf("%s: bytes = %d insock.start=%p insock.end=%p buf=%p\n", __func__, bytes, ssl->insock.start, ssl->insock.end, ssl->insock.buf);
	}
/*
...
decodeMore:
	...
	rc = matrixSslDecode(ssl->ssl, &ssl->insock, &ssl->inbuf, &error, &alertLevel, 
		&alertDescription);
//printf("decode %d, insock.start=%p insock.end=%p\n", rc, ssl->insock.start, ssl->insock.end);
	switch (rc) {
/*

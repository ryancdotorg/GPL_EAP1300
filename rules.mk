...
### SENAO ###
-include $(TOPDIR)/SENAO/configs/sysProductInfo.mk
...
comma:=,
merge=$(subst $(space),,$(1))
# TODO: not support mkhash, use md5s workaround.
#confvar=$(shell echo '$(foreach v,$(1),$(v)=$(subst ','\'',$($(v))))' | $(STAGING_DIR_HOST)/bin/mkhash md5)
confvar=$(shell echo '$(foreach v,$(1),$(v)=$(subst ','\'',$($(v))))' | md5sum | cut -d ' ' -f 1)
...
ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
  ...
  # arm-openwrt-linux-uclibcgnueabi-gcc with gcc-4.6 and gcc-4.8 version, use TOOLCHAIN_SUFFIX to sepreat it
ifneq ($(GCCV),4.6-linaro)
  TOOLCHAIN_SUFFIX=_gcc-$(GCCV)
endif
  TARGET_DIR_NAME:=target-$(GNU_TARGET_NAME)$(TOOLCHAIN_SUFFIX)$(if $(BUILD_SUFFIX),_$(BUILD_SUFFIX))
  TOOLCHAIN_DIR_NAME:=toolchain-$(GNU_TARGET_NAME)$(TOOLCHAIN_SUFFIX)
  export WRAPPER_INCLUDEDIR:=-I$(TOPDIR)/staging_dir/$(TARGET_DIR_NAME)/usr/include/uClibc++/
endif
...
ifeq ($(CONFIG_TARGET_NAME), "arm-linux-gnueabihf")
TARGET_CFLAGS+= $(if $(CONFIG_GCC_VERSION_4_4)$(CONFIG_GCC_VERSION_4_5),,-Wno-error=unused-but-set-variable) -Wno-error=unused-result
  ifeq ($(CONFIG_GCC_VERSION_8),y)
    # package: netifd-2015-12-16
    TARGET_CFLAGS+= -Wno-format-truncation
    # package: binutils-2.24 json-c-0.11
    TARGET_CFLAGS+= -Wno-error=implicit-fallthrough
    # package: binutils-2.24
    TARGET_CFLAGS+= -Wno-error=cast-function-type -Wno-error=format-overflow -Wno-error=shift-negative-value -Wno-error=int-in-bool-context
    # package: binutils-2.24
    TARGET_CFLAGS+= -Wno-error=tautological-compare -Wno-error=unused-const-variable -Wno-error=pointer-compare
    # package: elfutils-0.161
    TARGET_CFLAGS+= -Wno-error=stringop-overflow -Wno-error=nonnull-compare
    # package: iptables-1.4.21
    TARGET_LDFLAGS += -lm
  endif
else
...
SN_CONFIGS_DIR=$(TOPDIR)/SENAO/configs

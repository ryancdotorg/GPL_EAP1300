...
use Term::ANSIColor;
sub sn_chk_pkg_ver(@) {
	while (my @list=split('@', my $pkg=shift)) {
		my $file;
		$file = glob("tmp/info/.packageinfo-*_$list[0]");
		if (-e $file) {
			chk_pkg_ver($file, $list[1]);
		}
		else {
			print "Package ", color("yellow"), "$list[0]", color("reset"), ":\n";
			print "\tNot found!\n\n";
			die color("red"), "Please check the dependent package", color("reset"), "\n";
		}
	}
}

sub chk_pkg_ver(@) {
	parse_package_metadata(@_[0]) or exit 1;
	foreach my $name (sort {uc($a) cmp uc($b)} keys %package) {
		my $pkg = $package{$name};
		if ($pkg->{name} && $pkg->{version}) {
			#print "$pkg->{name}: ";
			#print "$pkg->{version}\n";
			my $current_ver = version_to_num($pkg->{version});
			my $required_ver = version_to_num(@_[1]);

			print "Package ", color("yellow"), "$pkg->{name}", color("reset"), ":\n";
			print "\tCurrent version is: $pkg->{version}\n";
			print "\tRequired version is: @_[1]\n\n";
...
sub parse_command() {
	...
	for ($cmd) {
		...
		/^package_version$/ and return sn_chk_pkg_ver(@ARGV);
	}
	print <<EOF
Available Commands:
	...
	$0 package_version [file]		SENAO package version information

...
qwrap_parse_ap() {
	...
	config_get disabled "$device" disabled 0
	config_get qwrap_enable "$device" qwrap_enable 0
	config_get opmode "$device" opmode ap
	...
	for vif in $vifs; do
		...
		[ "$mode_display" == "$opmode" ] || [ $mgmt_vif -eq 1 ] || continue
		...
		config_get extender_device "$vif" extender_device $qwrap_device
                [ -z "$extender_device" ] || {
...
qwrap_parse_bridge() {
    local bridge_mac=$(cat /sys/class/net/br-lan/address)

    [ -z "$bridge_mac" ] && {
	bridge_mac=$(ifconfig | grep br-lan | awk '{print $5}')
    }

    wrapd -S -g $wrapd_global_ctrl_iface -A $bridge_mac
}
...
qwrap_setup() {
    ...
    qwrap_parse_bridge
	config_foreach qwrap_restart_wrap_if wifi-device
	return 1
}

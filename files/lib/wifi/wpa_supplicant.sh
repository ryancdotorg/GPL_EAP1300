...
wpa_supplicant_setup_vif() {
	...
	local options="$3"
	local freq="" crypto=""
	local sae owe suite_b sae_groups sae_password
	...
	then
		...
		[ -n "$eap_type" ] && eap_type="eap=$(echo $eap_type | tr 'a-z' 'A-Z')"
		...
		case "$enc" in
			...
			*wep*)
				...
				wep_tx_keyidx="wep_tx_keyidx=$((key_id - 1))"
	...
	config_get ssid "$vif" ssid
        config_get preferbssid_enable "$vif" preferbssid_enable
        
	[ "$preferbssid_enable" = "1" ] && {	#senao fix if preferbssid disabled but bssid still has old value, it makes wpa_supplicant confing error

...
run_ramfs() { # <command> [...]
	install_bin /bin/busybox /bin/ash /bin/sh /bin/mount /bin/umount	\
		...
		/bin/cut /usr/bin/printf /bin/sync /usr/bin/tail
...
kill_remaining() { # [ <signal> ]
	...
	for stat in /proc/[0-9]*/stat; do
		...
		case "$name" in
			...
			*procd*|*ash*|*init*|*watchdog*|*ssh*|*dropbear*|*telnet*|*login*|*hostapd*|*wpa_supplicant*|*tee*|*rmgmt*|*sh*|*cli*|*snmpd*|*logger*|*wtp*|*lldp*) : ;;
...
do_upgrade() {
	...
	if [ "$REBOOT" == "1" ];then
		...
		echo "Does not automatically reboot.....You should reboot by yourself......"
	fi
	}
}

...
struct ieee80211_mktk_ie_data {
        ...
        u_int8_t pad1;
        u_int8_t namelen;
        u_int8_t radioname[15];
        u_int8_t pad2[5];
} __packed;

struct ieee80211_mktk_ie {
        u_int8_t        id;             /* IEEE80211_ELEMID_VENDOR */
        u_int8_t        len;
        u_int8_t        oui[3];
        u_int8_t        type;
        u_int16_t       version;
        struct ieee80211_mktk_ie_data iedata;
} __packed;

struct ieee80211_mktk_ie_legacy {
        u_int8_t        id;
        u_int8_t        len;
        struct ieee80211_mktk_ie_data iedata;
} __packed;
...
#if SENAO_SUPPORT_WDS_AUTO_RECONNECT
#define QCA_OUI_SN_WDS_AP               0x04
#endif
...
#if MESH_VENDOR_OUI
#define ENGENIUS_OUI                           MESH_VENDOR_OUI /* Vendor OUI */
#else
#define ENGENIUS_OUI                           0x6f0200        /* EnGenius OUI */
#endif
#define ENGENIUS_MESH_NAWDS_TYPE               0x53            /* 802.11s */                           
#define ENGENIUS_MESH_NAWDS_VERSION            1               
#define ENGENIUS_NAWDS_VERIFY_KEYLEN           5
#if SENAO_SUPPORT_MESH_EASYSETUP
#define ENGENIUS_MESH_EASYSETUP_TYPE           0x54 //TODO: need make sure
#define ENGENIUS_MESH_PASSWORD_LEN             32 //TODO: need to make sure len for AES encrypt
#endif

#if SENAO_SUPPORT_WLANCONFIG_CHECK_SENAO_OUI
#ifndef ENGENIUS_OUI
#define ENGENIUS_OUI                           0x6f0200
#endif
#define ENGENIUS_OUI_TYPE                      0x53
#endif

#define ENWDSINFO_MESH_ID_SIZE        4
#define ENWDSINFO_ADD_INFO_SIZE       7   // extra information size

#define ENWDSINFO_ORIG_SIZE           13  // enWdsInfo_oui[3] ... enWdsInfo_caps
#define ENWDSINFO_WITH_ID_SIZE        ((ENWDSINFO_ORIG_SIZE) + (ENWDSINFO_MESH_ID_SIZE))     // 17
#define ENWDSINFO_WITH_ES_EXT_SIZE    ((ENWDSINFO_WITH_ID_SIZE) + (ENWDSINFO_ADD_INFO_SIZE)) // 24

#define ESWDSINFO_ORIG_SIZE           137 // esWdsInfo_oui[3] ... esWdsInfo_caps
/*
 * EnGenius Mesh WDS information element.
 */
struct ieee80211_ie_enWdsInfo {
       u_int8_t    enWdsInfo_id;           /* IEEE80211_ELEMID_VENDOR */
       u_int8_t    enWdsInfo_len;          /* length in bytes */
       u_int8_t    enWdsInfo_oui[3];       /* 0x00, 0x02, 0x6f */
       u_int8_t    enWdsInfo_type;
       u_int8_t    enWdsInfo_version;
       u_int8_t    enWdsInfo_rssi;
       u_int8_t    enWdsInfo_security;
       u_int8_t    enWdsInfo_secure_verify[ENGENIUS_NAWDS_VERIFY_KEYLEN];
       u_int8_t    enWdsInfo_caps;
#if SENAO_SUPPORT_MESH_ID
       u_int32_t   enWdsInfo_meshid;       /* a number for mesh network ID */
#endif
#if SENAO_SUPPORT_MESH_EASYSETUP
       // mesh easy-setup extra information
       u_int8_t    enWdsInfo_es_cfg;      /* unconfigured:0, configured:1, none:2 */
       u_int32_t   enWdsInfo_es_sr_num;   /* SENAO serius number */
       u_int8_t    enWdsInfo_es_dev_type; /* device type: AP/IP-CAM/others */
       u_int8_t    enWdsInfo_es_domain;   /* regular domain */
#endif
} __packed;

#if SENAO_SUPPORT_MESH_EASYSETUP
#define ENGENIUS_NAWDS_EASYSETUP_KEYLEN  128 //ENGENIUS_NAWDS_EASYSETUP_PWD_LEN+44 //72//64
enum { 
    ENGENIUS_EASYSETUP_ENCRYPT_NONE = 0,
    ENGENIUS_EASYSETUP_ENCRYPT_AES,
	ENGENIUS_EASYSETUP_ENCRYPT_WEP,
};

/*
 * EnGenius Mesh easy setup information element.
 */
struct ieee80211_ie_esWdsInfo {
       u_int8_t    esWdsInfo_id;           /* IEEE80211_ELEMID_VENDOR */
       u_int8_t    esWdsInfo_len;         /* length in bytes */
       u_int8_t    esWdsInfo_oui[3];       /* 0x00, 0x02, 0x6f */
       u_int8_t    esWdsInfo_type;
       u_int8_t    esWdsInfo_version;
       u_int8_t    esWdsInfo_security_type;
       u_int8_t    esWdsInfo_wep_keyid; // key id : 1 ~ 4
       u_int8_t    esWdsInfo_wep_keylen; //5,13 bytes for ascii mode,and 10 ,26 bytes for hex mode
       u_int8_t    esWdsInfo_auth;       //auth
//     u_int8_t    esWdsInfo_wep_keytype; //is it needed ?
       u_int8_t    esWdsInfo_security_key[ENGENIUS_NAWDS_EASYSETUP_KEYLEN];
} __packed;

/* save easy-setup additional information from scan_entry */
struct se_data_enWdsInfo {
       u_int8_t    se_oui[3];
       u_int8_t    se_es_cfg;
       u_int32_t   se_es_sr_num;
       u_int8_t    se_es_dev_type;
       u_int8_t    se_es_domain;
} __packed;
#endif
...
/* TDMA information vendor specific IE */
#if SENAO_SUPPORT_TDMA
#ifndef TDMA_VENDOR_OUI
#ifndef ENGENIUS_OUI
#define ENGENIUS_OUI                           0x6f0200
#endif
#endif
#define ENGENIUS_OUI_TDMA_TYPE           0x60
/* vendor specific IEs for TDMA information */
#define ENGENIUS_OUI_TDMA_SUBTYPE        0x00
#define ENGENIUS_OUI_TDMA_VERSION        0x00
#endif
...
#if SENAO_SUPPORT_NMS /* SENAO_FEATURE_ADD, 012-sn-support-nms */
        u_int8_t isi_dorado_flags;  /* dorado capabilities */
        u_int32_t isi_ip_saddr;     /* ip addr*/
        u_int32_t isi_uptime;       /* ip addr*/
        u_int64_t isi_txbytes;
        u_int64_t isi_rxbytes;
        u_int32_t isi_txpackets;
        u_int32_t isi_rxpackets;
#endif /* SENAO_SUPPORT_NMS */
        /* XXX frag state? */

...
#ifdef CONFIG_UBOOT_RECOVERY_MODE /* idleman */
/* performance ARP request if needed */
extern int NetCheckARPTable(uchar *ether, struct in_addr dest, int dport, int sport, int len);
/* transmit TCP packet, assume already has this ARP entry */
extern int NetSendTCPPacket(uchar *ether, struct in_addr dest, int dport, int sport, int len);
#endif  /* #ifdef CONFIG_UBOOT_RECOVERY_MODE */

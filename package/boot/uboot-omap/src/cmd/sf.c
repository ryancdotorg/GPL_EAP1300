...
static int do_spi_flash_erase(int argc, char * const argv[])
{
	...
	ret = sf_parse_len_arg(argv[2], &size);
        ...
        printf("do_spi_flash_erase -> size =%d \n", size);
        
        if(size % flash->erase_size)
        {
            printf(" erase size %d ", size);
            
            size = size + flash->erase_size - ( size % flash->erase_size); /* Correction erase size  */
            
            printf(" Correction to %d \n", size);
        }       
        
	if (ret != 1)
		return -1;
#ifdef MACRONIX_4K_SECTOR
	if(size & 0xFFF)
	{
		size = ((size + 0x1000) & 0xFFFFF000);
	}
#endif  /* #ifdef MACRONIX_4K_SECTOR */
	/* Consistency checking */
...
void do_spi_fw_probe(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    do_spi_flash_probe(argc, argv);
}

void do_spi_fw_erase(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    do_spi_flash_erase(argc, argv);
}

void do_spi_fw_upgrade(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    do_spi_flash_read_write(argc, argv);
}

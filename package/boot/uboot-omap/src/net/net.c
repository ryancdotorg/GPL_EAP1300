...
#if defined(CONFIG_UBOOT_RECOVERY_MODE_LED_BLINK) && defined(CONFIG_UBOOT_CUSTOMIZATION)
#include "platform/board_customization.h"
#endif  /* #if defined(CONFIG_UBOOT_RECOVERY_MODE_LED_BLINK) && defined(CONFIG_UBOOT_CUSTOMIZATION) */
#include "tcp.h"
#include "httpd.h"
...
#define RECOVERY_MODE_BLINKING_ON_INTERVEL  1
#define RECOVERY_MODE_BLINKING_OFF_INTERVEL 2

static thand_f *recovery_mode_led_blink_timerHandler;   /* Current timeout handler */
static ulong recovery_mode_led_blink_timeStart;         /* Time base value */
static ulong recovery_mode_led_blink_timeDelta;         /* Current timeout value */
void RecoveryModeLedOn(void);
void RecoveryModeLedOff(void);

void RecoveryModeLedOn(void)
{
	recovery_mode_start_led_on();
	recovery_mode_led_blink_timerHandler = (thand_f *)RecoveryModeLedOff;
	recovery_mode_led_blink_timeStart = get_timer(0);
	recovery_mode_led_blink_timeDelta = RECOVERY_MODE_BLINKING_ON_INTERVEL * CONFIG_SYS_HZ;
}
void RecoveryModeLedOff(void)
{
	recovery_mode_start_led_off();
	recovery_mode_led_blink_timerHandler = (thand_f *)RecoveryModeLedOn;
	recovery_mode_led_blink_timeStart = get_timer(0);
	recovery_mode_led_blink_timeDelta = RECOVERY_MODE_BLINKING_OFF_INTERVEL * CONFIG_SYS_HZ;
}
#endif /* #if defined(CONFIG_UBOOT_RECOVERY_MODE_LED_BLINK) && defined(CONFIG_UBOOT_CUSTOMIZATION) */
...
#if defined(CONFIG_UBOOT_RECOVERY_MODE)// idleman-20140425. HTTP SERVER
	switch(protocol){
		case HTTP:
#if defined(CONFIG_UBOOT_RECOVERY_MODE_LED_BLINK) && defined(CONFIG_UBOOT_CUSTOMIZATION)
			RecoveryModeLedOn();
...
#if defined(CONFIG_UBOOT_RECOVERY_MODE)
		case HTTP:
			tcp_start();
			break;
#endif  /* #if defined(CONFIG_UBOOT_RECOVERY_MODE) */
...
#if defined(CONFIG_UBOOT_RECOVERY_MODE_LED_BLINK) && defined(CONFIG_UBOOT_CUSTOMIZATION)
		if(recovery_mode_led_blink_timerHandler && ((get_timer(0) - recovery_mode_led_blink_timeStart) > recovery_mode_led_blink_timeDelta))
		{
			recovery_mode_led_blink_timerHandler();
		}
#endif  /* #if defined(CONFIG_UBOOT_RECOVERY_MODE_LED_BLINK) && defined(CONFIG_UBOOT_CUSTOMIZATION) */
...
#ifdef CONFIG_UBOOT_RECOVERY_MODE
int NetCheckARPTable(uchar *ether, struct in_addr dest, int dport, int sport, int len)
{
	uchar *pkt;

	/* if MAC address was not discovered yet, save the packet and do an ARP request */
	if(memcpy(ether, net_null_ethaddr, 6) == 0)
	{
		net_arp_wait_packet_ip = dest;
		arp_wait_packet_ethaddr = ether;

		pkt = NetArpWaitTxPacket;
		pkt += net_set_ether(pkt, arp_wait_packet_ethaddr, PROT_IP);

		net_set_udp_header(pkt, dest, dport, sport, len);
		memcpy(pkt + IP_HDR_SIZE, (uchar *)net_tx_packet + (pkt - (uchar *)NetArpWaitTxPacket) + IP_HDR_SIZE, len);
		/* size of the waiting packet */
		arp_wait_tx_packet_size = (pkt - NetArpWaitTxPacket) + IP_HDR_SIZE + len;
		/* and do the ARP request */
		arp_wait_try = 1;
		arp_wait_timer_start = get_timer(0);
		arp_request();
...
int NetSendTCPPacket(uchar *ether, struct in_addr dest, int dport, int sport, int len)
{
	uchar *pkt;
	int eth_hdr_size;
	int pkt_hdr_size;
	...
	if (memcmp(ether, net_null_ethaddr, 6) == 0)
	{
		...
		/* size of the waiting packet */
		arp_wait_tx_packet_size = pkt_hdr_size + len;
	...
	{
		net_send_packet(net_tx_packet, pkt_hdr_size + len);
		return 0;	/* transmitted */
	}
}
#endif /* #ifdef CONFIG_UBOOT_RECOVERY_MODE */
...
void net_process_received_packet(uchar *in_packet, int len)
{
...
#ifdef CONFIG_UBOOT_RECOVERY_MODE /* idleman */
	datalen = len;
#endif  /* #ifdef CONFIG_UBOOT_RECOVERY_MODE */
	...
	case PROT_IP:
		...
		{
			if(protocolHandler(ip, datalen))
			{
				return;
			}
		}
#else   /* #ifdef CONFIG_UBOOT_RECOVERY_MODE */	

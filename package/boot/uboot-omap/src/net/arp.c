...
#if defined(CONFIG_UBOOT_RECOVERY_MODE) /* idleman */
	NetArpWaitTxPacket = NULL;/* The transmit packet */
#endif  /* #if defined(CONFIG_UBOOT_RECOVERY_MODE) */
...
#if defined(CONFIG_UBOOT_RECOVERY_MODE) /* idleman */
	NetArpWaitTxPacket = arp_tx_packet;/* The transmit packet */
#endif  /* #if defined(CONFIG_UBOOT_RECOVERY_MODE) */
}
...
void arp_receive(struct ethernet_hdr *et, struct ip_udp_hdr *ip, int len)
{
	...
	case ARPOP_REPLY:		/* arp reply */
		...
		if (reply_ip_addr.s_addr == net_arp_wait_reply_ip.s_addr) {
			...
			arp_wait_packet_ethaddr = NULL;
#if defined(CONFIG_UBOOT_RECOVERY_MODE) /* idleman */
			/* if Arp response requested by HTTP, send "TCP SYNC_ACK" packet immediately */
			if(g_httpd_started == 1) 
			{
				resend_tcp_sync_ack();
			}
#endif  /* #if defined(CONFIG_UBOOT_RECOVERY_MODE) */

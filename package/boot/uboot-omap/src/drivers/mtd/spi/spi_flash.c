...
static int spi_flash_block_lock_control(struct spi_flash *flash, int lock_flag)
{
	u8 buf[2], cmd;
	int ret = 0;
	if(lock_flag)/* lock the first 8 blocks */
	{
#ifdef CONFIG_SPI_FLASH_PROTECT_DISABLE
		printf("####################################\n");
		printf("# NO SPI FLASH WRITE PROTECTION\n");
		printf("####################################\n");
		buf[0] = BLOCK_NONE | MACRONIX_STATUS_WIP;
#else
		printf("Senao SPI FLASH: Set the protect block and WEL=0!!\n");
		buf[0] = BLOCK_AREA << BLOCK_SHIFH;
#endif  /* #ifdef CONFIG_SPI_FLASH_PROTECT_DISABLE */
	}
	else
	{
		printf("Senao SPI FLASH: Clear protect block and WEL=1!!\n");
		buf[0] = BLOCK_NONE | MACRONIX_STATUS_WIP;
	}
	buf[1] = CONFIGURATION_REG;
	cmd = CMD_WRITE_STATUS;
	ret = spi_flash_write_common(flash, &cmd, 1, &buf[0], 2);
	if (ret < 0) {
		debug("SF: fail to write status register\n");
		return ret;
	}
...
#endif  /* CONFIG_SPI_FLASH_BLOCK_LOCK */
...
#ifdef CONFIG_SPI_FLASH_ABNORMAL_WORKAROUND
...
#ifdef CONFIG_SPI_FLASH_ABNORMAL_WORKAROUND
		printf("SF: 0x%p => cmd = { 0x%02x 0x%02x%02x%02x } chunk_len = %zu\n",
...
#ifdef CONFIG_SPI_FLASH_MACRONIX /* idleman-20170327 */
#ifdef CONFIG_SPI_FLASH_BLOCK_LOCK
	if(data_len > (1024 * 1024))
	{
		/* large than 1024KiB means read kernel file then lock the block */
		spi_flash_block_lock_control(flash, BLOCK_LOCK);
	}
...
int spi_flash_scan(struct spi_flash *flash)
{
...
#if defined(CONFIG_SPI_FLASH_BLOCK_LOCK)
	...
	{
		spi_flash_block_lock_control(flash, BLOCK_UNLOCK);
	}
#else   /* #if defined(CONFIG_SPI_FLASH_BLOCK_LOCK) */
	/* Flash powers up read-only, so clear BP# bits */
	if (idcode[0] == SPI_FLASH_CFI_MFR_ATMEL ||
	    idcode[0] == SPI_FLASH_CFI_MFR_MACRONIX ||
	    idcode[0] == SPI_FLASH_CFI_MFR_SST)
		write_sr(flash, 0);
#endif  /* #if defined(CONFIG_SPI_FLASH_BLOCK_LOCK) */
	/* Assign spi data */

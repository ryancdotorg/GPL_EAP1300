...
I2C_ARRAY Sensor_init_table[] =     // new_SC2310_12bit_DVP_74.25Mpclk_30fps
{
    {0x0103,0x01},
    {0x0100,0x00},
    {0x3018,0x1f},
...
static int sc2310GetAEGain(ms_cus_sensor *handle, u32* gain)
{
    ...
    ANA_Fine_gainx1000 = 1000 * gain_reg[3].data/32;
    DIG_Fine_gainx1000 = 1000 * gain_reg[1].data/128;
    Coarse_gain = ((gain_reg[2].data&0x1C)>>2) +1;
    Dcg_gainx100 = ((gain_reg[2].data&0x20)>>5)>0 ? 272:100;
    DIG_gain = ((gain_reg[0].data&0x0f) + 1);
    ...
    *gain = (temp_gainx1000 * Coarse_gain * Dcg_gainx100 * DIG_gain) /100 /1000;
    SENSOR_DMSG("[%s] reg_data = 0x%x,  0x%x , 0x%x ,0x%x\n", __FUNCTION__, gain_reg[0].data,gain_reg[1].data,gain_reg[2].data,gain_reg[3].data);
...
static int sc2310SetAEGain(ms_cus_sensor *handle, u32 gain)
{
    ...
    gain_reg[3].data = ANA_Fine_gain_reg;
    gain_reg[2].data = (((Coarse_gain_reg<<2)|(Dcg_gain_reg<<5))&0x3C) | (0x03);
    gain_reg[1].data = DIG_Fine_gain_reg;
    gain_reg[0].data = DIG_gain_reg & 0xF;
...
int sc2310InitHandle(ms_cus_sensor* drv_handle)
{
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(sc2310_params));
...
static int __init sigmastar_sc2310_init_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
        ...
        {
            DrvRegisterSensorDriver(nCamID, sc2310InitHandle);
            pr_info("Connect %s linear to vif channel-%d\n",__FUNCTION__, nCamID);
        }
    }
    return 0;
}
static void __exit sigmastar_sc2310_exit_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
...
subsys_initcall(sigmastar_sc2310_init_driver);
module_exit(sigmastar_sc2310_exit_driver);

MODULE_DESCRIPTION("Sensor_sc2310");

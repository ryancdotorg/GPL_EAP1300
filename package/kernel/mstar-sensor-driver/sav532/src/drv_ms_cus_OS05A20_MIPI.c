...
#define Preview_line_period_DCG 13333//17814                     // MCLK=21.6 HTS/PCLK=3080 pixels/97.2MHZ=31.687us                              // 3126 for 25fps
#define vts_30fps           1984                               // VTS for 20fps
#define vts_30fps_DCG       2500                               // VTS for 20fps
...
{
//@@ Res 2688X1944 4lane MIPI1440Mbps HDRVC10 30fps MCLK24M
    {0x0100, 0x00},
    {0x0103, 0x01},
    {0x0303, 0x01},
    {0x0305, 0x5a},
...
static int OS05A20_SetOrien_HDR(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit)
{
    ...
    switch(orit)
    {
        ...
        break;
        case CUS_ORIT_M0F1:
            params->tMirror_reg_DCG[0].data = 0x20;
            params->tMirror_reg_DCG[1].data = 0x20;
            params->mirror_dirty = true;
            break;
        case CUS_ORIT_M1F1:
...
static int OS05A20_SetFPS_HDR_lef(ms_cus_sensor *handle, u32 fps)
{
    ...
    if(fps>=5 && fps <= 30){
        params->expo.fps = fps;
        params->expo.vts=  (vts_30fps_DCG*30)/fps;
    }else if(fps>=5000 && fps <= 30000){
        params->expo.fps = fps;
        params->expo.vts=  (vts_30fps_DCG*30000)/fps;
    }else{
        SENSOR_DMSG("[%s] FPS %d out of range.\n",__FUNCTION__,fps);
        return FAIL;
    }
...
static int OS05A20_GetShutterInfo_HDR(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    info->max = 1000000000/Preview_MIN_FPS;
...
int cus_camsensor_init_handle_hdr_dcg_lef(ms_cus_sensor* drv_handle)
{
    ...
    handle->pCus_sensor_GetShutterInfo = OS05A20_GetShutterInfo_HDR;

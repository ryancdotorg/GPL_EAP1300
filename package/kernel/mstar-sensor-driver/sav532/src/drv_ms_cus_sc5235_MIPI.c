...
#define Prv_Max_line_number 1920                    //maximum exposure line munber of sensor when preview
...
//2560x1944 20fps  594Mbps  118.8Mpix/s  356.4MHz cnt
...
I2C_ARRAY Sensor_init_table_2048x1536_29p98fps[] =
{
{0x0103, 0x01}, //soft reset
{0x0100, 0x00}, //stream stop
...
{0x3314, 0x14},
{0x3637, 0x20},
{0x3306, 0x50},//20180918[2c,6b]	330b=b0
{0x330b, 0xb0},//20180918[8d,cb]	3306=50
...
{0x320f, 0x3a},
{0x3f04, 0x02},
{0x3f05, 0x5c},  // hts / 2 - 3f08 - 0x04
{0x3235, 0x0c},
{0x3236, 0x72}, // vts x 2 - 2
{0x3200, 0x01},
{0x3201, 0x14},
{0x3202, 0x00},
{0x3203, 0xd2},
...
static int pCus_init_4M25fps(ms_cus_sensor *handle)
{
	int i,cnt=0;
	//ISensorIfAPI *sensor_if = &handle->sensor_if_api;
        SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
...
static int pCus_init_3M30fps(ms_cus_sensor *handle)
{
	int i,cnt=0;
	//ISensorIfAPI *sensor_if = &handle->sensor_if_api;
        SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
...
static int pCus_init_3M329p98fps(ms_cus_sensor *handle)
{
	int i,cnt=0;
	//ISensorIfAPI *sensor_if = &handle->sensor_if_api;
        SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_2048x1536_29p98fps);i++)
    {
	    if(Sensor_init_table_2048x1536_29p98fps[i].reg==0xffff)
	    {
		    SENSOR_MSLEEP(Sensor_init_table_2048x1536_29p98fps[i].data);
	    }
	    else
	    {
		    cnt = 0;
		    while(SensorReg_Write(Sensor_init_table_2048x1536_29p98fps[i].reg,Sensor_init_table_2048x1536_29p98fps[i].data) != SUCCESS)
		    {
			    cnt++;
			    //SENSOR_DMSG("Sensor_init_table_2048x1536_29p98fps -> Retry %d...\n",cnt);
			    if(cnt>=10)
			    {
				    //SENSOR_DMSG("[%s:%d]Sensor init fail!!\n", __FUNCTION__, __LINE__);
				    return FAIL;
...
static int pCus_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        ...
        case 1:
            ...
            handle->pCus_sensor_init = pCus_init_4M25fps;
            vts_30fps = 1620;
            params->expo.vts = vts_30fps;
            params->expo.fps=25;
        ...
        case 2:
            ...
            handle->pCus_sensor_init = pCus_init_3M30fps;
            vts_30fps = 1500;
            params->expo.vts = vts_30fps;
            params->expo.fps=30;
        ...
        case 3:
            ...
            handle->pCus_sensor_init = pCus_init_3M329p98fps;
            vts_30fps = 1594;
            params->expo.vts = vts_30fps;
            params->expo.fps=30;
            Preview_line_period = 20923;
            break;
        case 4:
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    handle->video_res_supported.res[1].height = 1520;
    ...
    sprintf(handle->video_res_supported.res[1].strResDesc, "2592x1520@25fps");

    handle->video_res_supported.num_res = 3;
    handle->video_res_supported.ulcur_res = 0;
    handle->video_res_supported.res[2].width = 2304;
    handle->video_res_supported.res[2].height = 1296;
    ...
    handle->video_res_supported.res[2].nOutputWidth= 2304;
    handle->video_res_supported.res[2].nOutputHeight= 1296;
    sprintf(handle->video_res_supported.res[2].strResDesc, "2304x1296@30fps");

    handle->video_res_supported.num_res = 4;
    handle->video_res_supported.ulcur_res = 0;
    handle->video_res_supported.res[3].width = 2048;
    handle->video_res_supported.res[3].height = 1536;
    handle->video_res_supported.res[3].max_fps= 30;//29.98
    ...
    handle->video_res_supported.res[3].nOutputWidth= 2048;
    handle->video_res_supported.res[3].nOutputHeight= 1536;
    sprintf(handle->video_res_supported.res[3].strResDesc, "2048x1536@30fps");
    ...
    handle->video_res_supported.res[4].max_fps= 20;//29.98
    ...
    sprintf(handle->video_res_supported.res[4].strResDesc, "2592x1944@@20fps");

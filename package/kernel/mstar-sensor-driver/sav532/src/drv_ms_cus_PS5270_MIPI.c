...
#define SENSOR_DATAPREC     CUS_DATAPRECISION_16    //CFG //CUS_DATAPRECISION_8, CUS_DATAPRECISION_10
...
I2C_ARRAY Sensor_init_table_HDR[] =
{
    {0xEF, 0x05}, //bank5
    {0x0F, 0x00},
    {0x42, 0x00},
    ...
    switch(status)
    {
        ...
        case CUS_FRAME_ACTIVE:
          ...
          if(params->orient_dirty)
          {
              ...
              //printf("[%s] set3 gain/regH/regL=%d/0x%x/0x%x\n", __FUNCTION__,gain_reg[1].data,gain_reg[2].data,gain_reg[3].data);
          }

			// if(fps_delay == 4){
				// sensor_if->SetSkipFrame(3);
			// }
...
static int pCus_SetAEGain_cal(ms_cus_sensor *handle, u32 gain) {
  ...
  gain_reg[1].data = (gain_reg[2].data)?i:(i-64);
  SENSOR_DMSG("[%s] set gain/reg=%d/0x%x\n", __FUNCTION__, gain,gain_reg[1].data);
  return SUCCESS;
}
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
  ...
  //printf("[%s] set total_gain=%d %f\n", __FUNCTION__,gain, total_gain_double);
  ...
  if(total_gain_double < 50000){
    sens = 1;     // LS
  //}else if(total_gain_double > 60000){
  }else{
    sens = 0;     // HS
  }
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(ps5270_params));
    ...
    SENSOR_DMSG(handle->video_res_supported.res[0].strResDesc, "1536x1536@25fps");
    ...
    strcpy(handle->video_res_supported.res[0].strResDesc, "1536x1536HDR@25fps");
...
static int __init sigmastar_ps5270_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_ps5270_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_ps5270_init_driver);
module_exit(sigmastar_ps5270_exit_driver);

MODULE_DESCRIPTION("Sensor_ps5270");

...
{
    MI_U16 u16FreqThrd;                //0~4095 default:528
    MI_U16 u16EdgeScoreThrd;           //0~4095 default:112
    MI_U16 u16ChromaThrdOfStrengthMax; //0~2047 default:360
    MI_U16 u16ChromaThrdOfStrengthMid; //0~2047 default:640
    MI_U16 u16ChromaThrdOfStrengthMin; //0~2047 default:1280
    MI_U8  u8StrengthMid;              //0~11   default:10
    MI_U8  u8StrengthMin;              //0~11   default 9
} FALSECOLOR_PARAM_t;
...
typedef struct NR3D_PARAM_s
{
    ...
    MI_U8  u8YSfStr;            //0 ~ 255
    MI_U8  u8YSfBlendLut[16];   //0 ~ 31

    MI_U8  u8CSfStr;            //0 ~ 255
    MI_U8  u8CSfExStr;          //0 ~ 255
    MI_U8  u8CSfExBlendGain;    //0 ~ 255
    MI_U16 u16CSfExBlendClip;   //0 ~ 511

    MI_U16 u16ShpBlendLut[16];  //0 ~ 1023
} NR3D_PARAM_t;
...
{
    MI_U8 u8BlendRatio;                 //0 ~ 63
    MI_U8 u8FilterLevel;                //0 ~ 5
    MI_U8 u8StrengthByY[16];            //0 ~ 63
    MI_ISP_BOOL_e bEnLscReference;      //0 ~ 1
} NRLUMA_PARAM_t;
...
{
    MI_U16 u16StrengthByY[8];      //0 ~ 511
    MI_U16 u16StrengthByYEdge[8];  //0 ~ 511
    MI_U16 u16StrengthByCEdge[8];  //0 ~ 511
    MI_U16 u16MaxStrength;          //0 ~ 950
} NRChroma_PARAM_t;
...
typedef struct WDR_PARAM_s
{
    ...
    MI_U8         u8ManualDetailEnhanceTbl[10];   //0~255; default:128
...
{
    MI_U16 u16DirThrd;            //129~511  def:141
    MI_U16 u16ColorGrayRatio;     //0~511    def:0
    MI_U16 u16EdgeSmoothThrdY;    //0~4095   def:3680
    MI_U16 u16EdgeSmoothThrdC;    //0~2047   def:160
} DEMOSAIC_PARAM_t;
...
{
    MI_ISP_BOOL_e bEnable;              //0 ~ 1
    MI_U16 u16TableR[4210];
    MI_U16 u16TableG[4210];
    MI_U16 u16TableB[4210];
...
typedef struct
{
    ...
    MI_U32 shutter_min;            /**< shutter Shutter min ns*/
    MI_U32 shutter_max;            /**< shutter Shutter max ns*/
...
typedef enum
{
    E_RAW_STORE_NODE_P0HEAD = 0, /* Control by VIF, Do not use */
    E_RAW_STORE_NODE_P1HEAD = 1, /* Control by VIF, Do not use */
    E_RAW_STORE_NODE_P0TAIL = 2,
    E_RAW_STORE_NODE_P1TAIL = 3,
    E_RAW_STORE_NODE_ISPOUT = 4,
    E_RAW_STORE_NODE_VDOS   = 5,
    E_RAW_STORE_NODE_ISPOUT_BEFORE_YEE   = 6,
    E_RAW_STORE_NODE_RGBIR_IR_ONLY = 7

...
#else
    handle->private_data = CamOsMemCalloc( 1, sizeof(ov4689_params) );
#endif
...
static int __init sigmastar_ov4689_init_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
        ...
        {
            DrvRegisterSensorDriver(nCamID, ov4689_MIPI_cus_camsensor_init_handle);
            pr_info("Connect %s linear to sensor pad %d\n",__FUNCTION__, nCamID);
            DrvRegisterPlaneDriver(nCamID, 1, ov4689_MIPI_cus_camsensor_init_handle);
            pr_info("Connect %s SEF to sensor pad %d\n",__FUNCTION__, nCamID);
            DrvRegisterPlaneDriver(nCamID, 0, ov4689_MIPI_cus_camsensor_init_handle);
            pr_info("Connect %s LEF to sensor pad %d\n",__FUNCTION__, nCamID);
        }
    }
    return 0;
}
static void __exit sigmastar_ov4689_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_ov4689_init_driver);
module_exit(sigmastar_ov4689_exit_driver);

MODULE_DESCRIPTION("Sensor_ov4689");

...
I2C_ARRAY Sensor_init_table[] =     // 1920*1080_30fps_27MCLK_756MMIPI_2840*1109*30
{
...
static int pCus_SetOrien(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit) {
   ...
   switch(orit) {
      case CUS_ORIT_M0F0:
        index = 0;
        mirror_reg[3].data = 0x00;
        mirror_reg[6].data = 0x07;
        mirror_reg[7].data = 0x08;
        handle->bayer_id = CUS_BAYER_BG;
        break;
      case CUS_ORIT_M1F0:
        index = 0x01;
        mirror_reg[3].data = 0x00;//Mirror/Flip
        mirror_reg[6].data = 0x06; //mem down en + enable auto BR first
        mirror_reg[7].data = 0x09; //vertical start
        handle->bayer_id = CUS_BAYER_RG;
        break;
      case CUS_ORIT_M0F1:
        index = 0x02;
        mirror_reg[3].data = 0x02;
        mirror_reg[6].data = 0x06;
        mirror_reg[7].data = 0x09;
        handle->bayer_id = CUS_BAYER_BG;
        break;
      case CUS_ORIT_M1F1:
        index = 0x03;
        mirror_reg[3].data = 0x02;
        mirror_reg[6].data = 0x07;
        mirror_reg[7].data = 0x08;//0x09;
        handle->bayer_id = CUS_BAYER_RG;
        break;
    }
    SENSOR_DMSG("pCus_SetOrien:%x\r\n", index);

    if (index != mirror_reg[2].data) {
      mirror_reg[2].data = index;
...
static int pCus_SetAEUSecs(ms_cus_sensor *handle, u32 us) {
   ...
   // lines <<= 4;
    expo_reg[0].data =(u16)( (lines>>8) & 0x00ff);
    expo_reg[1].data =(u16)( (lines>>0) & 0x00ff);
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
    ...
    if(gain16>255)
       gain16=255;
   // gain_reg[0].data = (gain16>>8)&0x01;//high bit
    gain_reg[0].data = gain16&0xff; //low byte
/*
    if(input_gain > MAX_A_GAIN){

            gain_reg[1].data=(u16)( (input_gain/MAX_A_GAIN)*128)&0xFF;
        }
    else{

            gain_reg[1].data=0x80;
        }
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(ov2735_params));
...
static int __init sigmastar_ov2735_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_ov2735_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_ov2735_init_driver);
module_exit(sigmastar_ov2735_exit_driver);

MODULE_DESCRIPTION("Sensor_ov2735");

...
static int sc2232SetAEGain(ms_cus_sensor *handle, u32 gain)
{
    ...
    if (gain <= (155/10) * 1024)
    {
        ...
        for(i = 1; i< sizeof(fine_again)/sizeof(FINE_GAIN);i++)
        {
            ...
            else if(Fine_again > fine_again[15].gain)
            {
                Fine_again_reg = 0x1f;
                break;
            }
...
int sc2232InitHandle(ms_cus_sensor* drv_handle)
{
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(sc2232_params));
...
static int __init sigmastar_sc2232_init_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
        ...
        {
            DrvRegisterSensorDriver(nCamID, sc2232InitHandle);
            pr_info("Connect %s linear to vif channel-%d\n",__FUNCTION__, nCamID);
        }
    }
    return 0;
}
static void __exit sigmastar_sc2232_exit_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
...
subsys_initcall(sigmastar_sc2232_init_driver);
module_exit(sigmastar_sc2232_exit_driver);

MODULE_DESCRIPTION("Sensor_sc2232");

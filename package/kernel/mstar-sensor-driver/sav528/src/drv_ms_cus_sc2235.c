...
I2C_ARRAY Sensor_init_table[] =     // PARA12_1928*1088_30fps_27MCLK_74.25PCLK
{
{0x0103,0x01},
{0xffff,0x05},    // delay
...
static int sc2235AEStatusNotify(ms_cus_sensor *handle, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    ...
    switch(status)
    {
        ...
        case CUS_FRAME_ACTIVE:
            ...
            SensorRegArrayW((I2C_ARRAY*)nr3_reg, sizeof(nr3_reg)/sizeof(I2C_ARRAY));
...
static int sc2235SetAEUSecs(ms_cus_sensor *handle, u32 us)
{
    ...
    lines = lines<<4;
    expo_reg[0].data = (lines>>16) & 0x000f;
    expo_reg[1].data = (lines>>8) & 0x00ff;
    expo_reg[2].data = (lines>>0) & 0x00f0;
...
static int sc2235GetAEGain(ms_cus_sensor *handle, u32* gain)
{
    ...
    Coarse_gain = ((gain_reg[1].data&0x1C)>>2) +1;
    Dgain = (((gain_reg[1].data&0xE0)>>5) + ((gain_reg[0].data<<3)&0x08) + 1);
...
static int sc2235SetAEGain(ms_cus_sensor *handle, u32 gain)
{
    ...
    u8 i=0 , Dgain = 1, Coarse_gain = 1; u32 Fine_gain = 10000;
    ...
    gain_reg[2].data = Fine_gain_reg;
    gain_reg[1].data = ((Coarse_gain_reg<<2)&0x1C) | ((Dgain_reg<<5)&0xE0) | 0x03;
    gain_reg[0].data = (Dgain_reg>>2) & 0x01;
    //pr_info("[%s] set gain =%d 0x%x ,0x%x, 0x%x\n", __FUNCTION__, gain,gain_reg[0].data,gain_reg[1].data,gain_reg[2].data);

    if (Fine_gain_reg == 0x1f && Coarse_gain_reg == 7) {
        nr2_reg[0].data = 0x3a;
        nr2_reg[1].data = 0x06;
    } else {
        nr2_reg[0].data = 0x2f;
...
int sc2235InitHandle(ms_cus_sensor* drv_handle)
{
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(sc2235_params));
...
static int __init sigmastar_sc2235_init_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
        ...
        {
            DrvRegisterSensorDriver(nCamID, sc2235InitHandle);
            pr_info("Connect %s linear to vif channel-%d\n",__FUNCTION__, nCamID);
        }
    }
    return 0;
}
static void __exit sigmastar_sc2235_exit_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
...
subsys_initcall(sigmastar_sc2235_init_driver);
module_exit(sigmastar_sc2235_exit_driver);

MODULE_DESCRIPTION("Sensor_sc2235");

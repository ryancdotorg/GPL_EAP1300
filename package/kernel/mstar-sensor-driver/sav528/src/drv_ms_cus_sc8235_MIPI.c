...
{
////Apollo_20fps_10bit_4lane_708.75Mbps_3840x2160(HTS=6300,VTS=2250)
    {0x0103,0x01},
    {0x0100,0x00},
    {0x336c,0xc2},
    {0x363b,0x08},
...
{
///Apollo_15fps_10bit_4lane_486Mbps_3840x2160_(HTS=5000,VTS=2592)
    {0x0103,0x01},
    {0x0100,0x00},
    {0x336c,0xc2},
    {0x363b,0x08},
...
{
///Apollo_MIPI_4lane_10bit_583.2Mbps_2560x1440_30fps_(HTS=2592,VTS=1500)
    {0x0103,0x01},
    {0x0100,0x00},
    {0x336c,0xc2},
    {0x363b,0x08},
...
{
///////Apollo_MIPI_10bit_4lane_720Mbps_1920x1080_60fps_hbin_vbin_(HTS=2133,VTS=1125).ini
    {0x0103,0x01},
    {0x0100,0x00},
    {0x336c,0xc2},
    {0x363b,0x08},
...
{
//DOL mode, Apollo_hdr(vc)_15fps_10bit_4lane_708.75Mbps_3840x2160_(HTS=4200,VTS=4500)
    {0x0103,0x01},
    {0x0100,0x00},
    {0x336c,0xc2},
    {0x363b,0x08},
...
static int pCus_poweroff(ms_cus_sensor *handle, u32 idx)
{
    // power/reset low
    ISensorIfAPI *sensor_if = &handle->sensor_if_api;
    sc8235_params *params = (sc8235_params *)handle->private_data;
    SENSOR_DMSG("[%s] power low\n", __FUNCTION__);
    sensor_if->PowerOff(idx, handle->pwdn_POLARITY);
...
static int pCus_GetSensorID(ms_cus_sensor *handle, u32 *id)
{
    ...
    *id >>= 8;
    SENSOR_DMSG("[%s]sc8235 Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);
    //SENSOR_DMSG("[%s]Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);

    return SUCCESS;
}

static int sc8235_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
...
static int sc8235_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle_linear(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(sc8235_params));
    params = (sc8235_params *)handle->private_data;

    ////////////////////////////////////
    //    sensor model ID                           //
    ////////////////////////////////////
    SENSOR_DMSG(handle->model_id,"sc8235_MIPI");
    ...
    handle->pCus_sensor_SetPatternMode = sc8235_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = sc8235_GetShutterInfo;
   // params->expo.vts=vts_30fps;
   // params->expo.fps = 30;
...
{
    ms_cus_sensor *handle = drv_handle;
    sc8235_params *params = NULL;

    cus_camsensor_init_handle_linear(drv_handle);
    params = (sc8235_params *)handle->private_data;

    sprintf(handle->model_id,"sc8235_MIPI_HDR_SEF");
...
static int __init sigmastar_sc8235_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_sc8235_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_sc8235_init_driver);
module_exit(sigmastar_sc8235_exit_driver);

MODULE_DESCRIPTION("Sensor_sc8235");

...
typedef struct {
    ...
    I2C_ARRAY *tGain_reg;
    CUS_GAIN_GAP_ARRAY *tGain_gap_compensate;
    I2C_ARRAY *tExpo_reg;
    I2C_ARRAY *tExpo_shr_dol1_reg;
    I2C_ARRAY *tExpo_rhs1_reg;
    I2C_ARRAY *tExpo_shr_dol2_reg;
    I2C_ARRAY *tVts_reg;
    I2C_ARRAY *tPatternTbl;
...
#endif
    //UartSendTrace("IMX230 Sensor_init_table_HDR_DOL_4lane\n");
...
int cus_camsensor_init_handle_linear(ms_cus_sensor* drv_handle)
{
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(imx230_params));
    params = (imx230_params *)handle->private_data;
    //SENSOR_DMSG("----~~~~~~~~~~~~-----cus_camsensor_init_handle_linear----~~~~~~~~~~~~~~`-\n");
    g_cur_init = params;
    ////////////////////////////////////
    //    sensor private data         //
    ////////////////////////////////////
    params->tCurrent_Mirror_Flip_Tbl = CamOsMemCalloc(1, sizeof(Current_Mirror_Flip_Tbl));
    params->tGain_reg                = CamOsMemCalloc(1, sizeof(gain_reg));
    params->tGain_gap_compensate     = CamOsMemCalloc(1, sizeof(gain_gap_compensate));
    params->tExpo_reg                = CamOsMemCalloc(1, sizeof(expo_reg));
    params->tExpo_shr_dol1_reg       = CamOsMemCalloc(1, sizeof(expo_shr_dol1_reg));
    params->tExpo_rhs1_reg           = CamOsMemCalloc(1, sizeof(expo_rhs1_reg));
    params->tExpo_shr_dol2_reg       = CamOsMemCalloc(1, sizeof(expo_shr_dol2_reg));
    params->tVts_reg                 = CamOsMemCalloc(1, sizeof(vts_reg));
    params->tPatternTbl              = CamOsMemCalloc(1, sizeof(PatternTbl));
...
int cus_camsensor_init_handle_hdr_dol_sef(ms_cus_sensor* drv_handle)
{
    ...
    //    sensor private data         //
    ////////////////////////////////////
    params->tCurrent_Mirror_Flip_Tbl = kzalloc(sizeof(Current_Mirror_Flip_Tbl),GFP_KERNEL);
    params->tGain_reg                = kzalloc(sizeof(gain_reg),GFP_KERNEL);
    params->tGain_gap_compensate     = kzalloc(sizeof(gain_gap_compensate),GFP_KERNEL);
    params->tExpo_reg                = kzalloc(sizeof(expo_reg),GFP_KERNEL);
    params->tExpo_shr_dol1_reg       = kzalloc(sizeof(expo_shr_dol1_reg),GFP_KERNEL);
    params->tExpo_rhs1_reg           = kzalloc(sizeof(expo_rhs1_reg),GFP_KERNEL);
    params->tExpo_shr_dol2_reg       = kzalloc(sizeof(expo_shr_dol2_reg),GFP_KERNEL);
    params->tVts_reg                 = kzalloc(sizeof(vts_reg),GFP_KERNEL);
    params->tPatternTbl              = kzalloc(sizeof(PatternTbl),GFP_KERNEL);
...
int cus_camsensor_init_handle_hdr_dol_lef(ms_cus_sensor* drv_handle)
{
    ...
    memcpy(params->tPatternTbl,PatternTbl,sizeof(PatternTbl));
#else
    handle->private_data = g_cur_init;
    params = (imx230_params *)handle->private_data;
#endif
...
int cus_camsensor_release_handle_dol_lef(ms_cus_sensor *handle) {
    ...
    if (handle && handle->private_data) {
        SENSOR_DMSG("[%s] release handle, handle %x, private data %x",
                ...
                (int)handle,
                (int)handle->private_data);
        kvfree(params->tCurrent_Mirror_Flip_Tbl);
        kvfree(params->tGain_reg);
        kvfree(params->tGain_gap_compensate);
        kvfree(params->tExpo_reg);
        kvfree(params->tExpo_shr_dol1_reg);
        kvfree(params->tExpo_rhs1_reg);
        kvfree(params->tExpo_shr_dol2_reg);
        kvfree(params->tVts_reg);
        kvfree(params->tPatternTbl);
        kvfree(handle->private_data);
...
static int __init sigmastar_imx230_hdr_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_imx230_hdr_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_imx230_hdr_init_driver);
module_exit(sigmastar_imx230_hdr_exit_driver);

MODULE_DESCRIPTION("Sensor_IMX230_HDR");

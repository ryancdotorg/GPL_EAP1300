...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(opn008_params));
...
static int __init sigmastar_opn008_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_opn008_exit_driver(void)
{
    int nCamID=0;
    for(nCamID=0;nCamID<4;++nCamID)
    {
...
subsys_initcall(sigmastar_opn008_init_driver);
module_exit(sigmastar_opn008_exit_driver);

MODULE_DESCRIPTION("Sensor_OPN008");

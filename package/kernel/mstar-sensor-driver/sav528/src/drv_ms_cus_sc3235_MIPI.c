...
#define SENSOR_MAXGAIN      (1575*39375)/1000000       //  (15.5*31)   max sensor gain, a-gain*conversion-gain*d-gain
...
int cus_camsensor_init_handle_linear(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    handle->private_data = CamOsMemCalloc(1, sizeof(sc3235_params));
...
static int __init sigmastar_sc3235_init_driver(void)
{
    int nCamID=0;
...
static void __exit sigmastar_sc3235_exit_driver(void)
{
    pr_info("sensordrv exit");
}

subsys_initcall(sigmastar_sc3235_init_driver);
module_exit(sigmastar_sc3235_exit_driver);

MODULE_DESCRIPTION("Sensor_sc3235");
